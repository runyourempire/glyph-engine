// GLYPH Component: primordial — auto-generated, do not edit.
(function(){
...
fn apply_color_matrix(color: vec3f) -> vec3f {
    let m = mat3x3f(
        vec3f(0.92, 0, 0.1),
        vec3f(-0.03, 0.85, 0.02),
        vec3f(0.15, 0.08, 1.25)
    );
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let ignition = u.p_ignition;
    let expansion = u.p_expansion;
    let consciousness = u.p_consciousness;
    ...
    // ── Layer 1: void ──
    {
        ...
        { let warp_x = fbm2(p * 1.800000 + vec2<f32>(0.0, 1.3), i32(6.000000), 0.550000, (0.180000 + (ignition * 0.120000)));
        let warp_y = fbm2(p * 1.800000 + vec2<f32>(1.7, 0.0), i32(6.000000), 0.550000, (0.180000 + (ignition * 0.120000)));
        p = p + vec2<f32>(warp_x, warp_y) * (0.180000 + (ignition * 0.120000)); }
        var sdf_result = fbm2((p * 2.500000 + vec2<f32>(time * 0.1, time * 0.07)), i32(6.000000), 0.500000, 2.000000);
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.010000, 0.005000, 0.025000), vec3<f32>(0.050000, 0.025000, 0.100000), vec3<f32>(0.200000, 0.100000, 0.400000), vec3<f32>(0.000000, 0.050000, 0.150000));
    ...
    // ── Layer 2: spiral ──
    {
        ...
        { let warp_x = fbm2(p * 4.000000 + vec2<f32>(0.0, 1.3), i32(5.000000), 0.500000, (0.250000 + (expansion * 0.120000)));
        let warp_y = fbm2(p * 4.000000 + vec2<f32>(1.7, 0.0), i32(5.000000), 0.500000, (0.250000 + (expansion * 0.120000)));
        p = p + vec2<f32>(warp_x, warp_y) * (0.250000 + (expansion * 0.120000)); }
        var sdf_result = fbm2((p * 5.000000 + vec2<f32>(time * 0.1, time * 0.07)), i32(5.000000), 0.500000, 2.000000);
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.020000, 0.010000, 0.040000), vec3<f32>(0.150000, 0.080000, 0.300000), vec3<f32>(0.500000, 0.300000, 0.700000), vec3<f32>(0.000000, 0.100000, 0.250000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>(0.080000, (-0.040000));
        { let warp_x = fbm2(p * 2.500000 + vec2<f32>(0.0, 1.3), i32(5.000000), 0.550000, (0.200000 + (consciousness * 0.080000)));
        let warp_y = fbm2(p * 2.500000 + vec2<f32>(1.7, 0.0), i32(5.000000), 0.550000, (0.200000 + (consciousness * 0.080000)));
        p = p + vec2<f32>(warp_x, warp_y) * (0.200000 + (consciousness * 0.080000)); }
        var sdf_result = fbm2((p * 3.500000 + vec2<f32>(time * 0.1, time * 0.07)), i32(5.000000), 0.500000, 2.000000);
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.0, 0.5, 0.3), vec3<f32>(0.2, 0.5, 0.4), vec3<f32>(1.0, 1.0, 1.0), vec3<f32>(0.0, 0.1, 0.3));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    // ── Layer 4: filaments ──
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = vec2<f32>(length(p), atan2(p.y, p.x));
        p = p + vec2<f32>(sin(p.y * 3.000000 + time * 0.500000), cos(p.x * 3.000000 + time * 0.500000)) * (0.050000 + (expansion * 0.030000));
        { let warp_x = fbm2(p * 5.000000 + vec2<f32>(0.0, 1.3), i32(4.000000), 0.400000, 0.150000);
        let warp_y = fbm2(p * 5.000000 + vec2<f32>(1.7, 0.0), i32(4.000000), 0.400000, 0.150000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.150000; }
        var sdf_result = noise2(p * 10.000000 + vec2<f32>(time * 0.1, time * 0.07));
        let glow_pulse = (0.600000 + (expansion * 0.400000)) * (0.9 + 0.1 * sin(time * 2.0));
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let warp_x = fbm2(p * 2.500000 + vec2<f32>(0.0, 1.3), i32(3.000000), 0.100000, 2.000000);
        let warp_y = fbm2(p * 2.500000 + vec2<f32>(1.7, 0.0), i32(3.000000), 0.100000, 2.000000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.100000; }
        var sdf_result = voronoi2(p * 22.000000 + vec2<f32>(time * 0.05, time * 0.03));
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.000000, 0.000000, 0.010000), vec3<f32>(0.050000, 0.050000, 0.080000), vec3<f32>(0.900000, 0.900000, 1.000000), vec3<f32>(0.000000, 0.000000, 0.000000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p + vec2<f32>(sin(p.y * 2.000000 + time * 0.400000), cos(p.x * 2.000000 + time * 0.400000)) * (0.015000 + (ignition * 0.010000));
        var sdf_result = sdf_circle(p, (0.030000 + (ignition * 0.020000)));
        let glow_pulse = (3.500000 + (ignition * 2.000000)) * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(1.000000, 0.820000, 0.400000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p + vec2<f32>(sin(p.y * 1.800000 + time * 0.300000), cos(p.x * 1.800000 + time * 0.300000)) * (0.012000 + (consciousness * 0.008000));
        var sdf_result = abs(length(p) - (0.140000 + (consciousness * 0.040000))) - 0.003000;
        let glow_pulse = (1.800000 + (consciousness * 1.200000)) * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p + vec2<f32>(sin(p.y * 1.500000 + time * 0.700000), cos(p.x * 1.500000 + time * 0.700000)) * 0.012000;
        var sdf_result = sdf_circle(p, (0.060000 + (consciousness * 0.025000)));
        for (var onion_i: i32 = 0; onion_i < i32(3.000000); onion_i = onion_i + 1) { sdf_result = abs(sdf_result) - 0.003000; }
        let glow_pulse = (1.000000 + (consciousness * 0.800000)) * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.300000, 0.220000, 0.550000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>((sin((time * 0.400000)) * ((0.130000 + (consciousness * 0.070000)))), (cos((time * 0.400000)) * ((0.130000 + (consciousness * 0.070000)))));
        var sdf_result = sdf_star(p, 5.000000, (0.015000 + (consciousness * 0.007000)), 0.006000);
        let glow_pulse = (2.000000 + (consciousness * 0.800000)) * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(1.000000, 0.650000, 0.250000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        var sdf_result = abs(length(p) - 0.460000) - 0.001000;
        let glow_pulse = (0.300000 + (consciousness * 0.150000)) * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.200000, 0.150000, 0.400000), color_result.a);
        let la = color_result.a;
        let lc = color_result.rgb;
        final_color = vec4<f32>(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
...
uniform float u_p_ignition;
uniform float u_p_expansion;
uniform float u_p_consciousness;
...
void main(){
    ...
    float ignition = u_p_ignition;
    float expansion = u_p_expansion;
    float consciousness = u_p_consciousness;
    ...
    // ── Layer 1: void ──
    {
        ...
        { float warp_x = fbm2(p * 1.800000 + vec2(0.0, 1.3), int(6.000000), 0.550000, (0.180000 + (ignition * 0.120000)));
        float warp_y = fbm2(p * 1.800000 + vec2(1.7, 0.0), int(6.000000), 0.550000, (0.180000 + (ignition * 0.120000)));
        p = p + vec2(warp_x, warp_y) * (0.180000 + (ignition * 0.120000)); }
        float sdf_result = fbm2((p * 2.500000 + vec2(time * 0.1, time * 0.07)), int(6.000000), 0.500000, 2.000000);
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.010000, 0.005000, 0.025000), vec3(0.050000, 0.025000, 0.100000), vec3(0.200000, 0.100000, 0.400000), vec3(0.000000, 0.050000, 0.150000));
    ...
    // ── Layer 2: spiral ──
    {
        ...
        { float warp_x = fbm2(p * 4.000000 + vec2(0.0, 1.3), int(5.000000), 0.500000, (0.250000 + (expansion * 0.120000)));
        float warp_y = fbm2(p * 4.000000 + vec2(1.7, 0.0), int(5.000000), 0.500000, (0.250000 + (expansion * 0.120000)));
        p = p + vec2(warp_x, warp_y) * (0.250000 + (expansion * 0.120000)); }
        float sdf_result = fbm2((p * 5.000000 + vec2(time * 0.1, time * 0.07)), int(5.000000), 0.500000, 2.000000);
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.020000, 0.010000, 0.040000), vec3(0.150000, 0.080000, 0.300000), vec3(0.500000, 0.300000, 0.700000), vec3(0.000000, 0.100000, 0.250000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.880000);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2(0.080000, (-0.040000));
        { float warp_x = fbm2(p * 2.500000 + vec2(0.0, 1.3), int(5.000000), 0.550000, (0.200000 + (consciousness * 0.080000)));
        float warp_y = fbm2(p * 2.500000 + vec2(1.7, 0.0), int(5.000000), 0.550000, (0.200000 + (consciousness * 0.080000)));
        p = p + vec2(warp_x, warp_y) * (0.200000 + (consciousness * 0.080000)); }
        float sdf_result = fbm2((p * 3.500000 + vec2(time * 0.1, time * 0.07)), int(5.000000), 0.500000, 2.000000);
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.0, 0.5, 0.3), vec3(0.2, 0.5, 0.4), vec3(1.0, 1.0, 1.0), vec3(0.0, 0.1, 0.3));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.890000);
        float la = color_result.a * 0.500000;
        vec3 lc = color_result.rgb * 0.500000;
        final_color = vec4(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
    ...
    // ── Layer 4: filaments ──
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = vec2(length(p), atan(p.y, p.x));
        p = p + vec2(sin(p.y * 3.000000 + time * 0.500000), cos(p.x * 3.000000 + time * 0.500000)) * (0.050000 + (expansion * 0.030000));
        { float warp_x = fbm2(p * 5.000000 + vec2(0.0, 1.3), int(4.000000), 0.400000, 0.150000);
        float warp_y = fbm2(p * 5.000000 + vec2(1.7, 0.0), int(4.000000), 0.400000, 0.150000);
        p = p + vec2(warp_x, warp_y) * 0.150000; }
        float sdf_result = noise2(p * 10.000000 + vec2(time * 0.1, time * 0.07));
        float glow_pulse = (0.600000 + (expansion * 0.400000)) * (0.9 + 0.1 * sin(time * 2.0));
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        { float warp_x = fbm2(p * 2.500000 + vec2(0.0, 1.3), int(3.000000), 0.100000, 2.000000);
        float warp_y = fbm2(p * 2.500000 + vec2(1.7, 0.0), int(3.000000), 0.100000, 2.000000);
        p = p + vec2(warp_x, warp_y) * 0.100000; }
        float sdf_result = voronoi2(p * 22.000000 + vec2(time * 0.05, time * 0.03));
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.000000, 0.000000, 0.010000), vec3(0.050000, 0.050000, 0.080000), vec3(0.900000, 0.900000, 1.000000), vec3(0.000000, 0.000000, 0.000000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.800000);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p + vec2(sin(p.y * 2.000000 + time * 0.400000), cos(p.x * 2.000000 + time * 0.400000)) * (0.015000 + (ignition * 0.010000));
        float sdf_result = sdf_circle(p, (0.030000 + (ignition * 0.020000)));
        float glow_pulse = (3.500000 + (ignition * 2.000000)) * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(1.000000, 0.820000, 0.400000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.900000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p + vec2(sin(p.y * 1.800000 + time * 0.300000), cos(p.x * 1.800000 + time * 0.300000)) * (0.012000 + (consciousness * 0.008000));
        float sdf_result = abs(length(p) - (0.140000 + (consciousness * 0.040000))) - 0.003000;
        float glow_pulse = (1.800000 + (consciousness * 1.200000)) * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p + vec2(sin(p.y * 1.500000 + time * 0.700000), cos(p.x * 1.500000 + time * 0.700000)) * 0.012000;
        float sdf_result = sdf_circle(p, (0.060000 + (consciousness * 0.025000)));
        for (int onion_i = 0; onion_i < int(3.000000); onion_i++) { sdf_result = abs(sdf_result) - 0.003000; }
        float glow_pulse = (1.000000 + (consciousness * 0.800000)) * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.300000, 0.220000, 0.550000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.840000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2((sin((time * 0.400000)) * ((0.130000 + (consciousness * 0.070000)))), (cos((time * 0.400000)) * ((0.130000 + (consciousness * 0.070000)))));
        float sdf_result = sdf_star(p, 5.000000, (0.015000 + (consciousness * 0.007000)), 0.006000);
        float glow_pulse = (2.000000 + (consciousness * 0.800000)) * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(1.000000, 0.650000, 0.250000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.820000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        float sdf_result = abs(length(p) - 0.460000) - 0.001000;
        float glow_pulse = (0.300000 + (consciousness * 0.150000)) * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.200000, 0.150000, 0.400000), color_result.a);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
        final_color = vec4(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
...
`;
const UNIFORMS = [{name:'ignition',default:0},{name:'expansion',default:0},{name:'consciousness',default:0}];
const COMPLEXITY = {layers:10,fbmOctaves:39,passes:3,memory:true,compute:false,is3d:false,tier:'extreme'};
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'ignition', target: 'seed', field: 'brightness', weight: 0.5 },
      { source: 'ignition', target: 'spiral', field: 'intensity', weight: 0.3 },
      { source: 'expansion', target: 'filaments', field: 'brightness', weight: 0.4 },
      { source: 'expansion', target: 'nebula', field: 'intensity', weight: 0.25 },
      { source: 'consciousness', target: 'rings', field: 'brightness', weight: 0.4 },
      { source: 'consciousness', target: 'pulse', field: 'brightness', weight: 0.3 },
      { source: 'consciousness', target: 'orbiter', field: 'brightness', weight: 0.25 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class GameArcTimeline {
  constructor() {
    ...
    this._entries = [
      { target: 'ignition', from: 0, to: 1, duration: 8, easing: 'ease_in_out' },
      { target: 'expansion', from: 0, to: 1, duration: 18, easing: 'ease_out' },
      { target: 'consciousness', from: 0, to: 1, duration: 30, easing: 'ease_in_out' },
    ];
  }
...
class Primordial extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('glyph-primordial: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get ignition() { return this._renderer?.userParams['ignition'] ?? this._pendingParams['ignition'] ?? 0; }
  set ignition(v) { this.setParam('ignition', v); }
  get expansion() { return this._renderer?.userParams['expansion'] ?? this._pendingParams['expansion'] ?? 0; }
  set expansion(v) { this.setParam('expansion', v); }
  get consciousness() { return this._renderer?.userParams['consciousness'] ?? this._pendingParams['consciousness'] ?? 0; }
  set consciousness(v) { this.setParam('consciousness', v); }
...
customElements.define('glyph-primordial', Primordial);
})();
