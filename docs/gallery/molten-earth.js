// GLYPH Component: molten-earth — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 1: magma ──
    {
        ...
        { let warp_x = fbm2(p * 1.000000 + vec2<f32>(0.0, 1.3), i32(6.000000), 0.550000, (0.350000 + (u.mouse_down * 0.100000)));
        let warp_y = fbm2(p * 1.000000 + vec2<f32>(1.7, 0.0), i32(6.000000), 0.550000, (0.350000 + (u.mouse_down * 0.100000)));
        p = p + vec2<f32>(warp_x, warp_y) * (0.350000 + (u.mouse_down * 0.100000)); }
        p = p + vec2<f32>(sin(p.y * 2.000000 + time * flow_speed), cos(p.x * 2.000000 + time * flow_speed)) * 0.100000;
        var sdf_result = fbm2((p * 2.000000 + vec2<f32>(time * 0.1, time * 0.07)), i32(5.000000), 0.500000, 2.000000);
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.450000, 0.250000, 0.120000), vec3<f32>(0.300000, 0.180000, 0.100000), vec3<f32>(0.800000, 0.600000, 0.300000), vec3<f32>(0.000000, 0.100000, 0.150000));
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let warp_x = fbm2(p * (2.000000 + (u.mouse.x * 0.400000)) + vec2<f32>(0.0, 1.3), i32(5.000000), 0.500000, (0.300000 + (heat * 0.060000)));
        let warp_y = fbm2(p * (2.000000 + (u.mouse.x * 0.400000)) + vec2<f32>(1.7, 0.0), i32(5.000000), 0.500000, (0.300000 + (heat * 0.060000)));
        p = p + vec2<f32>(warp_x, warp_y) * (0.300000 + (heat * 0.060000)); }
        p = p + vec2<f32>(sin(p.y * 2.500000 + time * (flow_speed * 1.200000)), cos(p.x * 2.500000 + time * (flow_speed * 1.200000))) * 0.080000;
        var sdf_result = voronoi2(p * 5.000000 + vec2<f32>(time * 0.05, time * 0.03));
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.400000, 0.200000, 0.080000), vec3<f32>(0.280000, 0.150000, 0.070000), vec3<f32>(1.200000, 0.700000, 0.300000), vec3<f32>(0.050000, 0.120000, 0.200000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let warp_x = fbm2(p * (3.500000 + (u.mouse.y * 0.300000)) + vec2<f32>(0.0, 1.3), i32(4.000000), 0.450000, 0.250000);
        let warp_y = fbm2(p * (3.500000 + (u.mouse.y * 0.300000)) + vec2<f32>(1.7, 0.0), i32(4.000000), 0.450000, 0.250000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.250000; }
        p = p + vec2<f32>(sin(p.y * 3.500000 + time * (flow_speed * 1.500000)), cos(p.x * 3.500000 + time * (flow_speed * 1.500000))) * 0.080000;
        var sdf_result = fbm2((p * 6.000000 + vec2<f32>(time * 0.1, time * 0.07)), i32(4.000000), 0.450000, 2.000000);
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.350000, 0.170000, 0.060000), vec3<f32>(0.220000, 0.120000, 0.050000), vec3<f32>(1.000000, 0.500000, 0.200000), vec3<f32>(0.080000, 0.150000, 0.250000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p + vec2<f32>(sin(p.y * 5.000000 + time * (flow_speed + (heat * 0.150000))), cos(p.x * 5.000000 + time * (flow_speed + (heat * 0.150000)))) * 0.200000;
        ...
        let glow_pulse = (2.500000 + (heat * 1.200000)) * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(1.000000, 0.550000, 0.120000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p + vec2<f32>(sin(p.y * 3.000000 + time * (flow_speed * 0.400000)), cos(p.x * 3.000000 + time * (flow_speed * 0.400000))) * 0.100000;
        { let warp_x = fbm2(p * 1.500000 + vec2<f32>(0.0, 1.3), i32(4.000000), 0.500000, 0.200000);
        let warp_y = fbm2(p * 1.500000 + vec2<f32>(1.7, 0.0), i32(4.000000), 0.500000, 0.200000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.200000; }
        var sdf_result = noise2(p * 5.000000 + vec2<f32>(time * 0.1, time * 0.07));
        let glow_pulse = (1.000000 + (heat * 0.400000)) * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.600000, 0.300000, 0.100000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p + vec2<f32>(sin(p.y * 7.000000 + time * ((flow_speed * 1.200000) + (heat * 0.300000))), cos(p.x * 7.000000 + time * ((flow_speed * 1.200000) + (heat * 0.300000)))) * 0.250000;
        ...
        let glow_pulse = (4.000000 + (heat * 1.500000)) * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(1.000000, 0.800000, 0.300000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        let sdf_result = smoothstep(0.000000, 0.600000, length(p));
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.380000, 0.200000, 0.100000), vec3<f32>(0.200000, 0.120000, 0.080000), vec3<f32>(0.600000, 0.400000, 0.200000), vec3<f32>(0.000000, 0.100000, 0.050000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let la = color_result.a;
        let lc = color_result.rgb;
        final_color = vec4<f32>(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
...
`;
const UNIFORMS = [{name:'heat',default:0},{name:'flow_speed',default:0.2}];
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'heat', target: 'embers', field: 'brightness', weight: 0.4 },
      { source: 'heat', target: 'flares', field: 'brightness', weight: 0.3 },
      { source: 'heat', target: 'cracks', field: 'intensity', weight: 0.2 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class MoltenEarth extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('glyph-molten-earth: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get flow_speed() { return this._renderer?.userParams['flow_speed'] ?? this._pendingParams['flow_speed'] ?? 0.2; }
...
customElements.define('glyph-molten-earth', MoltenEarth);
})();
