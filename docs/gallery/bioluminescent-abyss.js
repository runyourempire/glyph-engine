// GLYPH Component: bioluminescent-abyss — auto-generated, do not edit.
(function(){
...
void main(){
    ...
    // ── Layer 1: deep ──
    {
        ...
        { float warp_x = fbm2(p * 1.500000 + vec2(0.0, 1.3), int(5.000000), 0.500000, 0.120000);
        float warp_y = fbm2(p * 1.500000 + vec2(1.7, 0.0), int(5.000000), 0.500000, 0.120000);
        p = p + vec2(warp_x, warp_y) * 0.120000; }
        float sdf_result = fbm2((p * 2.000000 + vec2(time * 0.1, time * 0.07)), int(5.000000), 0.500000, 2.000000);
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.000000, 0.001000, 0.003000), vec3(0.002000, 0.004000, 0.008000), vec3(0.300000, 0.500000, 0.800000), vec3(0.000000, 0.150000, 0.300000));
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2((sin((time * 0.200000)) * 0.100000), (cos((time * 0.150000)) * 0.120000));
        ...
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.000000, 0.002000, 0.004000), vec3(0.005000, 0.020000, 0.040000), vec3(0.300000, 0.700000, 1.000000), vec3(0.500000, 0.150000, 0.250000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.880000);
        float la = color_result.a;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2((cos((time * 0.250000)) * 0.150000), (sin((time * 0.300000)) * 0.100000));
        { float warp_x = fbm2(p * 4.000000 + vec2(0.0, 1.3), int(3.000000), 0.080000, 2.000000);
        float warp_y = fbm2(p * 4.000000 + vec2(1.7, 0.0), int(3.000000), 0.080000, 2.000000);
        p = p + vec2(warp_x, warp_y) * 0.080000; }
        float sdf_result = voronoi2(p * 5.000000 + vec2(time * 0.05, time * 0.03));
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.001000, 0.003000, 0.005000), vec3(0.003000, 0.010000, 0.025000), vec3(0.200000, 0.600000, 0.900000), vec3(0.400000, 0.100000, 0.200000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.850000);
        float la = color_result.a;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p + vec2(sin(p.y * 2.000000 + time * 0.400000), cos(p.x * 2.000000 + time * 0.400000)) * (0.025000 + (pulse * 0.015000));
        float sdf_result = sdf_circle(p, (0.080000 + (pulse * 0.030000)));
        float shade_fw = fwidth(sdf_result);
        float shade_alpha = 1.0 - smoothstep(-shade_fw, shade_fw, sdf_result);
        vec4 color_result = vec4(vec3(0.100000, 0.600000, 0.900000) * shade_alpha, shade_alpha);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.900000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p + vec2(sin(p.y * 3.000000 + time * 0.300000), cos(p.x * 3.000000 + time * 0.300000)) * 0.030000;
        float sdf_result = abs(length(p) - (0.160000 + (pulse * 0.020000))) - 0.006000;
        float shade_fw = fwidth(sdf_result);
        float shade_alpha = 1.0 - smoothstep(-shade_fw, shade_fw, sdf_result);
        vec4 color_result = vec4(vec3(0.050000, 0.300000, 0.700000) * shade_alpha, shade_alpha);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.860000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        float sdf_result = abs(length(p) - 0.220000) - 0.002000;
        float shade_fw = fwidth(sdf_result);
        float shade_alpha = 1.0 - smoothstep(-shade_fw, shade_fw, sdf_result);
        vec4 color_result = vec4(vec3(0.080000, 0.400000, 0.800000) * shade_alpha, shade_alpha);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.780000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        { float warp_x = fbm2(p * 8.000000 + vec2(0.0, 1.3), int(3.000000), 0.150000, 2.000000);
        float warp_y = fbm2(p * 8.000000 + vec2(1.7, 0.0), int(3.000000), 0.150000, 2.000000);
        p = p + vec2(warp_x, warp_y) * 0.150000; }
        float sdf_result = voronoi2(p * 22.000000 + vec2(time * 0.05, time * 0.03));
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.000000, 0.001000, 0.002000), vec3(0.002000, 0.008000, 0.015000), vec3(0.500000, 0.800000, 1.000000), vec3(0.000000, 0.300000, 0.500000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.800000);
        float la = color_result.a;
...
`;
const UNIFORMS = [{name:'depth',default:0.5},{name:'pulse',default:0}];
const COMPLEXITY = {layers:7,fbmOctaves:19,passes:1,memory:true,compute:false,is3d:false,tier:'extreme'};
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'depth', target: 'colony', field: 'brightness', weight: 0.3 },
      { source: 'depth', target: 'cluster', field: 'intensity', weight: 0.2 },
      { source: 'pulse', target: 'creature', field: 'brightness', weight: 0.4 },
      { source: 'pulse', target: 'tentacles', field: 'brightness', weight: 0.3 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class GameArcTimeline {
  constructor() {
    ...
    this._entries = [
      { target: 'pulse', from: 0, to: 1, duration: 5, easing: 'ease_in_out' },
      { target: 'depth', from: 0.5, to: 0.9, duration: 12, easing: 'ease_in_out' },
    ];
  }
...
class BioluminescentAbyss extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('glyph-bioluminescent-abyss: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
...
customElements.define('glyph-bioluminescent-abyss', BioluminescentAbyss);
})();
