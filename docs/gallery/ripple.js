// GLYPH Component: ripple — auto-generated, do not edit.
(function(){
...
`;
const UNIFORMS = [{name:'expand',default:0}];
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'expand', target: 'waves', field: 'brightness', weight: 0.2 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class GameArcTimeline {
  constructor() {
    ...
    this._entries = [
      { target: 'expand', from: 0, to: 1, duration: 8, easing: 'ease_in_out' },
    ];
  }
...
class Ripple extends HTMLElement {
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('glyph-ripple: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get expand() { return this._renderer?.userParams['expand'] ?? this._pendingParams['expand'] ?? 0; }
  set expand(v) { this.setParam('expand', v); }
...
customElements.define('glyph-ripple', Ripple);
})();
