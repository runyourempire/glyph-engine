// GLYPH Component: volcanic-flow — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 1: ground ──
    {
        ...
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.500000, 0.300000, 0.150000), vec3<f32>(0.300000, 0.200000, 0.100000), vec3<f32>(0.800000, 0.600000, 0.300000), vec3<f32>(0.000000, 0.100000, 0.150000));
    ...
    // ── Layer 2: embers ──
    {
        ...
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(1.000000, 0.500000, 0.150000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    // ── Layer 3: smoke ──
    {
        ...
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.800000, 0.350000, 0.100000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    // ── Layer 4: ash ──
    {
        ...
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(1.000000, 0.700000, 0.200000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    // ── Layer 5: heat ──
    {
        ...
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.500000, 0.300000, 0.100000), vec3<f32>(0.400000, 0.250000, 0.150000), vec3<f32>(0.800000, 0.500000, 0.200000), vec3<f32>(0.000000, 0.100000, 0.000000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let la = color_result.a;
        let lc = color_result.rgb;
        final_color = vec4<f32>(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    ...
    // ── Layer 6: sparks ──
    {
        ...
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(1.000000, 0.900000, 0.400000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
...
void main(){
    ...
    // ── Layer 1: ground ──
    {
        ...
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.500000, 0.300000, 0.150000), vec3(0.300000, 0.200000, 0.100000), vec3(0.800000, 0.600000, 0.300000), vec3(0.000000, 0.100000, 0.150000));
    ...
    // ── Layer 2: embers ──
    {
        ...
        color_result = vec4(color_result.rgb * vec3(1.000000, 0.500000, 0.150000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.870000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    // ── Layer 3: smoke ──
    {
        ...
        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.800000, 0.350000, 0.100000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.960000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    // ── Layer 4: ash ──
    {
        ...
        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(1.000000, 0.700000, 0.200000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.800000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    // ── Layer 5: heat ──
    {
        ...
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.500000, 0.300000, 0.100000), vec3(0.400000, 0.250000, 0.150000), vec3(0.800000, 0.500000, 0.200000), vec3(0.000000, 0.100000, 0.000000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        float la = color_result.a;
        vec3 lc = color_result.rgb;
        final_color = vec4(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    ...
    // ── Layer 6: sparks ──
    {
        ...
        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(1.000000, 0.900000, 0.400000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.720000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'life', target: 'embers', field: 'brightness', weight: 0.6 },
      { source: 'life', target: 'smoke', field: 'intensity', weight: 0.4 },
      { source: 'life', target: 'sparks', field: 'brightness', weight: 0.5 },
      { source: 'warmth', target: 'ground', field: 'brightness', weight: 0.3 },
      { source: 'warmth', target: 'heat', field: 'brightness', weight: 0.5 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class VolcanicFlow extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('glyph-volcanic-flow: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
...
customElements.define('glyph-volcanic-flow', VolcanicFlow);
})();
