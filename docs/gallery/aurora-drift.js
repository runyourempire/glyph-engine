// GLYPH Component: aurora-drift — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let solar_wind = u.p_solar_wind;
    let curtain_wave = u.p_curtain_wave;
    let altitude = u.p_altitude;
    let color_temp = u.p_color_temp;
    ...
    // ── Layer 1: sky ──
    {
        ...
        { let warp_x = fbm2(p * 1.000000 + vec2<f32>(0.0, 1.3), i32(3.000000), 0.400000, 0.060000);
        let warp_y = fbm2(p * 1.000000 + vec2<f32>(1.7, 0.0), i32(3.000000), 0.400000, 0.060000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.060000; }
        var sdf_result = voronoi2(p * 30.000000 + vec2<f32>(time * 0.05, time * 0.03));
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.003000, 0.005000, 0.015000), vec3<f32>(0.008000, 0.008000, 0.020000), vec3<f32>(0.600000, 0.600000, 0.800000), vec3<f32>(0.000000, 0.000000, 0.000000));
    ...
    // ── Layer 2: curtain_green ──
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>(0.000000, ((0.120000 + (u.mouse.y * 0.060000)) - 0.030000));
        { let warp_x = fbm2(p * (1.500000 + (u.mouse.x * 0.300000)) + vec2<f32>(0.0, 1.3), i32(6.000000), 0.600000, ((0.300000 + (solar_wind * 0.120000)) + (u.mouse_down * 0.060000)));
        let warp_y = fbm2(p * (1.500000 + (u.mouse.x * 0.300000)) + vec2<f32>(1.7, 0.0), i32(6.000000), 0.600000, ((0.300000 + (solar_wind * 0.120000)) + (u.mouse_down * 0.060000)));
        p = p + vec2<f32>(warp_x, warp_y) * ((0.300000 + (solar_wind * 0.120000)) + (u.mouse_down * 0.060000)); }
        p = p + vec2<f32>(sin(p.y * 2.000000 + time * (0.250000 + (curtain_wave * 0.150000))), cos(p.x * 2.000000 + time * (0.250000 + (curtain_wave * 0.150000)))) * (0.100000 + (curtain_wave * 0.040000));
        var sdf_result = fbm2((p * (altitude * 0.600000) + vec2<f32>(time * 0.1, time * 0.07)), i32(6.000000), 0.550000, 2.000000);
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>((0.005000 + (color_temp * 0.010000)), (0.030000 - (color_temp * 0.010000)), 0.015000), vec3<f32>((0.020000 + (color_temp * 0.040000)), (0.120000 - (color_temp * 0.040000)), (0.060000 + (color_temp * 0.030000))), vec3<f32>(0.500000, (1.000000 - (color_temp * 0.300000)), (0.500000 + (color_temp * 0.300000))), vec3<f32>((0.100000 + (color_temp * 0.100000)), (0.300000 - (color_temp * 0.100000)), (0.200000 + (color_temp * 0.100000))));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
    ...
    // ── Layer 3: curtain_blue ──
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>(0.080000, (((-0.080000) + (u.mouse.y * 0.040000)) - 0.020000));
        { let warp_x = fbm2(p * (2.000000 + (u.mouse.x * 0.200000)) + vec2<f32>(0.0, 1.3), i32(5.000000), 0.550000, (0.250000 + (solar_wind * 0.100000)));
        let warp_y = fbm2(p * (2.000000 + (u.mouse.x * 0.200000)) + vec2<f32>(1.7, 0.0), i32(5.000000), 0.550000, (0.250000 + (solar_wind * 0.100000)));
        p = p + vec2<f32>(warp_x, warp_y) * (0.250000 + (solar_wind * 0.100000)); }
        p = p + vec2<f32>(sin(p.y * 2.500000 + time * (0.300000 + (curtain_wave * 0.120000))), cos(p.x * 2.500000 + time * (0.300000 + (curtain_wave * 0.120000)))) * (0.080000 + (curtain_wave * 0.030000));
        var sdf_result = fbm2((p * (altitude * 0.800000) + vec2<f32>(time * 0.1, time * 0.07)), i32(5.000000), 0.500000, 2.000000);
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.005000, 0.010000, 0.025000), vec3<f32>(0.020000, 0.040000, 0.100000), vec3<f32>(0.500000, 0.800000, 1.000000), vec3<f32>(0.150000, 0.250000, 0.550000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>(0.000000, 0.150000);
        { let warp_x = fbm2(p * 2.500000 + vec2<f32>(0.0, 1.3), i32(4.000000), 0.500000, (0.200000 + (solar_wind * 0.080000)));
        let warp_y = fbm2(p * 2.500000 + vec2<f32>(1.7, 0.0), i32(4.000000), 0.500000, (0.200000 + (solar_wind * 0.080000)));
        p = p + vec2<f32>(warp_x, warp_y) * (0.200000 + (solar_wind * 0.080000)); }
        p = p + vec2<f32>(sin(p.y * 3.000000 + time * (0.350000 + (curtain_wave * 0.150000))), cos(p.x * 3.000000 + time * (0.350000 + (curtain_wave * 0.150000)))) * 0.060000;
        var sdf_result = noise2(p * (altitude * 1.200000) + vec2<f32>(time * 0.1, time * 0.07));
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.010000, (0.040000 - (color_temp * 0.020000)), (0.010000 + (color_temp * 0.020000))), vec3<f32>(0.030000, (0.100000 - (color_temp * 0.030000)), (0.040000 + (color_temp * 0.040000))), vec3<f32>(0.600000, 1.000000, 0.500000), vec3<f32>(0.000000, 0.300000, 0.000000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
        color_result = mix(color_result, prev_color, 0.800000);
        let la = color_result.a * 0.450000;
        let lc = color_result.rgb * 0.450000;
        final_color = vec4<f32>(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
    ...
    // ── Layer 5: corona ──
    {
        ...
        { let warp_x = fbm2(p * 2.000000 + vec2<f32>(0.0, 1.3), i32(4.000000), 0.400000, (0.150000 + (solar_wind * 0.060000)));
        let warp_y = fbm2(p * 2.000000 + vec2<f32>(1.7, 0.0), i32(4.000000), 0.400000, (0.150000 + (solar_wind * 0.060000)));
        p = p + vec2<f32>(warp_x, warp_y) * (0.150000 + (solar_wind * 0.060000)); }
        var sdf_result = noise2(p * (altitude * 1.500000) + vec2<f32>(time * 0.1, time * 0.07));
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.005000, 0.015000, 0.005000), vec3<f32>(0.020000, 0.060000, 0.030000), vec3<f32>(0.400000, 0.800000, 0.600000), vec3<f32>(0.100000, 0.200000, 0.300000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>(0.000000, (-0.300000));
        let sdf_result = smoothstep(0.000000, 0.350000, length(p));
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.010000, 0.015000, 0.020000), vec3<f32>(0.020000, 0.025000, 0.030000), vec3<f32>(0.300000, 0.350000, 0.400000), vec3<f32>(0.000000, 0.100000, 0.150000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    // ── Layer 7: cursor ──
    {
        ...
        var sdf_result = sdf_circle(p, (0.025000 + (u.mouse_down * 0.030000)));
        let glow_pulse = (0.300000 + (u.mouse_down * 0.400000)) * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.200000, 0.500000, 0.300000), color_result.a);
        let la = color_result.a;
        let lc = color_result.rgb;
        final_color = vec4<f32>(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
...
uniform float u_p_solar_wind;
uniform float u_p_curtain_wave;
uniform float u_p_altitude;
uniform float u_p_color_temp;
...
void main(){
    ...
    float solar_wind = u_p_solar_wind;
    float curtain_wave = u_p_curtain_wave;
    float altitude = u_p_altitude;
    float color_temp = u_p_color_temp;
    ...
    // ── Layer 1: sky ──
    {
        ...
        { float warp_x = fbm2(p * 1.000000 + vec2(0.0, 1.3), int(3.000000), 0.400000, 0.060000);
        float warp_y = fbm2(p * 1.000000 + vec2(1.7, 0.0), int(3.000000), 0.400000, 0.060000);
        p = p + vec2(warp_x, warp_y) * 0.060000; }
        float sdf_result = voronoi2(p * 30.000000 + vec2(time * 0.05, time * 0.03));
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.003000, 0.005000, 0.015000), vec3(0.008000, 0.008000, 0.020000), vec3(0.600000, 0.600000, 0.800000), vec3(0.000000, 0.000000, 0.000000));
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2(0.000000, ((0.120000 + (u_mouse.y * 0.060000)) - 0.030000));
        { float warp_x = fbm2(p * (1.500000 + (u_mouse.x * 0.300000)) + vec2(0.0, 1.3), int(6.000000), 0.600000, ((0.300000 + (solar_wind * 0.120000)) + (u_mouse_down * 0.060000)));
        float warp_y = fbm2(p * (1.500000 + (u_mouse.x * 0.300000)) + vec2(1.7, 0.0), int(6.000000), 0.600000, ((0.300000 + (solar_wind * 0.120000)) + (u_mouse_down * 0.060000)));
        p = p + vec2(warp_x, warp_y) * ((0.300000 + (solar_wind * 0.120000)) + (u_mouse_down * 0.060000)); }
        p = p + vec2(sin(p.y * 2.000000 + time * (0.250000 + (curtain_wave * 0.150000))), cos(p.x * 2.000000 + time * (0.250000 + (curtain_wave * 0.150000)))) * (0.100000 + (curtain_wave * 0.040000));
        float sdf_result = fbm2((p * (altitude * 0.600000) + vec2(time * 0.1, time * 0.07)), int(6.000000), 0.550000, 2.000000);
        vec3 pal_rgb = cosine_palette(sdf_result, vec3((0.005000 + (color_temp * 0.010000)), (0.030000 - (color_temp * 0.010000)), 0.015000), vec3((0.020000 + (color_temp * 0.040000)), (0.120000 - (color_temp * 0.040000)), (0.060000 + (color_temp * 0.030000))), vec3(0.500000, (1.000000 - (color_temp * 0.300000)), (0.500000 + (color_temp * 0.300000))), vec3((0.100000 + (color_temp * 0.100000)), (0.300000 - (color_temp * 0.100000)), (0.200000 + (color_temp * 0.100000))));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2(0.080000, (((-0.080000) + (u_mouse.y * 0.040000)) - 0.020000));
        { float warp_x = fbm2(p * (2.000000 + (u_mouse.x * 0.200000)) + vec2(0.0, 1.3), int(5.000000), 0.550000, (0.250000 + (solar_wind * 0.100000)));
        float warp_y = fbm2(p * (2.000000 + (u_mouse.x * 0.200000)) + vec2(1.7, 0.0), int(5.000000), 0.550000, (0.250000 + (solar_wind * 0.100000)));
        p = p + vec2(warp_x, warp_y) * (0.250000 + (solar_wind * 0.100000)); }
        p = p + vec2(sin(p.y * 2.500000 + time * (0.300000 + (curtain_wave * 0.120000))), cos(p.x * 2.500000 + time * (0.300000 + (curtain_wave * 0.120000)))) * (0.080000 + (curtain_wave * 0.030000));
        float sdf_result = fbm2((p * (altitude * 0.800000) + vec2(time * 0.1, time * 0.07)), int(5.000000), 0.500000, 2.000000);
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.005000, 0.010000, 0.025000), vec3(0.020000, 0.040000, 0.100000), vec3(0.500000, 0.800000, 1.000000), vec3(0.150000, 0.250000, 0.550000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.840000);
        float la = color_result.a * 0.700000;
        vec3 lc = color_result.rgb * 0.700000;
        final_color = vec4(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2(0.000000, 0.150000);
        { float warp_x = fbm2(p * 2.500000 + vec2(0.0, 1.3), int(4.000000), 0.500000, (0.200000 + (solar_wind * 0.080000)));
        float warp_y = fbm2(p * 2.500000 + vec2(1.7, 0.0), int(4.000000), 0.500000, (0.200000 + (solar_wind * 0.080000)));
        p = p + vec2(warp_x, warp_y) * (0.200000 + (solar_wind * 0.080000)); }
        p = p + vec2(sin(p.y * 3.000000 + time * (0.350000 + (curtain_wave * 0.150000))), cos(p.x * 3.000000 + time * (0.350000 + (curtain_wave * 0.150000)))) * 0.060000;
        float sdf_result = noise2(p * (altitude * 1.200000) + vec2(time * 0.1, time * 0.07));
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.010000, (0.040000 - (color_temp * 0.020000)), (0.010000 + (color_temp * 0.020000))), vec3(0.030000, (0.100000 - (color_temp * 0.030000)), (0.040000 + (color_temp * 0.040000))), vec3(0.600000, 1.000000, 0.500000), vec3(0.000000, 0.300000, 0.000000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.800000);
        float la = color_result.a * 0.450000;
        vec3 lc = color_result.rgb * 0.450000;
        final_color = vec4(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
    ...
    // ── Layer 5: corona ──
    {
        ...
        { float warp_x = fbm2(p * 2.000000 + vec2(0.0, 1.3), int(4.000000), 0.400000, (0.150000 + (solar_wind * 0.060000)));
        float warp_y = fbm2(p * 2.000000 + vec2(1.7, 0.0), int(4.000000), 0.400000, (0.150000 + (solar_wind * 0.060000)));
        p = p + vec2(warp_x, warp_y) * (0.150000 + (solar_wind * 0.060000)); }
        float sdf_result = noise2(p * (altitude * 1.500000) + vec2(time * 0.1, time * 0.07));
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.005000, 0.015000, 0.005000), vec3(0.020000, 0.060000, 0.030000), vec3(0.400000, 0.800000, 0.600000), vec3(0.100000, 0.200000, 0.300000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.760000);
        float la = color_result.a * 0.300000;
        vec3 lc = color_result.rgb * 0.300000;
        final_color = vec4(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2(0.000000, (-0.300000));
        float sdf_result = smoothstep(0.000000, 0.350000, length(p));
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.010000, 0.015000, 0.020000), vec3(0.020000, 0.025000, 0.030000), vec3(0.300000, 0.350000, 0.400000), vec3(0.000000, 0.100000, 0.150000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.900000);
        float la = color_result.a * 0.400000;
        vec3 lc = color_result.rgb * 0.400000;
        final_color = vec4(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2(((u_mouse.x * 2.000000) - 1.000000), ((u_mouse.y * 2.000000) - 1.000000));
        float sdf_result = sdf_circle(p, (0.025000 + (u_mouse_down * 0.030000)));
        float glow_pulse = (0.300000 + (u_mouse_down * 0.400000)) * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.200000, 0.500000, 0.300000), color_result.a);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
        final_color = vec4(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
...
`;
const UNIFORMS = [{name:'solar_wind',default:0},{name:'curtain_wave',default:0},{name:'altitude',default:5},{name:'color_temp',default:0.5}];
const COMPLEXITY = {layers:7,fbmOctaves:33,passes:2,memory:true,compute:false,is3d:false,tier:'extreme'};
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let vign = 1.0 - 0.250000 * length(uv - 0.5);
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'solar_wind', target: 'curtain_green', field: 'intensity', weight: 0.3 },
      { source: 'solar_wind', target: 'curtain_blue', field: 'brightness', weight: 0.25 },
      { source: 'curtain_wave', target: 'peaks', field: 'brightness', weight: 0.3 },
      { source: 'curtain_wave', target: 'corona', field: 'intensity', weight: 0.2 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class GameArcTimeline {
  constructor() {
    ...
    this._entries = [
      { target: 'solar_wind', from: 0, to: 1, duration: 10, easing: 'ease_in_out' },
      { target: 'curtain_wave', from: 0, to: 0.8, duration: 16, easing: 'ease_out' },
    ];
  }
...
class AuroraDrift extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    const gpu = new GlyphRenderer(this._canvas, WGSL_V, WGSL_F, UNIFORMS, PASS_SHADERS);
    if (await gpu.init()) {
      this._renderer = gpu;
    } else {
      ...
      if (gl.init()) {
        ...
        console.warn('glyph-aurora-drift: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get solar_wind() { return this._renderer?.userParams['solar_wind'] ?? this._pendingParams['solar_wind'] ?? 0; }
  set solar_wind(v) { this.setParam('solar_wind', v); }
  get curtain_wave() { return this._renderer?.userParams['curtain_wave'] ?? this._pendingParams['curtain_wave'] ?? 0; }
  set curtain_wave(v) { this.setParam('curtain_wave', v); }
  get altitude() { return this._renderer?.userParams['altitude'] ?? this._pendingParams['altitude'] ?? 5; }
  set altitude(v) { this.setParam('altitude', v); }
  get color_temp() { return this._renderer?.userParams['color_temp'] ?? this._pendingParams['color_temp'] ?? 0.5; }
  set color_temp(v) { this.setParam('color_temp', v); }
...
customElements.define('glyph-aurora-drift', AuroraDrift);
})();
