// GLYPH Component: amethyst-flow — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 1: crystal ──
    {
        ...
        { let warp_x = fbm2(p * 1.000000 + vec2<f32>(0.0, 1.3), i32(5.000000), 0.500000, (0.300000 + (u.mouse_down * 0.080000)));
        let warp_y = fbm2(p * 1.000000 + vec2<f32>(1.7, 0.0), i32(5.000000), 0.500000, (0.300000 + (u.mouse_down * 0.080000)));
        p = p + vec2<f32>(warp_x, warp_y) * (0.300000 + (u.mouse_down * 0.080000)); }
        var sdf_result = fbm2((p * 2.000000 + vec2<f32>(time * 0.1, time * 0.07)), i32(4.000000), 0.500000, 2.000000);
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.400000, 0.300000, 0.500000), vec3<f32>(0.300000, 0.200000, 0.300000), vec3<f32>(0.800000, 0.600000, 0.300000), vec3<f32>(0.150000, 0.100000, 0.200000));
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p + vec2<f32>(sin(p.y * 4.000000 + time * 0.400000), cos(p.x * 4.000000 + time * 0.400000)) * 0.200000;
        p = vec2<f32>(glyph_mod(p.x + 0.150000 * 0.5, 0.150000) - 0.150000 * 0.5, glyph_mod(p.y + 0.150000 * 0.5, 0.150000) - 0.150000 * 0.5);
        var sdf_result = sdf_circle(p, 0.030000);
        let glow_pulse = (3.000000 + (life * 0.500000)) * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.700000, 0.400000, 1.000000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p + vec2<f32>(sin(p.y * 3.000000 + time * 0.300000), cos(p.x * 3.000000 + time * 0.300000)) * 0.150000;
        { let warp_x = fbm2(p * 2.000000 + vec2<f32>(0.0, 1.3), i32(4.000000), 0.250000, 2.000000);
        let warp_y = fbm2(p * 2.000000 + vec2<f32>(1.7, 0.0), i32(4.000000), 0.250000, 2.000000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.250000; }
        var sdf_result = noise2(p * 6.000000 + vec2<f32>(time * 0.1, time * 0.07));
        let glow_pulse = (1.500000 + (life * 0.300000)) * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.600000, 0.300000, 0.800000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let warp_x = fbm2(p * 5.000000 + vec2<f32>(0.0, 1.3), i32(3.000000), 0.300000, 2.000000);
        let warp_y = fbm2(p * 5.000000 + vec2<f32>(1.7, 0.0), i32(3.000000), 0.300000, 2.000000);
        ...
        let glow_pulse = (2.500000 + (life * 0.500000)) * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        let sdf_result = smoothstep(0.000000, 0.700000, length(p));
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.350000, 0.250000, 0.450000), vec3<f32>(0.250000, 0.200000, 0.300000), vec3<f32>(0.800000, 0.500000, 0.300000), vec3<f32>(0.100000, 0.050000, 0.150000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let la = color_result.a;
        let lc = color_result.rgb;
        final_color = vec4<f32>(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p + vec2<f32>(sin(p.y * 7.000000 + time * 0.800000), cos(p.x * 7.000000 + time * 0.800000)) * 0.250000;
        { let warp_x = fbm2(p * 3.000000 + vec2<f32>(0.0, 1.3), i32(3.000000), 0.350000, 2.000000);
        let warp_y = fbm2(p * 3.000000 + vec2<f32>(1.7, 0.0), i32(3.000000), 0.350000, 2.000000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.350000; }
        var sdf_result = fbm2((p * 18.000000 + vec2<f32>(time * 0.1, time * 0.07)), i32(2.000000), 0.500000, 2.000000);
        let glow_pulse = (4.000000 + (life * 1.000000)) * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.900000, 0.700000, 1.000000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
...
uniform float u_p_life;
uniform float u_p_warmth;
...
void main(){
    ...
    // ── Layer 1: crystal ──
    {
        ...
        { float warp_x = fbm2(p * 1.000000 + vec2(0.0, 1.3), int(5.000000), 0.500000, (0.300000 + (u_mouse_down * 0.080000)));
        float warp_y = fbm2(p * 1.000000 + vec2(1.7, 0.0), int(5.000000), 0.500000, (0.300000 + (u_mouse_down * 0.080000)));
        p = p + vec2(warp_x, warp_y) * (0.300000 + (u_mouse_down * 0.080000)); }
        float sdf_result = fbm2((p * 2.000000 + vec2(time * 0.1, time * 0.07)), int(4.000000), 0.500000, 2.000000);
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.400000, 0.300000, 0.500000), vec3(0.300000, 0.200000, 0.300000), vec3(0.800000, 0.600000, 0.300000), vec3(0.150000, 0.100000, 0.200000));
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p + vec2(sin(p.y * 4.000000 + time * 0.400000), cos(p.x * 4.000000 + time * 0.400000)) * 0.200000;
        p = vec2(mod(p.x + 0.150000 * 0.5, 0.150000) - 0.150000 * 0.5, mod(p.y + 0.150000 * 0.5, 0.150000) - 0.150000 * 0.5);
        float sdf_result = sdf_circle(p, 0.030000);
        float glow_pulse = (3.000000 + (life * 0.500000)) * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.700000, 0.400000, 1.000000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.870000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p + vec2(sin(p.y * 3.000000 + time * 0.300000), cos(p.x * 3.000000 + time * 0.300000)) * 0.150000;
        { float warp_x = fbm2(p * 2.000000 + vec2(0.0, 1.3), int(4.000000), 0.250000, 2.000000);
        float warp_y = fbm2(p * 2.000000 + vec2(1.7, 0.0), int(4.000000), 0.250000, 2.000000);
        p = p + vec2(warp_x, warp_y) * 0.250000; }
        float sdf_result = noise2(p * 6.000000 + vec2(time * 0.1, time * 0.07));
        float glow_pulse = (1.500000 + (life * 0.300000)) * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.600000, 0.300000, 0.800000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.960000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        { float warp_x = fbm2(p * 5.000000 + vec2(0.0, 1.3), int(3.000000), 0.300000, 2.000000);
        float warp_y = fbm2(p * 5.000000 + vec2(1.7, 0.0), int(3.000000), 0.300000, 2.000000);
        p = p + vec2(warp_x, warp_y) * 0.300000; }
        float sdf_result = fbm2((p * 12.000000 + vec2(time * 0.1, time * 0.07)), int(3.000000), 0.500000, 2.000000);
        float glow_pulse = (2.500000 + (life * 0.500000)) * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        float sdf_result = smoothstep(0.000000, 0.700000, length(p));
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.350000, 0.250000, 0.450000), vec3(0.250000, 0.200000, 0.300000), vec3(0.800000, 0.500000, 0.300000), vec3(0.100000, 0.050000, 0.150000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        float la = color_result.a;
        vec3 lc = color_result.rgb;
        final_color = vec4(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p + vec2(sin(p.y * 7.000000 + time * 0.800000), cos(p.x * 7.000000 + time * 0.800000)) * 0.250000;
        ...
        float glow_pulse = (4.000000 + (life * 1.000000)) * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.900000, 0.700000, 1.000000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.720000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
...
`;
const UNIFORMS = [{name:'life',default:0},{name:'warmth',default:0}];
...
`;
const PASS_WGSL_1 = `// Post-processing pass: atmosphere
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'life', target: 'nodes', field: 'brightness', weight: 0.6 },
      { source: 'life', target: 'energy', field: 'intensity', weight: 0.4 },
      { source: 'life', target: 'flashes', field: 'brightness', weight: 0.5 },
      { source: 'warmth', target: 'crystal', field: 'brightness', weight: 0.3 },
      { source: 'warmth', target: 'aura', field: 'brightness', weight: 0.5 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class GameArcTimeline {
  constructor() {
    ...
    this._entries = [
      { target: 'life', from: 0, to: 1, duration: 8, easing: 'ease_in_out' },
      { target: 'warmth', from: 0, to: 0.8, duration: 6, easing: 'ease_out' },
    ];
  }
...
class AmethystFlow extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('glyph-amethyst-flow: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get life() { return this._renderer?.userParams['life'] ?? this._pendingParams['life'] ?? 0; }
  set life(v) { this.setParam('life', v); }
  get warmth() { return this._renderer?.userParams['warmth'] ?? this._pendingParams['warmth'] ?? 0; }
  set warmth(v) { this.setParam('warmth', v); }
...
customElements.define('glyph-amethyst-flow', AmethystFlow);
})();
