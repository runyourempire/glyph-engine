// GLYPH Component: ember-flow — auto-generated, do not edit.
(function(){
...
fn apply_color_matrix(color: vec3f) -> vec3f {
    let m = mat3x3f(
        vec3f(1.1, 0, 0.01),
        vec3f(-0.02, 0.85, -0.01),
        vec3f(0.03, 0.02, 0.75)
    );
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 1: magma ──
    {
        ...
        { let warp_x = fbm2(p * 1.000000 + vec2<f32>(0.0, 1.3), i32(6.000000), 0.550000, (0.350000 + (u.mouse_down * 0.080000)));
        let warp_y = fbm2(p * 1.000000 + vec2<f32>(1.7, 0.0), i32(6.000000), 0.550000, (0.350000 + (u.mouse_down * 0.080000)));
        p = p + vec2<f32>(warp_x, warp_y) * (0.350000 + (u.mouse_down * 0.080000)); }
        p = p + vec2<f32>(sin(p.y * 2.000000 + time * (0.200000 + (heat * 0.100000))), cos(p.x * 2.000000 + time * (0.200000 + (heat * 0.100000)))) * (viscosity * 0.500000);
        var sdf_result = fbm2((p * 2.000000 + vec2<f32>(time * 0.1, time * 0.07)), i32(5.000000), 0.500000, 2.000000);
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.400000, 0.200000, 0.100000), vec3<f32>(0.300000, 0.150000, 0.100000), vec3<f32>(0.800000, 0.600000, 0.300000), vec3<f32>(0.000000, 0.100000, 0.150000));
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let warp_x = fbm2(p * (2.000000 + (u.mouse.x * 0.400000)) + vec2<f32>(0.0, 1.3), i32(5.000000), 0.500000, (viscosity + (heat * 0.080000)));
        let warp_y = fbm2(p * (2.000000 + (u.mouse.x * 0.400000)) + vec2<f32>(1.7, 0.0), i32(5.000000), 0.500000, (viscosity + (heat * 0.080000)));
        p = p + vec2<f32>(warp_x, warp_y) * (viscosity + (heat * 0.080000)); }
        p = p + vec2<f32>(sin(p.y * 2.500000 + time * (0.300000 + (heat * 0.150000))), cos(p.x * 2.500000 + time * (0.300000 + (heat * 0.150000)))) * (viscosity * 0.700000);
        var sdf_result = voronoi2(p * 6.000000 + vec2<f32>(time * 0.05, time * 0.03));
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.350000, 0.150000, 0.050000), vec3<f32>(0.250000, 0.120000, 0.050000), vec3<f32>(1.500000, 0.700000, 0.300000), vec3<f32>(0.050000, 0.120000, 0.200000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let warp_x = fbm2(p * (3.000000 + (u.mouse.y * 0.300000)) + vec2<f32>(0.0, 1.3), i32(4.000000), 0.450000, (viscosity * 0.900000));
        let warp_y = fbm2(p * (3.000000 + (u.mouse.y * 0.300000)) + vec2<f32>(1.7, 0.0), i32(4.000000), 0.450000, (viscosity * 0.900000));
        p = p + vec2<f32>(warp_x, warp_y) * (viscosity * 0.900000); }
        p = p + vec2<f32>(sin(p.y * 3.500000 + time * (0.350000 + (heat * 0.200000))), cos(p.x * 3.500000 + time * (0.350000 + (heat * 0.200000)))) * (viscosity * 0.600000);
        var sdf_result = fbm2((p * 6.000000 + vec2<f32>(time * 0.1, time * 0.07)), i32(4.000000), 0.450000, 2.000000);
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.300000, 0.120000, 0.030000), vec3<f32>(0.200000, 0.100000, 0.030000), vec3<f32>(1.200000, 0.500000, 0.200000), vec3<f32>(0.080000, 0.150000, 0.220000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p + vec2<f32>(sin(p.y * 5.000000 + time * (0.400000 + (heat * 0.200000))), cos(p.x * 5.000000 + time * (0.400000 + (heat * 0.200000)))) * 0.200000;
        { let warp_x = fbm2(p * 2.500000 + vec2<f32>(0.0, 1.3), i32(3.000000), 0.250000, 2.000000);
        let warp_y = fbm2(p * 2.500000 + vec2<f32>(1.7, 0.0), i32(3.000000), 0.250000, 2.000000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.250000; }
        p = vec2<f32>(glyph_mod(p.x + 0.100000 * 0.5, 0.100000) - 0.100000 * 0.5, glyph_mod(p.y + 0.100000 * 0.5, 0.100000) - 0.100000 * 0.5);
        var sdf_result = sdf_circle(p, (0.012000 + (heat * 0.004000)));
        let glow_pulse = (2.000000 + (heat * 1.000000)) * (0.9 + 0.1 * sin(time * 2.0));
    ...
    // ── Layer 5: cursor ──
    {
        ...
        var sdf_result = sdf_circle(p, (0.030000 + (u.mouse_down * 0.040000)));
        let glow_pulse = (0.400000 + (u.mouse_down * 0.600000)) * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(1.000000, 0.600000, 0.150000), color_result.a);
        let la = color_result.a;
        let lc = color_result.rgb;
        final_color = vec4<f32>(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
...
void main(){
    ...
    // ── Layer 1: magma ──
    {
        ...
        { float warp_x = fbm2(p * 1.000000 + vec2(0.0, 1.3), int(6.000000), 0.550000, (0.350000 + (u_mouse_down * 0.080000)));
        float warp_y = fbm2(p * 1.000000 + vec2(1.7, 0.0), int(6.000000), 0.550000, (0.350000 + (u_mouse_down * 0.080000)));
        p = p + vec2(warp_x, warp_y) * (0.350000 + (u_mouse_down * 0.080000)); }
        p = p + vec2(sin(p.y * 2.000000 + time * (0.200000 + (heat * 0.100000))), cos(p.x * 2.000000 + time * (0.200000 + (heat * 0.100000)))) * (viscosity * 0.500000);
        float sdf_result = fbm2((p * 2.000000 + vec2(time * 0.1, time * 0.07)), int(5.000000), 0.500000, 2.000000);
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.400000, 0.200000, 0.100000), vec3(0.300000, 0.150000, 0.100000), vec3(0.800000, 0.600000, 0.300000), vec3(0.000000, 0.100000, 0.150000));
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        { float warp_x = fbm2(p * (2.000000 + (u_mouse.x * 0.400000)) + vec2(0.0, 1.3), int(5.000000), 0.500000, (viscosity + (heat * 0.080000)));
        float warp_y = fbm2(p * (2.000000 + (u_mouse.x * 0.400000)) + vec2(1.7, 0.0), int(5.000000), 0.500000, (viscosity + (heat * 0.080000)));
        p = p + vec2(warp_x, warp_y) * (viscosity + (heat * 0.080000)); }
        p = p + vec2(sin(p.y * 2.500000 + time * (0.300000 + (heat * 0.150000))), cos(p.x * 2.500000 + time * (0.300000 + (heat * 0.150000)))) * (viscosity * 0.700000);
        float sdf_result = voronoi2(p * 6.000000 + vec2(time * 0.05, time * 0.03));
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.350000, 0.150000, 0.050000), vec3(0.250000, 0.120000, 0.050000), vec3(1.500000, 0.700000, 0.300000), vec3(0.050000, 0.120000, 0.200000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.860000);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        { float warp_x = fbm2(p * (3.000000 + (u_mouse.y * 0.300000)) + vec2(0.0, 1.3), int(4.000000), 0.450000, (viscosity * 0.900000));
        float warp_y = fbm2(p * (3.000000 + (u_mouse.y * 0.300000)) + vec2(1.7, 0.0), int(4.000000), 0.450000, (viscosity * 0.900000));
        p = p + vec2(warp_x, warp_y) * (viscosity * 0.900000); }
        p = p + vec2(sin(p.y * 3.500000 + time * (0.350000 + (heat * 0.200000))), cos(p.x * 3.500000 + time * (0.350000 + (heat * 0.200000)))) * (viscosity * 0.600000);
        float sdf_result = fbm2((p * 6.000000 + vec2(time * 0.1, time * 0.07)), int(4.000000), 0.450000, 2.000000);
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.300000, 0.120000, 0.030000), vec3(0.200000, 0.100000, 0.030000), vec3(1.200000, 0.500000, 0.200000), vec3(0.080000, 0.150000, 0.220000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.820000);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p + vec2(sin(p.y * 5.000000 + time * (0.400000 + (heat * 0.200000))), cos(p.x * 5.000000 + time * (0.400000 + (heat * 0.200000)))) * 0.200000;
        ...
        float glow_pulse = (2.000000 + (heat * 1.000000)) * (0.9 + 0.1 * sin(time * 2.0));
    ...
    // ── Layer 5: cursor ──
    {
        ...
        float sdf_result = sdf_circle(p, (0.030000 + (u_mouse_down * 0.040000)));
        float glow_pulse = (0.400000 + (u_mouse_down * 0.600000)) * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(1.000000, 0.600000, 0.150000), color_result.a);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
        final_color = vec4(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
...
`;
const UNIFORMS = [{name:'heat',default:0},{name:'viscosity',default:0.28}];
const COMPLEXITY = {layers:5,fbmOctaves:27,passes:2,memory:true,compute:false,is3d:false,tier:'extreme'};
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'heat', target: 'veins', field: 'intensity', weight: 0.25 },
      { source: 'heat', target: 'embers', field: 'brightness', weight: 0.3 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class GameArcTimeline {
  constructor() {
    ...
    this._entries = [
      { target: 'heat', from: 0, to: 0.9, duration: 12, easing: 'ease_in_out' },
    ];
  }
...
class EmberFlow extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('glyph-ember-flow: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get viscosity() { return this._renderer?.userParams['viscosity'] ?? this._pendingParams['viscosity'] ?? 0.28; }
  set viscosity(v) { this.setParam('viscosity', v); }
...
customElements.define('glyph-ember-flow', EmberFlow);
})();
