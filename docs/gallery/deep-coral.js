// GLYPH Component: deep-coral — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.003000, 0.008000, 0.020000), color_result.a);
...
void main(){
    ...
    color_result = vec4(color_result.rgb * vec3(0.003000, 0.008000, 0.020000), color_result.a);
...
`;
const UNIFORMS = [{name:'color_r',default:0.3},{name:'color_g',default:0.6},{name:'color_b',default:1.5}];
...
class GameReactionField {
  constructor(device, computeCode) { this._w = 256; this._h = 256; this._device = device; this._code = computeCode; this._resetTime = 0; }
  ...
  _seed() {
    ...
    const r = Math.floor(Math.max(this._w, this._h) * 0.2);
    for (let dy = -r; dy <= r; dy++) for (let dx = -r; dx <= r; dx++) {
      if (dx*dx + dy*dy <= r*r) {
        const idx = ((cy + dy) * this._w + (cx + dx)) * 2;
        if (idx >= 0 && idx < init.length - 1) init[idx + 1] = 1.0;
      }
    }
    this._device.queue.writeBuffer(this._bufA, 0, init);
    this._resetTime = performance.now() * 0.001;
  }

  dispatch(steps = 8) {
    const t = performance.now() * 0.001;
    const elapsed = t - this._resetTime;
    // Boomerang: 12s cycle — grow 9s, dissolve 3s, then re-seed
    if (elapsed > 12) this._seed();
    // Dissolve phase: spike kill in final 3s to melt patterns
    const dissolve = elapsed > 9 ? (elapsed - 9) / 3 : 0;
    const device = this._device;
    const params = new ArrayBuffer(24);
    const f = new Float32Array(params);
    const u = new Uint32Array(params);
    f[0] = 0.037 + Math.sin(t * 0.7) * 0.001 - dissolve * 0.03;
    f[1] = 0.06 + Math.cos(t * 0.5) * 0.0005 + dissolve * 0.02;
    f[2] = 1; f[3] = 0.5;
...
class DeepCoral extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    const gpu = new GlyphRenderer(this._canvas, WGSL_V, WGSL_F, UNIFORMS, PASS_SHADERS, 'react');
    if (await gpu.init()) {
      this._renderer = gpu;
    } else {
      ...
      if (gl.init()) {
        ...
        console.warn('glyph-deep-coral: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get color_r() { return this._renderer?.userParams['color_r'] ?? this._pendingParams['color_r'] ?? 0.3; }
  set color_r(v) { this.setParam('color_r', v); }
  get color_g() { return this._renderer?.userParams['color_g'] ?? this._pendingParams['color_g'] ?? 0.6; }
...
customElements.define('glyph-deep-coral', DeepCoral);
})();
