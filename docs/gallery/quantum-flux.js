// GLYPH Component: quantum-flux — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let entanglement = u.p_entanglement;
    ...
    // ── Layer 1: wave_fn ──
    {
        ...
        { let warp_x = fbm2(p * 2.000000 + vec2<f32>(0.0, 1.3), i32(5.000000), 0.500000, 0.200000);
        let warp_y = fbm2(p * 2.000000 + vec2<f32>(1.7, 0.0), i32(5.000000), 0.500000, 0.200000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.200000; }
        var sdf_result = fbm2((p * 4.000000 + vec2<f32>(time * 0.1, time * 0.07)), i32(6.000000), 0.550000, 2.000000);
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.000000, 0.020000, 0.060000), vec3<f32>(0.150000, 0.100000, 0.300000), vec3<f32>(0.600000, 0.400000, 1.000000), vec3<f32>(0.100000, 0.250000, 0.500000));
    ...
    // ── Layer 2: interference_a ──
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let warp_x = fbm2(p * 1.500000 + vec2<f32>(0.0, 1.3), i32(4.000000), (0.150000 + (coherence * 0.100000)), 2.000000);
        let warp_y = fbm2(p * 1.500000 + vec2<f32>(1.7, 0.0), i32(4.000000), (0.150000 + (coherence * 0.100000)), 2.000000);
        p = p + vec2<f32>(warp_x, warp_y) * (0.150000 + (coherence * 0.100000)); }
        var sdf_result = noise2(p * 10.000000 + vec2<f32>(time * 0.1, time * 0.07));
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.5, 0.2, 0.5), vec3<f32>(0.5, 0.3, 0.5), vec3<f32>(1.0, 1.0, 1.0), vec3<f32>(0.15, 0.0, 0.5));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    // ── Layer 3: interference_b ──
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let ra = time * 1.570000; let rc = cos(ra); let rs = sin(ra);
        p = vec2<f32>(p.x * rc - p.y * rs, p.x * rs + p.y * rc); }
        { let warp_x = fbm2(p * 1.500000 + vec2<f32>(0.0, 1.3), i32(4.000000), (0.150000 + (coherence * 0.080000)), 2.000000);
        let warp_y = fbm2(p * 1.500000 + vec2<f32>(1.7, 0.0), i32(4.000000), (0.150000 + (coherence * 0.080000)), 2.000000);
        p = p + vec2<f32>(warp_x, warp_y) * (0.150000 + (coherence * 0.080000)); }
        var sdf_result = noise2(p * 10.000000 + vec2<f32>(time * 0.1, time * 0.07));
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p + vec2<f32>(sin(p.y * 2.000000 + time * 0.700000), cos(p.x * 2.000000 + time * 0.700000)) * (0.030000 + (coherence * 0.020000));
        var sdf_result = sdf_circle(p, ((0.040000 + (coherence * 0.030000)) + (u.mouse_down * 0.050000)));
        let glow_pulse = (5.000000 + (coherence * 3.000000)) * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.800000, 0.600000, 1.000000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p + vec2<f32>(sin(p.y * 3.000000 + time * 0.500000), cos(p.x * 3.000000 + time * 0.500000)) * 0.050000;
        var sdf_result = sdf_circle(p, (0.120000 + (entanglement * 0.040000)));
        for (var onion_i: i32 = 0; onion_i < i32(3.000000); onion_i = onion_i + 1) { sdf_result = abs(sdf_result) - 0.008000; }
        let glow_pulse = (1.500000 + (entanglement * 0.800000)) * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.400000, 0.300000, 0.800000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = vec2<f32>(abs(p.x), p.y);
        p = p - vec2<f32>(0.200000, 0.000000);
        p = p + vec2<f32>(sin(p.y * 2.000000 + time * 0.700000), cos(p.x * 2.000000 + time * 0.700000)) * 0.030000;
        var sdf_result = sdf_circle(p, (0.030000 + (entanglement * 0.020000)));
        let glow_pulse = (3.000000 + (entanglement * 2.000000)) * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(1.000000, 0.500000, 0.700000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let warp_x = fbm2(p * 8.000000 + vec2<f32>(0.0, 1.3), i32(3.000000), 0.200000, 2.000000);
        let warp_y = fbm2(p * 8.000000 + vec2<f32>(1.7, 0.0), i32(3.000000), 0.200000, 2.000000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.200000; }
        var sdf_result = voronoi2(p * 20.000000 + vec2<f32>(time * 0.05, time * 0.03));
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.000000, 0.010000, 0.030000), vec3<f32>(0.050000, 0.030000, 0.100000), vec3<f32>(0.400000, 0.300000, 0.700000), vec3<f32>(0.000000, 0.150000, 0.300000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
...
uniform float u_p_coherence;
uniform float u_p_entanglement;
...
void main(){
    ...
    float coherence = u_p_coherence;
    float entanglement = u_p_entanglement;
    ...
    // ── Layer 1: wave_fn ──
    {
        ...
        { float warp_x = fbm2(p * 2.000000 + vec2(0.0, 1.3), int(5.000000), 0.500000, 0.200000);
        float warp_y = fbm2(p * 2.000000 + vec2(1.7, 0.0), int(5.000000), 0.500000, 0.200000);
        p = p + vec2(warp_x, warp_y) * 0.200000; }
        float sdf_result = fbm2((p * 4.000000 + vec2(time * 0.1, time * 0.07)), int(6.000000), 0.550000, 2.000000);
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.000000, 0.020000, 0.060000), vec3(0.150000, 0.100000, 0.300000), vec3(0.600000, 0.400000, 1.000000), vec3(0.100000, 0.250000, 0.500000));
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        { float warp_x = fbm2(p * 1.500000 + vec2(0.0, 1.3), int(4.000000), (0.150000 + (coherence * 0.100000)), 2.000000);
        float warp_y = fbm2(p * 1.500000 + vec2(1.7, 0.0), int(4.000000), (0.150000 + (coherence * 0.100000)), 2.000000);
        p = p + vec2(warp_x, warp_y) * (0.150000 + (coherence * 0.100000)); }
        float sdf_result = noise2(p * 10.000000 + vec2(time * 0.1, time * 0.07));
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.5, 0.2, 0.5), vec3(0.5, 0.3, 0.5), vec3(1.0, 1.0, 1.0), vec3(0.15, 0.0, 0.5));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.880000);
        float la = color_result.a;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        { float ra = time * 1.570000; float rc = cos(ra); float rs = sin(ra);
        p = vec2(p.x * rc - p.y * rs, p.x * rs + p.y * rc); }
        { float warp_x = fbm2(p * 1.500000 + vec2(0.0, 1.3), int(4.000000), (0.150000 + (coherence * 0.080000)), 2.000000);
        float warp_y = fbm2(p * 1.500000 + vec2(1.7, 0.0), int(4.000000), (0.150000 + (coherence * 0.080000)), 2.000000);
        p = p + vec2(warp_x, warp_y) * (0.150000 + (coherence * 0.080000)); }
        float sdf_result = noise2(p * 10.000000 + vec2(time * 0.1, time * 0.07));
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p + vec2(sin(p.y * 2.000000 + time * 0.700000), cos(p.x * 2.000000 + time * 0.700000)) * (0.030000 + (coherence * 0.020000));
        float sdf_result = sdf_circle(p, ((0.040000 + (coherence * 0.030000)) + (u_mouse_down * 0.050000)));
        float glow_pulse = (5.000000 + (coherence * 3.000000)) * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.800000, 0.600000, 1.000000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.930000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p + vec2(sin(p.y * 3.000000 + time * 0.500000), cos(p.x * 3.000000 + time * 0.500000)) * 0.050000;
        float sdf_result = sdf_circle(p, (0.120000 + (entanglement * 0.040000)));
        for (int onion_i = 0; onion_i < int(3.000000); onion_i++) { sdf_result = abs(sdf_result) - 0.008000; }
        float glow_pulse = (1.500000 + (entanglement * 0.800000)) * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.400000, 0.300000, 0.800000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.890000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    // ── Layer 6: entangled ──
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = vec2(abs(p.x), p.y);
        p = p - vec2(0.200000, 0.000000);
        p = p + vec2(sin(p.y * 2.000000 + time * 0.700000), cos(p.x * 2.000000 + time * 0.700000)) * 0.030000;
        float sdf_result = sdf_circle(p, (0.030000 + (entanglement * 0.020000)));
        float glow_pulse = (3.000000 + (entanglement * 2.000000)) * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(1.000000, 0.500000, 0.700000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.900000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        { float warp_x = fbm2(p * 8.000000 + vec2(0.0, 1.3), int(3.000000), 0.200000, 2.000000);
        float warp_y = fbm2(p * 8.000000 + vec2(1.7, 0.0), int(3.000000), 0.200000, 2.000000);
        p = p + vec2(warp_x, warp_y) * 0.200000; }
        float sdf_result = voronoi2(p * 20.000000 + vec2(time * 0.05, time * 0.03));
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.000000, 0.010000, 0.030000), vec3(0.050000, 0.030000, 0.100000), vec3(0.400000, 0.300000, 0.700000), vec3(0.000000, 0.150000, 0.300000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.830000);
        float la = color_result.a;
...
`;
const UNIFORMS = [{name:'coherence',default:0},{name:'entanglement',default:0.3}];
const COMPLEXITY = {layers:7,fbmOctaves:22,passes:3,memory:true,compute:false,is3d:false,tier:'extreme'};
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let ca_dir = normalize(uv - 0.5) * 0.005000;
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'coherence', target: 'particle', field: 'brightness', weight: 0.5 },
      { source: 'coherence', target: 'interference_a', field: 'intensity', weight: 0.4 },
      { source: 'entanglement', target: 'entangled', field: 'brightness', weight: 0.5 },
      { source: 'entanglement', target: 'uncertainty', field: 'brightness', weight: 0.3 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class GameArcTimeline {
  constructor() {
    ...
    this._entries = [
      { target: 'coherence', from: 0, to: 1, duration: 8, easing: 'ease_in_out' },
      { target: 'entanglement', from: 0.3, to: 1, duration: 12, easing: 'ease_in_out' },
    ];
  }
...
class QuantumFlux extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('glyph-quantum-flux: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get coherence() { return this._renderer?.userParams['coherence'] ?? this._pendingParams['coherence'] ?? 0; }
  set coherence(v) { this.setParam('coherence', v); }
  get entanglement() { return this._renderer?.userParams['entanglement'] ?? this._pendingParams['entanglement'] ?? 0.3; }
  set entanglement(v) { this.setParam('entanglement', v); }
...
customElements.define('glyph-quantum-flux', QuantumFlux);
})();
