// GLYPH Component: deep-current — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 1: water ──
    {
        ...
        p = p + vec2<f32>(sin(p.y * 2.000000 + time * flow_speed), cos(p.x * 2.000000 + time * flow_speed)) * 0.120000;
        var sdf_result = fbm2((p * 2.000000 + vec2<f32>(time * 0.1, time * 0.07)), i32(5.000000), 0.500000, 2.000000);
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.250000, 0.350000, 0.500000), vec3<f32>(0.200000, 0.250000, 0.300000), vec3<f32>(0.800000, 0.600000, 0.400000), vec3<f32>(0.550000, 0.600000, 0.700000));
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let warp_x = fbm2(p * (2.500000 + (u.mouse.x * 0.400000)) + vec2<f32>(0.0, 1.3), i32(5.000000), 0.500000, (0.280000 + (u.mouse_down * 0.060000)));
        let warp_y = fbm2(p * (2.500000 + (u.mouse.x * 0.400000)) + vec2<f32>(1.7, 0.0), i32(5.000000), 0.500000, (0.280000 + (u.mouse_down * 0.060000)));
        p = p + vec2<f32>(warp_x, warp_y) * (0.280000 + (u.mouse_down * 0.060000)); }
        p = p + vec2<f32>(sin(p.y * 3.000000 + time * (flow_speed * 1.300000)), cos(p.x * 3.000000 + time * (flow_speed * 1.300000))) * 0.100000;
        var sdf_result = fbm2((p * 4.000000 + vec2<f32>(time * 0.1, time * 0.07)), i32(4.000000), 0.450000, 2.000000);
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.200000, 0.300000, 0.450000), vec3<f32>(0.180000, 0.220000, 0.280000), vec3<f32>(1.000000, 0.800000, 0.500000), vec3<f32>(0.600000, 0.650000, 0.720000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    // ── Layer 3: organisms ──
    {
        ...
        { let warp_x = fbm2(p * 2.000000 + vec2<f32>(0.0, 1.3), i32(3.000000), 0.220000, 2.000000);
        let warp_y = fbm2(p * 2.000000 + vec2<f32>(1.7, 0.0), i32(3.000000), 0.220000, 2.000000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.220000; }
        p = vec2<f32>(glyph_mod(p.x + 0.130000 * 0.5, 0.130000) - 0.130000 * 0.5, glyph_mod(p.y + 0.130000 * 0.5, 0.130000) - 0.130000 * 0.5);
        var sdf_result = sdf_circle(p, 0.018000);
        let glow_pulse = (2.500000 + (bio_glow * 1.000000)) * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.150000, 0.500000, 0.850000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    // ── Layer 4: vapor ──
    {
        ...
        let glow_pulse = (1.200000 + (bio_glow * 0.400000)) * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.100000, 0.350000, 0.650000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p + vec2<f32>(sin(p.y * 6.000000 + time * (flow_speed * 0.900000)), cos(p.x * 6.000000 + time * (flow_speed * 0.900000))) * 0.220000;
        { let warp_x = fbm2(p * 3.000000 + vec2<f32>(0.0, 1.3), i32(3.000000), 0.300000, 2.000000);
        let warp_y = fbm2(p * 3.000000 + vec2<f32>(1.7, 0.0), i32(3.000000), 0.300000, 2.000000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.300000; }
        var sdf_result = fbm2((p * 16.000000 + vec2<f32>(time * 0.1, time * 0.07)), i32(2.000000), 0.500000, 2.000000);
        let glow_pulse = (3.500000 + (bio_glow * 1.000000)) * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.400000, 0.700000, 1.000000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    // ── Layer 6: ambient ──
    {
        ...
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.220000, 0.280000, 0.400000), vec3<f32>(0.150000, 0.180000, 0.250000), vec3<f32>(0.600000, 0.500000, 0.350000), vec3<f32>(0.000000, 0.080000, 0.120000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let la = color_result.a;
        let lc = color_result.rgb;
        final_color = vec4<f32>(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
...
void main(){
    ...
    // ── Layer 1: water ──
    {
        ...
        { float warp_x = fbm2(p * 1.200000 + vec2(0.0, 1.3), int(6.000000), 0.550000, (0.320000 + (u_mouse_down * 0.080000)));
        float warp_y = fbm2(p * 1.200000 + vec2(1.7, 0.0), int(6.000000), 0.550000, (0.320000 + (u_mouse_down * 0.080000)));
        p = p + vec2(warp_x, warp_y) * (0.320000 + (u_mouse_down * 0.080000)); }
        p = p + vec2(sin(p.y * 2.000000 + time * flow_speed), cos(p.x * 2.000000 + time * flow_speed)) * 0.120000;
        float sdf_result = fbm2((p * 2.000000 + vec2(time * 0.1, time * 0.07)), int(5.000000), 0.500000, 2.000000);
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.250000, 0.350000, 0.500000), vec3(0.200000, 0.250000, 0.300000), vec3(0.800000, 0.600000, 0.400000), vec3(0.550000, 0.600000, 0.700000));
    ...
    // ── Layer 2: ripples ──
    {
        ...
        p = p + vec2(sin(p.y * 3.000000 + time * (flow_speed * 1.300000)), cos(p.x * 3.000000 + time * (flow_speed * 1.300000))) * 0.100000;
        float sdf_result = fbm2((p * 4.000000 + vec2(time * 0.1, time * 0.07)), int(4.000000), 0.450000, 2.000000);
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.200000, 0.300000, 0.450000), vec3(0.180000, 0.220000, 0.280000), vec3(1.000000, 0.800000, 0.500000), vec3(0.600000, 0.650000, 0.720000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.860000);
        float la = color_result.a;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p + vec2(sin(p.y * 4.000000 + time * (flow_speed * 0.700000)), cos(p.x * 4.000000 + time * (flow_speed * 0.700000))) * 0.180000;
        ...
        p = vec2(mod(p.x + 0.130000 * 0.5, 0.130000) - 0.130000 * 0.5, mod(p.y + 0.130000 * 0.5, 0.130000) - 0.130000 * 0.5);
        float sdf_result = sdf_circle(p, 0.018000);
        float glow_pulse = (2.500000 + (bio_glow * 1.000000)) * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.150000, 0.500000, 0.850000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.840000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p + vec2(sin(p.y * 3.000000 + time * (flow_speed * 0.500000)), cos(p.x * 3.000000 + time * (flow_speed * 0.500000))) * 0.120000;
        { float warp_x = fbm2(p * 2.000000 + vec2(0.0, 1.3), int(4.000000), 0.500000, 0.220000);
        float warp_y = fbm2(p * 2.000000 + vec2(1.7, 0.0), int(4.000000), 0.500000, 0.220000);
        p = p + vec2(warp_x, warp_y) * 0.220000; }
        float sdf_result = noise2(p * 5.000000 + vec2(time * 0.1, time * 0.07));
        float glow_pulse = (1.200000 + (bio_glow * 0.400000)) * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.100000, 0.350000, 0.650000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.950000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p + vec2(sin(p.y * 6.000000 + time * (flow_speed * 0.900000)), cos(p.x * 6.000000 + time * (flow_speed * 0.900000))) * 0.220000;
        ...
        float sdf_result = fbm2((p * 16.000000 + vec2(time * 0.1, time * 0.07)), int(2.000000), 0.500000, 2.000000);
        float glow_pulse = (3.500000 + (bio_glow * 1.000000)) * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.400000, 0.700000, 1.000000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.700000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    // ── Layer 6: ambient ──
    {
        ...
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.220000, 0.280000, 0.400000), vec3(0.150000, 0.180000, 0.250000), vec3(0.600000, 0.500000, 0.350000), vec3(0.000000, 0.080000, 0.120000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        float la = color_result.a;
        vec3 lc = color_result.rgb;
        final_color = vec4(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
...
`;
const UNIFORMS = [{name:'flow_speed',default:0.3},{name:'bio_glow',default:0}];
const COMPLEXITY = {layers:6,fbmOctaves:32,passes:2,memory:true,compute:false,is3d:false,tier:'extreme'};
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'bio_glow', target: 'organisms', field: 'brightness', weight: 0.3 },
      { source: 'bio_glow', target: 'sparkles', field: 'brightness', weight: 0.25 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class GameArcTimeline {
  constructor() {
    ...
    this._entries = [
      { target: 'bio_glow', from: 0, to: 0.8, duration: 10, easing: 'ease_in_out' },
    ];
  }
...
class DeepCurrent extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('glyph-deep-current: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get flow_speed() { return this._renderer?.userParams['flow_speed'] ?? this._pendingParams['flow_speed'] ?? 0.3; }
  set flow_speed(v) { this.setParam('flow_speed', v); }
  get bio_glow() { return this._renderer?.userParams['bio_glow'] ?? this._pendingParams['bio_glow'] ?? 0; }
  set bio_glow(v) { this.setParam('bio_glow', v); }
...
customElements.define('glyph-deep-current', DeepCurrent);
})();
