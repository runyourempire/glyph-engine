// GLYPH Component: mitosis — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.015000, 0.003000, 0.005000), color_result.a);
...
void main(){
    ...
    color_result = vec4(color_result.rgb * vec3(0.015000, 0.003000, 0.005000), color_result.a);
...
`;
const UNIFORMS = [{name:'color_r',default:1.5},{name:'color_g',default:0.15},{name:'color_b',default:0.15}];
...
class GameReactionField {
  ...
  dispatch(steps = 8) {
    ...
    f[0] = 0.028 + Math.sin(t * 0.7) * 0.001 - dissolve * 0.03;
    f[1] = 0.062 + Math.cos(t * 0.5) * 0.0005 + dissolve * 0.02;
    f[2] = 1; f[3] = 0.5;
...
class Mitosis extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('glyph-mitosis: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get color_r() { return this._renderer?.userParams['color_r'] ?? this._pendingParams['color_r'] ?? 1.5; }
  set color_r(v) { this.setParam('color_r', v); }
  get color_g() { return this._renderer?.userParams['color_g'] ?? this._pendingParams['color_g'] ?? 0.15; }
  set color_g(v) { this.setParam('color_g', v); }
  get color_b() { return this._renderer?.userParams['color_b'] ?? this._pendingParams['color_b'] ?? 0.15; }
...
customElements.define('glyph-mitosis', Mitosis);
})();
