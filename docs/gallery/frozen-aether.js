// GLYPH Component: frozen-aether — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 1: frost_field ──
    {
        ...
        { let warp_x = fbm2(p * 2.000000 + vec2<f32>(0.0, 1.3), i32(4.000000), 0.400000, 0.120000);
        let warp_y = fbm2(p * 2.000000 + vec2<f32>(1.7, 0.0), i32(4.000000), 0.400000, 0.120000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.120000; }
        var sdf_result = voronoi2(p * 12.000000 + vec2<f32>(time * 0.05, time * 0.03));
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.5, 0.7, 0.9), vec3<f32>(0.2, 0.2, 0.1), vec3<f32>(1.0, 1.0, 1.0), vec3<f32>(0.0, 0.05, 0.15));
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let ra = time * 0.100000; let rc = cos(ra); let rs = sin(ra);
        ...
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.7, 0.8, 0.95), vec3<f32>(0.2, 0.15, 0.1), vec3<f32>(1.0, 1.0, 1.0), vec3<f32>(0.0, 0.05, 0.1));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let warp_x = fbm2(p * 4.000000 + vec2<f32>(0.0, 1.3), i32(5.000000), 0.550000, 0.200000);
        let warp_y = fbm2(p * 4.000000 + vec2<f32>(1.7, 0.0), i32(5.000000), 0.550000, 0.200000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.200000; }
        var sdf_result = fbm2((p * 6.000000 + vec2<f32>(time * 0.1, time * 0.07)), i32(6.000000), 0.450000, 2.000000);
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.5, 0.5, 0.55), vec3<f32>(0.2, 0.2, 0.2), vec3<f32>(1.0, 1.0, 1.0), vec3<f32>(0.0, 0.0, 0.1));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p + vec2<f32>(sin(p.y * 1.500000 + time * 0.150000), cos(p.x * 1.500000 + time * 0.150000)) * (0.010000 + (thaw * 0.010000));
        var sdf_result = sdf_star(p, 6.000000, (0.120000 + (shimmer * 0.020000)), 0.060000);
        let glow_pulse = (3.500000 + (shimmer * 1.500000)) * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.700000, 0.850000, 1.000000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p + vec2<f32>(sin(p.y * 2.000000 + time * 0.200000), cos(p.x * 2.000000 + time * 0.200000)) * 0.015000;
        var sdf_result = abs(length(p) - 0.160000) - 0.025000;
        let glow_pulse = (2.000000 + (shimmer * 0.800000)) * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.500000, 0.700000, 0.950000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let warp_x = fbm2(p * 6.000000 + vec2<f32>(0.0, 1.3), i32(3.000000), 0.150000, 2.000000);
        let warp_y = fbm2(p * 6.000000 + vec2<f32>(1.7, 0.0), i32(3.000000), 0.150000, 2.000000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.150000; }
        var sdf_result = voronoi2(p * 25.000000 + vec2<f32>(time * 0.05, time * 0.03));
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.000000, 0.020000, 0.050000), vec3<f32>(0.100000, 0.150000, 0.200000), vec3<f32>(0.800000, 0.900000, 1.000000), vec3<f32>(0.000000, 0.100000, 0.200000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        var sdf_result = abs(length(p) - 0.460000) - 0.003000;
        let glow_pulse = 0.600000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.400000, 0.500000, 0.700000), color_result.a);
        let la = color_result.a;
        let lc = color_result.rgb;
        final_color = vec4<f32>(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
...
void main(){
    ...
    // ── Layer 1: frost_field ──
    {
        ...
        { float warp_x = fbm2(p * 2.000000 + vec2(0.0, 1.3), int(4.000000), 0.400000, 0.120000);
        float warp_y = fbm2(p * 2.000000 + vec2(1.7, 0.0), int(4.000000), 0.400000, 0.120000);
        p = p + vec2(warp_x, warp_y) * 0.120000; }
        float sdf_result = voronoi2(p * 12.000000 + vec2(time * 0.05, time * 0.03));
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.5, 0.7, 0.9), vec3(0.2, 0.2, 0.1), vec3(1.0, 1.0, 1.0), vec3(0.0, 0.05, 0.15));
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        { float ra = time * 0.100000; float rc = cos(ra); float rs = sin(ra);
        ...
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.7, 0.8, 0.95), vec3(0.2, 0.15, 0.1), vec3(1.0, 1.0, 1.0), vec3(0.0, 0.05, 0.1));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.940000);
        float la = color_result.a;
    ...
    // ── Layer 3: veins ──
    {
        ...
        float sdf_result = fbm2((p * 6.000000 + vec2(time * 0.1, time * 0.07)), int(6.000000), 0.450000, 2.000000);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p + vec2(sin(p.y * 1.500000 + time * 0.150000), cos(p.x * 1.500000 + time * 0.150000)) * (0.010000 + (thaw * 0.010000));
        float sdf_result = sdf_star(p, 6.000000, (0.120000 + (shimmer * 0.020000)), 0.060000);
        float glow_pulse = (3.500000 + (shimmer * 1.500000)) * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.700000, 0.850000, 1.000000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.930000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p + vec2(sin(p.y * 2.000000 + time * 0.200000), cos(p.x * 2.000000 + time * 0.200000)) * 0.015000;
        float sdf_result = abs(length(p) - 0.160000) - 0.025000;
        float glow_pulse = (2.000000 + (shimmer * 0.800000)) * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.500000, 0.700000, 0.950000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.900000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        { float warp_x = fbm2(p * 6.000000 + vec2(0.0, 1.3), int(3.000000), 0.150000, 2.000000);
        float warp_y = fbm2(p * 6.000000 + vec2(1.7, 0.0), int(3.000000), 0.150000, 2.000000);
        p = p + vec2(warp_x, warp_y) * 0.150000; }
        float sdf_result = voronoi2(p * 25.000000 + vec2(time * 0.05, time * 0.03));
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.000000, 0.020000, 0.050000), vec3(0.100000, 0.150000, 0.200000), vec3(0.800000, 0.900000, 1.000000), vec3(0.000000, 0.100000, 0.200000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.850000);
        float la = color_result.a;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        float sdf_result = abs(length(p) - 0.460000) - 0.003000;
        float glow_pulse = 0.600000 * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.400000, 0.500000, 0.700000), color_result.a);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
        final_color = vec4(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
...
`;
const UNIFORMS = [{name:'thaw',default:0},{name:'shimmer',default:0.3}];
const COMPLEXITY = {layers:7,fbmOctaves:21,passes:3,memory:true,compute:false,is3d:false,tier:'extreme'};
const PASS_WGSL_0 = `// Post-processing pass: frost
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'shimmer', target: 'gem', field: 'brightness', weight: 0.4 },
      { source: 'shimmer', target: 'halo', field: 'brightness', weight: 0.3 },
      { source: 'thaw', target: 'crystals', field: 'intensity', weight: 0.2 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class GameArcTimeline {
  constructor() {
    ...
    this._entries = [
      { target: 'thaw', from: 0, to: 0.5, duration: 20, easing: 'ease_in_out' },
      { target: 'shimmer', from: 0.3, to: 1, duration: 15, easing: 'ease_in_out' },
    ];
  }
...
class FrozenAether extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('glyph-frozen-aether: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get thaw() { return this._renderer?.userParams['thaw'] ?? this._pendingParams['thaw'] ?? 0; }
  set thaw(v) { this.setParam('thaw', v); }
  get shimmer() { return this._renderer?.userParams['shimmer'] ?? this._pendingParams['shimmer'] ?? 0.3; }
...
customElements.define('glyph-frozen-aether', FrozenAether);
})();
