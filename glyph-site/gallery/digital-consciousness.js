// GLYPH Component: digital-consciousness — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 1: data_flow ──
    {
        ...
        p = p / 0.400000;
        { let warp_x = fbm2(p * 2.000000 + vec2<f32>(0.0, 1.3), i32(4.000000), 0.500000, 0.200000);
        let warp_y = fbm2(p * 2.000000 + vec2<f32>(1.7, 0.0), i32(4.000000), 0.500000, 0.200000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.200000; }
        var sdf_result = fbm2((p * 6.000000 + vec2<f32>(time * 0.1, time * 0.07)), i32(5.000000), 0.500000, 2.000000);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let warp_x = fbm2(p * 1.000000 + vec2<f32>(0.0, 1.3), i32(3.000000), 0.050000, 2.000000);
        let warp_y = fbm2(p * 1.000000 + vec2<f32>(1.7, 0.0), i32(3.000000), 0.050000, 2.000000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.050000; }
        let gx = abs(glyph_mod(p.x + 0.060000 * 0.5, 0.060000) - 0.060000 * 0.5) - 0.001000;
        let gy = abs(glyph_mod(p.y + 0.060000 * 0.5, 0.060000) - 0.060000 * 0.5) - 0.001000;
        var sdf_result = min(gx, gy);
        let glow_pulse = (0.800000 + (processing * 0.400000)) * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.000000, 0.300000, 0.100000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let warp_x = fbm2(p * 3.000000 + vec2<f32>(0.0, 1.3), i32(6.000000), 0.600000, 2.200000);
        let warp_y = fbm2(p * 3.000000 + vec2<f32>(1.7, 0.0), i32(6.000000), 0.600000, 2.200000);
        p = p + vec2<f32>(warp_x, warp_y) * (0.250000 + (awareness * 0.100000)); }
        var sdf_result = fbm2((p * 5.000000 + vec2<f32>(time * 0.1, time * 0.07)), i32(6.000000), 0.500000, 2.000000);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p + vec2<f32>(sin(p.y * 1.500000 + time * 0.500000), cos(p.x * 1.500000 + time * 0.500000)) * (0.020000 + (awareness * 0.020000));
        var sdf_result = sdf_circle(p, (0.060000 + (awareness * 0.040000)));
        let glow_pulse = (4.500000 + (awareness * 3.000000)) * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.200000, 1.000000, 0.500000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    // ── Layer 5: thought ──
    {
        ...
        var sdf_result = sdf_circle(p, (0.180000 + (awareness * 0.060000)));
        for (var onion_i: i32 = 0; onion_i < i32(4.000000); onion_i = onion_i + 1) { sdf_result = abs(sdf_result) - 0.006000; }
        let glow_pulse = (1.500000 + (processing * 0.800000)) * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.100000, 0.600000, 0.300000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    // ── Layer 6: nodes ──
    {
        ...
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.000000, 0.030000, 0.010000), vec3<f32>(0.020000, 0.150000, 0.050000), vec3<f32>(0.100000, 0.800000, 0.300000), vec3<f32>(0.000000, 0.330000, 0.100000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    // ── Layer 7: frame ──
    {
        ...
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.000000, 0.200000, 0.080000), color_result.a);
        let la = color_result.a;
        let lc = color_result.rgb;
        final_color = vec4<f32>(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
...
uniform float u_p_awareness;
uniform float u_p_processing;
...
void main(){
    ...
    float awareness = u_p_awareness;
    float processing = u_p_processing;
    ...
    // ── Layer 1: data_flow ──
    {
        ...
        p = p / 0.400000;
        { float warp_x = fbm2(p * 2.000000 + vec2(0.0, 1.3), int(4.000000), 0.500000, 0.200000);
        float warp_y = fbm2(p * 2.000000 + vec2(1.7, 0.0), int(4.000000), 0.500000, 0.200000);
        p = p + vec2(warp_x, warp_y) * 0.200000; }
        float sdf_result = fbm2((p * 6.000000 + vec2(time * 0.1, time * 0.07)), int(5.000000), 0.500000, 2.000000);
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.0, 0.3, 0.0), vec3(0.0, 0.5, 0.0), vec3(0.0, 1.0, 0.0), vec3(0.0, 0.2, 0.0));
    ...
    // ── Layer 2: circuit ──
    {
        ...
        float gx = abs(mod(p.x + 0.060000 * 0.5, 0.060000) - 0.060000 * 0.5) - 0.001000;
        float gy = abs(mod(p.y + 0.060000 * 0.5, 0.060000) - 0.060000 * 0.5) - 0.001000;
        float sdf_result = min(gx, gy);
        float glow_pulse = (0.800000 + (processing * 0.400000)) * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.000000, 0.300000, 0.100000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.870000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    // ── Layer 3: pathways ──
    {
        ...
        p = p + vec2(warp_x, warp_y) * (0.250000 + (awareness * 0.100000)); }
        float sdf_result = fbm2((p * 5.000000 + vec2(time * 0.1, time * 0.07)), int(6.000000), 0.500000, 2.000000);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p + vec2(sin(p.y * 1.500000 + time * 0.500000), cos(p.x * 1.500000 + time * 0.500000)) * (0.020000 + (awareness * 0.020000));
        float sdf_result = sdf_circle(p, (0.060000 + (awareness * 0.040000)));
        float glow_pulse = (4.500000 + (awareness * 3.000000)) * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.200000, 1.000000, 0.500000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.950000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    // ── Layer 5: thought ──
    {
        ...
        float sdf_result = sdf_circle(p, (0.180000 + (awareness * 0.060000)));
        for (int onion_i = 0; onion_i < int(4.000000); onion_i++) { sdf_result = abs(sdf_result) - 0.006000; }
        float glow_pulse = (1.500000 + (processing * 0.800000)) * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.100000, 0.600000, 0.300000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.880000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    // ── Layer 6: nodes ──
    {
        ...
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.000000, 0.030000, 0.010000), vec3(0.020000, 0.150000, 0.050000), vec3(0.100000, 0.800000, 0.300000), vec3(0.000000, 0.330000, 0.100000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.860000);
        float la = color_result.a;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        float sdf_result = abs(length(p) - 0.450000) - 0.002000;
        float glow_pulse = 0.600000 * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.000000, 0.200000, 0.080000), color_result.a);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
        final_color = vec4(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
...
`;
const UNIFORMS = [{name:'awareness',default:0},{name:'processing',default:0.5}];
...
`;
const PASS_WGSL_1 = `// Post-processing pass: scan
...
`;
const PASS_WGSL_2 = `// Post-processing pass: frame_pass
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'awareness', target: 'core', field: 'brightness', weight: 0.6 },
      { source: 'awareness', target: 'pathways', field: 'intensity', weight: 0.4 },
      { source: 'processing', target: 'thought', field: 'brightness', weight: 0.4 },
      { source: 'processing', target: 'circuit', field: 'brightness', weight: 0.3 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class GameArcTimeline {
  constructor() {
    ...
    this._entries = [
      { target: 'awareness', from: 0, to: 1, duration: 10, easing: 'ease_in_out' },
      { target: 'processing', from: 0.5, to: 1, duration: 8, easing: 'ease_out' },
    ];
  }
...
class DigitalConsciousness extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('glyph-digital-consciousness: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get awareness() { return this._renderer?.userParams['awareness'] ?? this._pendingParams['awareness'] ?? 0; }
  set awareness(v) { this.setParam('awareness', v); }
  get processing() { return this._renderer?.userParams['processing'] ?? this._pendingParams['processing'] ?? 0.5; }
  set processing(v) { this.setParam('processing', v); }
...
customElements.define('glyph-digital-consciousness', DigitalConsciousness);
})();
