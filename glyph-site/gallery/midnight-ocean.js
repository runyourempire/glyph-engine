// GLYPH Component: midnight-ocean — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 3: organisms ──
    {
        ...
        p = vec2<f32>(glyph_mod(p.x + 0.120000 * 0.5, 0.120000) - 0.120000 * 0.5, glyph_mod(p.y + 0.120000 * 0.5, 0.120000) - 0.120000 * 0.5);
...
uniform float u_p_flow_strength;
uniform float u_p_bio_intensity;
...
void main(){
    ...
    float flow_strength = u_p_flow_strength;
    float bio_intensity = u_p_bio_intensity;
    ...
    // ── Layer 1: water ──
    {
        ...
        p = p + vec2(sin(p.y * 2.000000 + time * flow_speed), cos(p.x * 2.000000 + time * flow_speed)) * (flow_strength * 0.400000);
        float sdf_result = fbm2((p * 2.500000 + vec2(time * 0.1, time * 0.07)), int(5.000000), 0.500000, 2.000000);
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.150000, 0.200000, 0.350000), vec3(0.150000, 0.200000, 0.300000), vec3(1.000000, 0.800000, 0.600000), vec3(0.550000, 0.600000, 0.700000));
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        { float warp_x = fbm2(p * (2.500000 + (u_mouse.x * 0.400000)) + vec2(0.0, 1.3), int(5.000000), 0.500000, (0.250000 + (u_mouse_down * 0.060000)));
        float warp_y = fbm2(p * (2.500000 + (u_mouse.x * 0.400000)) + vec2(1.7, 0.0), int(5.000000), 0.500000, (0.250000 + (u_mouse_down * 0.060000)));
        p = p + vec2(warp_x, warp_y) * (0.250000 + (u_mouse_down * 0.060000)); }
        p = p + vec2(sin(p.y * 3.000000 + time * (flow_speed * 1.400000)), cos(p.x * 3.000000 + time * (flow_speed * 1.400000))) * (flow_strength * 0.500000);
        float sdf_result = fbm2((p * 5.000000 + vec2(time * 0.1, time * 0.07)), int(4.000000), 0.450000, 2.000000);
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.100000, 0.180000, 0.300000), vec3(0.120000, 0.180000, 0.250000), vec3(1.200000, 0.800000, 0.500000), vec3(0.600000, 0.650000, 0.750000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.850000);
    ...
    // ── Layer 3: organisms ──
    {
        ...
        p = vec2(mod(p.x + 0.120000 * 0.5, 0.120000) - 0.120000 * 0.5, mod(p.y + 0.120000 * 0.5, 0.120000) - 0.120000 * 0.5);
        float sdf_result = sdf_circle(p, 0.020000);
        float glow_pulse = (2.500000 + (bio_intensity * 1.000000)) * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.200000, 0.600000, 0.900000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.820000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    // ── Layer 4: trails ──
    {
        ...
        float glow_pulse = (1.200000 + (bio_intensity * 0.500000)) * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.150000, 0.400000, 0.700000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.940000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    // ── Layer 5: cursor ──
    {
        ...
        float sdf_result = sdf_circle(p, (0.030000 + (u_mouse_down * 0.030000)));
        float glow_pulse = (0.500000 + (u_mouse_down * 0.500000)) * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.200000, 0.500000, 0.800000), color_result.a);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
        final_color = vec4(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
...
`;
const UNIFORMS = [{name:'flow_speed',default:0.3},{name:'flow_strength',default:0.28},{name:'bio_intensity',default:0}];
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'bio_intensity', target: 'organisms', field: 'brightness', weight: 0.3 },
      { source: 'bio_intensity', target: 'trails', field: 'intensity', weight: 0.2 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class GameArcTimeline {
  constructor() {
    ...
    this._entries = [
      { target: 'bio_intensity', from: 0, to: 0.8, duration: 12, easing: 'ease_in_out' },
    ];
  }
...
class MidnightOcean extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('glyph-midnight-ocean: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get flow_strength() { return this._renderer?.userParams['flow_strength'] ?? this._pendingParams['flow_strength'] ?? 0.28; }
  set flow_strength(v) { this.setParam('flow_strength', v); }
  get bio_intensity() { return this._renderer?.userParams['bio_intensity'] ?? this._pendingParams['bio_intensity'] ?? 0; }
  set bio_intensity(v) { this.setParam('bio_intensity', v); }
...
customElements.define('glyph-midnight-ocean', MidnightOcean);
})();
