// GLYPH Component: void-engine — auto-generated, do not edit.
(function(){
...
void main(){
    ...
    // ── Layer 1: substrate ──
    {
        ...
        { float warp_x = fbm2(p * 2.000000 + vec2(0.0, 1.3), int(6.000000), 0.500000, 0.200000);
        float warp_y = fbm2(p * 2.000000 + vec2(1.7, 0.0), int(6.000000), 0.500000, 0.200000);
        p = p + vec2(warp_x, warp_y) * 0.200000; }
        float sdf_result = fbm2((p * 2.500000 + vec2(time * 0.1, time * 0.07)), int(6.000000), 0.550000, 2.000000);
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.010000, 0.010000, 0.030000), vec3(0.050000, 0.030000, 0.100000), vec3(0.150000, 0.100000, 0.250000), vec3(0.000000, 0.050000, 0.150000));
    ...
    // ── Layer 2: intel_field ──
    {
        ...
        { float warp_x = fbm2(p * 3.000000 + vec2(0.0, 1.3), int(5.000000), 0.550000, (0.250000 + (awareness * 0.120000)));
        float warp_y = fbm2(p * 3.000000 + vec2(1.7, 0.0), int(5.000000), 0.550000, (0.250000 + (awareness * 0.120000)));
        p = p + vec2(warp_x, warp_y) * (0.250000 + (awareness * 0.120000)); }
        float sdf_result = fbm2((p * 4.000000 + vec2(time * 0.1, time * 0.07)), int(6.000000), 0.500000, 2.000000);
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.000000, 0.020000, 0.050000), vec3(0.080000, 0.060000, 0.200000), vec3(0.400000, 0.300000, 0.800000), vec3(0.000000, 0.150000, 0.400000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.930000);
        float la = color_result.a;
    ...
    // ── Layer 3: streams ──
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = vec2(length(p), atan(p.y, p.x));
        { float warp_x = fbm2(p * 5.000000 + vec2(0.0, 1.3), int(4.000000), 0.500000, 0.200000);
        float warp_y = fbm2(p * 5.000000 + vec2(1.7, 0.0), int(4.000000), 0.500000, 0.200000);
        p = p + vec2(warp_x, warp_y) * 0.200000; }
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p + vec2(sin(p.y * 1.500000 + time * 0.300000), cos(p.x * 1.500000 + time * 0.300000)) * (0.015000 + (pulse * 0.010000));
        float sdf_result = sdf_circle(p, (0.070000 + (pulse * 0.020000)));
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p + vec2(sin(p.y * 2.000000 + time * 0.400000), cos(p.x * 2.000000 + time * 0.400000)) * (0.020000 + (pulse * 0.010000));
        float sdf_result = abs(length(p) - (0.090000 + (pulse * 0.020000))) - 0.004000;
        float glow_pulse = (4.000000 + (awareness * 2.500000)) * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.600000, 0.500000, 1.000000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.950000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p + vec2(sin(p.y * 2.000000 + time * 0.500000), cos(p.x * 2.000000 + time * 0.500000)) * 0.030000;
        float sdf_result = sdf_circle(p, (0.160000 + (pulse * 0.050000)));
        for (int onion_i = 0; onion_i < int(3.000000); onion_i++) { sdf_result = abs(sdf_result) - 0.005000; }
        float glow_pulse = (1.500000 + (pulse * 1.000000)) * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.300000, 0.250000, 0.600000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.900000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2((sin((time * 0.400000)) * 0.220000), (cos((time * 0.400000)) * 0.220000));
        p = p + vec2(sin(p.y * 2.000000 + time * 0.600000), cos(p.x * 2.000000 + time * 0.600000)) * 0.020000;
        float sdf_result = sdf_circle(p, (0.020000 + (awareness * 0.010000)));
        float glow_pulse = (2.500000 + (awareness * 1.500000)) * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.500000, 0.400000, 0.900000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.880000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2((cos((time * 0.350000)) * 0.250000), (sin((time * 0.300000)) * 0.200000));
        p = p + vec2(sin(p.y * 2.000000 + time * 0.500000), cos(p.x * 2.000000 + time * 0.500000)) * 0.020000;
        float sdf_result = sdf_circle(p, (0.015000 + (depth * 0.008000)));
        float glow_pulse = (2.000000 + (depth * 1.000000)) * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.400000, 0.500000, 0.800000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.870000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        { float warp_x = fbm2(p * 2.500000 + vec2(0.0, 1.3), int(5.000000), 0.600000, (0.200000 + (awareness * 0.100000)));
        float warp_y = fbm2(p * 2.500000 + vec2(1.7, 0.0), int(5.000000), 0.600000, (0.200000 + (awareness * 0.100000)));
        p = p + vec2(warp_x, warp_y) * (0.200000 + (awareness * 0.100000)); }
    ...
    // ── Layer 10: frame ──
    {
        ...
        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.200000, 0.180000, 0.350000), color_result.a);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
        final_color = vec4(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
...
`;
const UNIFORMS = [{name:'pulse',default:0},{name:'awareness',default:0.3},{name:'depth',default:0.5}];
const COMPLEXITY = {layers:10,fbmOctaves:37,passes:3,memory:true,compute:false,is3d:false,tier:'extreme'};
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'pulse', target: 'boundary', field: 'brightness', weight: 0.5 },
      { source: 'pulse', target: 'heartbeat', field: 'brightness', weight: 0.6 },
      { source: 'awareness', target: 'intel_field', field: 'intensity', weight: 0.4 },
      { source: 'awareness', target: 'node_a', field: 'brightness', weight: 0.4 },
      { source: 'depth', target: 'streams', field: 'intensity', weight: 0.3 },
      { source: 'depth', target: 'emanation', field: 'brightness', weight: 0.3 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class GameArcTimeline {
  constructor() {
    ...
    this._entries = [
      ...
      { target: 'awareness', from: 0.3, to: 1, duration: 10, easing: 'ease_in_out' },
      { target: 'depth', from: 0.5, to: 1, duration: 15, easing: 'ease_in_out' },
    ];
  }
...
class VoidEngine extends HTMLElement {
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('glyph-void-engine: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get awareness() { return this._renderer?.userParams['awareness'] ?? this._pendingParams['awareness'] ?? 0.3; }
  set awareness(v) { this.setParam('awareness', v); }
  get depth() { return this._renderer?.userParams['depth'] ?? this._pendingParams['depth'] ?? 0.5; }
...
customElements.define('glyph-void-engine', VoidEngine);
})();
