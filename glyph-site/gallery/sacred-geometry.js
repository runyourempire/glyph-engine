// GLYPH Component: sacred-geometry — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 1: mandala ──
    {
        ...
        let r_sector = 6.28318 / 12.000000;
        let r_a = glyph_mod(r_angle + r_sector * 0.5, r_sector) - r_sector * 0.5;
        let r_r = length(p);
        p = vec2<f32>(r_r * cos(r_a), r_r * sin(r_a)); }
        p = p + vec2<f32>(sin(p.y * 2.000000 + time * 0.300000), cos(p.x * 2.000000 + time * 0.300000)) * 0.100000;
        { let warp_x = fbm2(p * 1.500000 + vec2<f32>(0.0, 1.3), i32(5.000000), 0.500000, (0.300000 + (breath * 0.080000)));
        let warp_y = fbm2(p * 1.500000 + vec2<f32>(1.7, 0.0), i32(5.000000), 0.500000, (0.300000 + (breath * 0.080000)));
        p = p + vec2<f32>(warp_x, warp_y) * (0.300000 + (breath * 0.080000)); }
        var sdf_result = fbm2((p * 2.500000 + vec2<f32>(time * 0.1, time * 0.07)), i32(5.000000), 0.500000, 2.000000);
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.012000, 0.008000, 0.003000), vec3<f32>(0.065000, 0.045000, 0.018000), vec3<f32>(0.800000, 0.600000, 0.300000), vec3<f32>(0.000000, 0.100000, 0.200000));
    ...
    // ── Layer 2: depth ──
    {
        ...
        let r_r = length(p);
        p = vec2<f32>(r_r * cos(r_a), r_r * sin(r_a)); }
        p = p + vec2<f32>(sin(p.y * 3.000000 + time * 0.400000), cos(p.x * 3.000000 + time * 0.400000)) * 0.060000;
        { let warp_x = fbm2(p * 3.000000 + vec2<f32>(0.0, 1.3), i32(3.000000), 0.150000, 2.000000);
        let warp_y = fbm2(p * 3.000000 + vec2<f32>(1.7, 0.0), i32(3.000000), 0.150000, 2.000000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.150000; }
        var sdf_result = fbm2((p * 5.000000 + vec2<f32>(time * 0.1, time * 0.07)), i32(3.000000), 0.500000, 2.000000);
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.005000, 0.003000, 0.001000), vec3<f32>(0.025000, 0.018000, 0.007000), vec3<f32>(0.600000, 0.800000, 0.400000), vec3<f32>(0.050000, 0.150000, 0.100000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
...
void main(){
    ...
    // ── Layer 1: mandala ──
    {
        ...
        float r_sector = 6.28318 / 12.000000;
        float r_a = mod(r_angle + r_sector * 0.5, r_sector) - r_sector * 0.5;
        float r_r = length(p);
        p = vec2(r_r * cos(r_a), r_r * sin(r_a)); }
        p = p + vec2(sin(p.y * 2.000000 + time * 0.300000), cos(p.x * 2.000000 + time * 0.300000)) * 0.100000;
        { float warp_x = fbm2(p * 1.500000 + vec2(0.0, 1.3), int(5.000000), 0.500000, (0.300000 + (breath * 0.080000)));
        float warp_y = fbm2(p * 1.500000 + vec2(1.7, 0.0), int(5.000000), 0.500000, (0.300000 + (breath * 0.080000)));
        p = p + vec2(warp_x, warp_y) * (0.300000 + (breath * 0.080000)); }
        float sdf_result = fbm2((p * 2.500000 + vec2(time * 0.1, time * 0.07)), int(5.000000), 0.500000, 2.000000);
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.012000, 0.008000, 0.003000), vec3(0.065000, 0.045000, 0.018000), vec3(0.800000, 0.600000, 0.300000), vec3(0.000000, 0.100000, 0.200000));
    ...
    // ── Layer 2: depth ──
    {
        ...
        float r_r = length(p);
        p = vec2(r_r * cos(r_a), r_r * sin(r_a)); }
        p = p + vec2(sin(p.y * 3.000000 + time * 0.400000), cos(p.x * 3.000000 + time * 0.400000)) * 0.060000;
        { float warp_x = fbm2(p * 3.000000 + vec2(0.0, 1.3), int(3.000000), 0.150000, 2.000000);
        float warp_y = fbm2(p * 3.000000 + vec2(1.7, 0.0), int(3.000000), 0.150000, 2.000000);
        p = p + vec2(warp_x, warp_y) * 0.150000; }
        float sdf_result = fbm2((p * 5.000000 + vec2(time * 0.1, time * 0.07)), int(3.000000), 0.500000, 2.000000);
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.005000, 0.003000, 0.001000), vec3(0.025000, 0.018000, 0.007000), vec3(0.600000, 0.800000, 0.400000), vec3(0.050000, 0.150000, 0.100000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.880000);
        float la = color_result.a;
...
`;
const UNIFORMS = [{name:'breath',default:0}];
const COMPLEXITY = {layers:2,fbmOctaves:16,passes:1,memory:true,compute:false,is3d:false,tier:'heavy'};
...
class GameArcTimeline {
  constructor() {
    ...
    this._entries = [
      { target: 'breath', from: 0, to: 1, duration: 8, easing: 'ease_in_out' },
    ];
  }
...
class SacredGeometry extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('glyph-sacred-geometry: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get breath() { return this._renderer?.userParams['breath'] ?? this._pendingParams['breath'] ?? 0; }
  set breath(v) { this.setParam('breath', v); }
...
customElements.define('glyph-sacred-geometry', SacredGeometry);
})();
