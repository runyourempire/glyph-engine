// GLYPH Component: stellar-nursery — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let formation = u.p_formation;
    let radiation = u.p_radiation;
    ...
    // ── Layer 1: emission ──
    {
        ...
        { let warp_x = fbm2(p * 2.000000 + vec2<f32>(0.0, 1.3), i32(6.000000), 0.600000, 2.000000);
        let warp_y = fbm2(p * 2.000000 + vec2<f32>(1.7, 0.0), i32(6.000000), 0.600000, 2.000000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.300000; }
        var sdf_result = fbm2((p * 2.500000 + vec2<f32>(time * 0.1, time * 0.07)), i32(7.000000), 0.550000, 2.000000);
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.050000, 0.010000, 0.020000), vec3<f32>(0.300000, 0.080000, 0.150000), vec3<f32>(0.800000, 0.300000, 0.500000), vec3<f32>(0.000000, 0.150000, 0.300000));
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>(0.100000, 0.050000);
        { let warp_x = fbm2(p * 2.500000 + vec2<f32>(0.0, 1.3), i32(5.000000), 0.550000, 0.250000);
        let warp_y = fbm2(p * 2.500000 + vec2<f32>(1.7, 0.0), i32(5.000000), 0.550000, 0.250000);
        ...
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.010000, 0.020000, 0.060000), vec3<f32>(0.080000, 0.150000, 0.350000), vec3<f32>(0.400000, 0.600000, 1.000000), vec3<f32>(0.100000, 0.200000, 0.500000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let warp_x = fbm2(p * 1.500000 + vec2<f32>(0.0, 1.3), i32(5.000000), 0.650000, 0.350000);
        let warp_y = fbm2(p * 1.500000 + vec2<f32>(1.7, 0.0), i32(5.000000), 0.650000, 0.350000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.350000; }
        var sdf_result = fbm2((p * 2.000000 + vec2<f32>(time * 0.1, time * 0.07)), i32(6.000000), 0.600000, 2.000000);
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.010000, 0.005000, 0.010000), vec3<f32>(0.040000, 0.020000, 0.050000), vec3<f32>(0.100000, 0.060000, 0.120000), vec3<f32>(0.000000, 0.050000, 0.100000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>((-0.080000), 0.050000);
        p = p + vec2<f32>(sin(p.y * 2.000000 + time * 0.400000), cos(p.x * 2.000000 + time * 0.400000)) * (0.020000 + (formation * 0.015000));
        var sdf_result = sdf_circle(p, (0.040000 + (formation * 0.025000)));
        let glow_pulse = (4.000000 + (formation * 3.000000)) * (0.9 + 0.1 * sin(time * 2.0));
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>(0.120000, (-0.100000));
        p = p + vec2<f32>(sin(p.y * 2.000000 + time * 0.500000), cos(p.x * 2.000000 + time * 0.500000)) * 0.020000;
        var sdf_result = sdf_circle(p, (0.030000 + (formation * 0.015000)));
        let glow_pulse = (3.000000 + (formation * 2.000000)) * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.800000, 0.850000, 1.000000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let warp_x = fbm2(p * 3.000000 + vec2<f32>(0.0, 1.3), i32(4.000000), (0.200000 + (radiation * 0.100000)), 2.000000);
        let warp_y = fbm2(p * 3.000000 + vec2<f32>(1.7, 0.0), i32(4.000000), (0.200000 + (radiation * 0.100000)), 2.000000);
        p = p + vec2<f32>(warp_x, warp_y) * (0.200000 + (radiation * 0.100000)); }
        var sdf_result = noise2(p * 5.000000 + vec2<f32>(time * 0.1, time * 0.07));
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.5, 0.3, 0.2), vec3<f32>(0.5, 0.2, 0.3), vec3<f32>(1.0, 1.0, 0.5), vec3<f32>(0.8, 0.9, 0.3));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let warp_x = fbm2(p * 1.000000 + vec2<f32>(0.0, 1.3), i32(2.000000), 0.030000, 2.000000);
        let warp_y = fbm2(p * 1.000000 + vec2<f32>(1.7, 0.0), i32(2.000000), 0.030000, 2.000000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.030000; }
        var sdf_result = voronoi2(p * 28.000000 + vec2<f32>(time * 0.05, time * 0.03));
...
uniform float u_p_formation;
uniform float u_p_radiation;
...
void main(){
    ...
    float formation = u_p_formation;
    float radiation = u_p_radiation;
    ...
    // ── Layer 1: emission ──
    {
        ...
        { float warp_x = fbm2(p * 2.000000 + vec2(0.0, 1.3), int(6.000000), 0.600000, 2.000000);
        float warp_y = fbm2(p * 2.000000 + vec2(1.7, 0.0), int(6.000000), 0.600000, 2.000000);
        p = p + vec2(warp_x, warp_y) * 0.300000; }
        float sdf_result = fbm2((p * 2.500000 + vec2(time * 0.1, time * 0.07)), int(7.000000), 0.550000, 2.000000);
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.050000, 0.010000, 0.020000), vec3(0.300000, 0.080000, 0.150000), vec3(0.800000, 0.300000, 0.500000), vec3(0.000000, 0.150000, 0.300000));
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2(0.100000, 0.050000);
        { float warp_x = fbm2(p * 2.500000 + vec2(0.0, 1.3), int(5.000000), 0.550000, 0.250000);
        float warp_y = fbm2(p * 2.500000 + vec2(1.7, 0.0), int(5.000000), 0.550000, 0.250000);
        p = p + vec2(warp_x, warp_y) * 0.250000; }
        float sdf_result = fbm2((p * 3.000000 + vec2(time * 0.1, time * 0.07)), int(6.000000), 0.500000, 2.000000);
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.010000, 0.020000, 0.060000), vec3(0.080000, 0.150000, 0.350000), vec3(0.400000, 0.600000, 1.000000), vec3(0.100000, 0.200000, 0.500000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.910000);
        float la = color_result.a;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        { float warp_x = fbm2(p * 1.500000 + vec2(0.0, 1.3), int(5.000000), 0.650000, 0.350000);
        float warp_y = fbm2(p * 1.500000 + vec2(1.7, 0.0), int(5.000000), 0.650000, 0.350000);
        p = p + vec2(warp_x, warp_y) * 0.350000; }
        float sdf_result = fbm2((p * 2.000000 + vec2(time * 0.1, time * 0.07)), int(6.000000), 0.600000, 2.000000);
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.010000, 0.005000, 0.010000), vec3(0.040000, 0.020000, 0.050000), vec3(0.100000, 0.060000, 0.120000), vec3(0.000000, 0.050000, 0.100000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.890000);
        float la = color_result.a;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2((-0.080000), 0.050000);
        p = p + vec2(sin(p.y * 2.000000 + time * 0.400000), cos(p.x * 2.000000 + time * 0.400000)) * (0.020000 + (formation * 0.015000));
        float sdf_result = sdf_circle(p, (0.040000 + (formation * 0.025000)));
        float glow_pulse = (4.000000 + (formation * 3.000000)) * (0.9 + 0.1 * sin(time * 2.0));
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2(0.120000, (-0.100000));
        p = p + vec2(sin(p.y * 2.000000 + time * 0.500000), cos(p.x * 2.000000 + time * 0.500000)) * 0.020000;
        float sdf_result = sdf_circle(p, (0.030000 + (formation * 0.015000)));
        float glow_pulse = (3.000000 + (formation * 2.000000)) * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.800000, 0.850000, 1.000000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.920000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        { float warp_x = fbm2(p * 3.000000 + vec2(0.0, 1.3), int(4.000000), (0.200000 + (radiation * 0.100000)), 2.000000);
        float warp_y = fbm2(p * 3.000000 + vec2(1.7, 0.0), int(4.000000), (0.200000 + (radiation * 0.100000)), 2.000000);
        p = p + vec2(warp_x, warp_y) * (0.200000 + (radiation * 0.100000)); }
        float sdf_result = noise2(p * 5.000000 + vec2(time * 0.1, time * 0.07));
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.5, 0.3, 0.2), vec3(0.5, 0.2, 0.3), vec3(1.0, 1.0, 0.5), vec3(0.8, 0.9, 0.3));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.870000);
        float la = color_result.a;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        { float warp_x = fbm2(p * 1.000000 + vec2(0.0, 1.3), int(2.000000), 0.030000, 2.000000);
        float warp_y = fbm2(p * 1.000000 + vec2(1.7, 0.0), int(2.000000), 0.030000, 2.000000);
        p = p + vec2(warp_x, warp_y) * 0.030000; }
...
`;
const UNIFORMS = [{name:'formation',default:0},{name:'radiation',default:0.4}];
const COMPLEXITY = {layers:7,fbmOctaves:41,passes:2,memory:true,compute:false,is3d:false,tier:'extreme'};
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'formation', target: 'proto_a', field: 'brightness', weight: 0.6 },
      { source: 'formation', target: 'proto_b', field: 'brightness', weight: 0.5 },
      { source: 'radiation', target: 'front', field: 'intensity', weight: 0.4 },
      { source: 'radiation', target: 'emission', field: 'brightness', weight: 0.3 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class GameArcTimeline {
  constructor() {
    ...
    this._entries = [
      { target: 'formation', from: 0, to: 1, duration: 12, easing: 'ease_in_out' },
      { target: 'radiation', from: 0.4, to: 1, duration: 15, easing: 'ease_in_out' },
    ];
  }
...
class StellarNursery extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('glyph-stellar-nursery: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get formation() { return this._renderer?.userParams['formation'] ?? this._pendingParams['formation'] ?? 0; }
  set formation(v) { this.setParam('formation', v); }
  get radiation() { return this._renderer?.userParams['radiation'] ?? this._pendingParams['radiation'] ?? 0.4; }
  set radiation(v) { this.setParam('radiation', v); }
...
customElements.define('glyph-stellar-nursery', StellarNursery);
})();
