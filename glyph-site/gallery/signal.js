// GLYPH Component: signal — auto-generated, do not edit.
(function(){
...
`;
const UNIFORMS = [{name:'pulse',default:0.3}];
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'pulse', target: 'center', field: 'brightness', weight: 0.3 },
      { source: 'pulse', target: 'probe', field: 'brightness', weight: 0.2 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class GameArcTimeline {
  constructor() {
    ...
    this._entries = [
      { target: 'pulse', from: 0.3, to: 0.9, duration: 5, easing: 'ease_in_out' },
    ];
  }
...
class Signal extends HTMLElement {
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('glyph-signal: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get pulse() { return this._renderer?.userParams['pulse'] ?? this._pendingParams['pulse'] ?? 0.3; }
...
customElements.define('glyph-signal', Signal);
})();
