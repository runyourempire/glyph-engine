// GLYPH Component: galactic-spiral — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 1: deep_space ──
    {
        ...
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.000000, 0.000000, 0.010000), vec3<f32>(0.050000, 0.050000, 0.080000), vec3<f32>(0.800000, 0.800000, 0.900000), vec3<f32>(0.000000, 0.000000, 0.000000));
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let ra = time * (spin * 0.300000); let rc = cos(ra); let rs = sin(ra);
        p = vec2<f32>(p.x * rc - p.y * rs, p.x * rs + p.y * rc); }
        p = vec2<f32>(length(p), atan2(p.y, p.x));
        { let warp_x = fbm2(p * 2.500000 + vec2<f32>(0.0, 1.3), i32(5.000000), 0.500000, (0.300000 + (spin * 0.100000)));
        let warp_y = fbm2(p * 2.500000 + vec2<f32>(1.7, 0.0), i32(5.000000), 0.500000, (0.300000 + (spin * 0.100000)));
        p = p + vec2<f32>(warp_x, warp_y) * (0.300000 + (spin * 0.100000)); }
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let ra = time * ((spin * 0.300000) + 3.140000); let rc = cos(ra); let rs = sin(ra);
        p = vec2<f32>(p.x * rc - p.y * rs, p.x * rs + p.y * rc); }
        p = vec2<f32>(length(p), atan2(p.y, p.x));
        { let warp_x = fbm2(p * 2.500000 + vec2<f32>(0.0, 1.3), i32(5.000000), 0.500000, 0.280000);
        let warp_y = fbm2(p * 2.500000 + vec2<f32>(1.7, 0.0), i32(5.000000), 0.500000, 0.280000);
        ...
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.020000, 0.010000, 0.050000), vec3<f32>(0.200000, 0.100000, 0.300000), vec3<f32>(0.800000, 0.600000, 1.000000), vec3<f32>(0.100000, 0.200000, 0.500000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let ra = time * (spin * 0.150000); let rc = cos(ra); let rs = sin(ra);
        p = vec2<f32>(p.x * rc - p.y * rs, p.x * rs + p.y * rc); }
        { let warp_x = fbm2(p * 3.000000 + vec2<f32>(0.0, 1.3), i32(6.000000), 0.600000, 0.250000);
        let warp_y = fbm2(p * 3.000000 + vec2<f32>(1.7, 0.0), i32(6.000000), 0.600000, 0.250000);
        ...
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.010000, 0.005000, 0.020000), vec3<f32>(0.080000, 0.040000, 0.120000), vec3<f32>(0.300000, 0.200000, 0.400000), vec3<f32>(0.000000, 0.100000, 0.250000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p + vec2<f32>(sin(p.y * 2.000000 + time * 0.300000), cos(p.x * 2.000000 + time * 0.300000)) * 0.020000;
        var sdf_result = sdf_circle(p, (0.060000 + (luminosity * 0.020000)));
        let glow_pulse = (5.000000 + (luminosity * 3.000000)) * (0.9 + 0.1 * sin(time * 2.0));
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p + vec2<f32>(sin(p.y * 1.500000 + time * 0.200000), cos(p.x * 1.500000 + time * 0.200000)) * 0.015000;
        var sdf_result = sdf_circle(p, 0.150000);
        let glow_pulse = (2.000000 + (luminosity * 1.000000)) * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.800000, 0.600000, 0.400000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let ra = time * (spin * 0.200000); let rc = cos(ra); let rs = sin(ra);
        p = vec2<f32>(p.x * rc - p.y * rs, p.x * rs + p.y * rc); }
        { let warp_x = fbm2(p * 5.000000 + vec2<f32>(0.0, 1.3), i32(3.000000), 0.150000, 2.000000);
        let warp_y = fbm2(p * 5.000000 + vec2<f32>(1.7, 0.0), i32(3.000000), 0.150000, 2.000000);
...
`;
const UNIFORMS = [{name:'spin',default:0.2},{name:'luminosity',default:0.4}];
const COMPLEXITY = {layers:7,fbmOctaves:39,passes:2,memory:true,compute:false,is3d:false,tier:'extreme'};
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'spin', target: 'arm_a', field: 'intensity', weight: 0.3 },
      { source: 'spin', target: 'arm_b', field: 'intensity', weight: 0.3 },
      { source: 'luminosity', target: 'core', field: 'brightness', weight: 0.5 },
      { source: 'luminosity', target: 'core_halo', field: 'brightness', weight: 0.4 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class GameArcTimeline {
  constructor() {
    ...
    this._entries = [
      { target: 'spin', from: 0.2, to: 1, duration: 20, easing: 'ease_in_out' },
      { target: 'luminosity', from: 0.4, to: 1, duration: 15, easing: 'ease_in_out' },
    ];
  }
...
class GalacticSpiral extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('glyph-galactic-spiral: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get spin() { return this._renderer?.userParams['spin'] ?? this._pendingParams['spin'] ?? 0.2; }
  set spin(v) { this.setParam('spin', v); }
  get luminosity() { return this._renderer?.userParams['luminosity'] ?? this._pendingParams['luminosity'] ?? 0.4; }
  set luminosity(v) { this.setParam('luminosity', v); }
...
customElements.define('glyph-galactic-spiral', GalacticSpiral);
})();
