// GLYPH Component: labyrinth — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.008000, 0.008000, 0.012000), color_result.a);
...
void main(){
    ...
    color_result = vec4(color_result.rgb * vec3(0.008000, 0.008000, 0.012000), color_result.a);
...
`;
const UNIFORMS = [{name:'color_r',default:1.2},{name:'color_g',default:1.2},{name:'color_b',default:1.4}];
...
class GameReactionField {
  ...
  _seed() {
    ...
    for (let i = 0; i < cellCount; i++) {
      if (Math.random() < 0.04) init[i * 2 + 1] = 1.0;
    }
  ...
  dispatch(steps = 8) {
    ...
    f[0] = 0.029 + Math.sin(t * 0.7) * 0.001 - dissolve * 0.03;
    f[1] = 0.057 + Math.cos(t * 0.5) * 0.0005 + dissolve * 0.02;
    f[2] = 1; f[3] = 0.5;
...
class Labyrinth extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('glyph-labyrinth: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get color_r() { return this._renderer?.userParams['color_r'] ?? this._pendingParams['color_r'] ?? 1.2; }
...
customElements.define('glyph-labyrinth', Labyrinth);
})();
