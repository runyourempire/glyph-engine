// GLYPH Component: nexus — auto-generated, do not edit.
(function(){
...
void main(){
    ...
    // ── Layer 1: void ──
    {
        ...
        { float warp_x = fbm2(p * 1.200000 + vec2(0.0, 1.3), int(4.000000), 0.400000, 0.080000);
        float warp_y = fbm2(p * 1.200000 + vec2(1.7, 0.0), int(4.000000), 0.400000, 0.080000);
        p = p + vec2(warp_x, warp_y) * 0.080000; }
        float sdf_result = fbm2((p * 1.500000 + vec2(time * 0.1, time * 0.07)), int(4.000000), 0.400000, 2.000000);
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.010000, 0.010000, 0.020000), vec3(0.020000, 0.010000, 0.040000), vec3(0.060000, 0.030000, 0.100000), vec3(0.000000, 0.000000, 0.050000));
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p + vec2(sin(p.y * 2.500000 + time * 0.250000), cos(p.x * 2.500000 + time * 0.250000)) * (0.080000 + (u_mouse_down * 0.060000));
        { float warp_x = fbm2(p * (2.000000 + (u_mouse.x * 0.500000)) + vec2(0.0, 1.3), int(5.000000), 0.500000, (0.200000 + (activity * 0.080000)));
        float warp_y = fbm2(p * (2.000000 + (u_mouse.x * 0.500000)) + vec2(1.7, 0.0), int(5.000000), 0.500000, (0.200000 + (activity * 0.080000)));
        p = p + vec2(warp_x, warp_y) * (0.200000 + (activity * 0.080000)); }
        float sdf_result = voronoi2(p * 7.000000 + vec2(time * 0.05, time * 0.03));
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.020000, 0.040000, 0.080000), vec3(0.080000, 0.120000, 0.250000), vec3(1.500000, 0.800000, 0.500000), vec3(0.550000, 0.650000, 0.750000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        { float warp_x = fbm2(p * (3.000000 + (u_mouse.y * 0.300000)) + vec2(0.0, 1.3), int(4.000000), 0.450000, (0.150000 + (activity * 0.050000)));
        float warp_y = fbm2(p * (3.000000 + (u_mouse.y * 0.300000)) + vec2(1.7, 0.0), int(4.000000), 0.450000, (0.150000 + (activity * 0.050000)));
        p = p + vec2(warp_x, warp_y) * (0.150000 + (activity * 0.050000)); }
        p = p + vec2(sin(p.y * 3.000000 + time * 0.350000), cos(p.x * 3.000000 + time * 0.350000)) * 0.060000;
        float sdf_result = voronoi2(p * 15.000000 + vec2(time * 0.05, time * 0.03));
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.000000, 0.020000, 0.060000), vec3(0.050000, 0.100000, 0.200000), vec3(2.000000, 1.000000, 0.500000), vec3(0.150000, 0.350000, 0.600000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.780000);
    ...
    // ── Layer 4: signals ──
    {
        ...
        { float warp_x = fbm2(p * 3.000000 + vec2(0.0, 1.3), int(4.000000), 0.400000, (0.120000 + (coherence * 0.060000)));
        float warp_y = fbm2(p * 3.000000 + vec2(1.7, 0.0), int(4.000000), 0.400000, (0.120000 + (coherence * 0.060000)));
        p = p + vec2(warp_x, warp_y) * (0.120000 + (coherence * 0.060000)); }
        float sdf_result = noise2(p * 8.000000 + vec2(time * 0.1, time * 0.07));
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.000000, 0.020000, 0.050000), vec3(0.100000, 0.150000, 0.300000), vec3(1.000000, 0.800000, 0.500000), vec3(0.000000, 0.200000, 0.500000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.760000);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        { float warp_x = fbm2(p * 2.500000 + vec2(0.0, 1.3), int(6.000000), 0.550000, 0.200000);
        float warp_y = fbm2(p * 2.500000 + vec2(1.7, 0.0), int(6.000000), 0.550000, 0.200000);
        p = p + vec2(warp_x, warp_y) * 0.200000; }
        float sdf_result = fbm2((p * 3.000000 + vec2(time * 0.1, time * 0.07)), int(6.000000), 0.500000, 2.000000);
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.010000, 0.020000, 0.050000), vec3(0.060000, 0.080000, 0.180000), vec3(0.400000, 0.250000, 0.600000), vec3(0.000000, 0.100000, 0.250000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.820000);
    ...
    // ── Layer 6: hub ──
    {
        ...
        p = p + vec2(sin(p.y * 2.000000 + time * 0.350000), cos(p.x * 2.000000 + time * 0.350000)) * (0.015000 + (u_mouse_down * 0.010000));
        float sdf_result = abs(length(p) - (0.120000 + (coherence * 0.030000))) - 0.005000;
        float glow_pulse = (1.200000 + (coherence * 0.500000)) * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.400000, 0.600000, 1.000000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.800000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    // ── Layer 7: core ──
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2(((u_mouse.x * 0.100000) - 0.050000), ((u_mouse.y * 0.100000) - 0.050000));
        float sdf_result = sdf_circle(p, ((0.020000 + (u_mouse_down * 0.015000)) + (activity * 0.010000)));
        float glow_pulse = (1.500000 + (activity * 0.800000)) * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2((sin((time * 0.300000)) * 0.280000), (cos((time * 0.300000)) * 0.240000));
        float sdf_result = sdf_circle(p, (0.008000 + (coherence * 0.004000)));
        float glow_pulse = (1.000000 + (coherence * 0.400000)) * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2((cos((time * 0.220000)) * 0.320000), (sin((time * 0.180000)) * 0.270000));
        float sdf_result = sdf_circle(p, (0.006000 + (coherence * 0.003000)));
        float glow_pulse = (0.800000 + (coherence * 0.300000)) * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(1.000000, 0.500000, 0.800000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.680000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        float sdf_result = smoothstep(0.250000, 0.550000, length(p));
...
`;
const UNIFORMS = [{name:'activity',default:0},{name:'coherence',default:0}];
const COMPLEXITY = {layers:10,fbmOctaves:33,passes:3,memory:true,compute:false,is3d:false,tier:'extreme'};
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'activity', target: 'network', field: 'intensity', weight: 0.3 },
      { source: 'activity', target: 'lattice', field: 'brightness', weight: 0.25 },
      { source: 'coherence', target: 'signals', field: 'intensity', weight: 0.3 },
      { source: 'coherence', target: 'hub', field: 'brightness', weight: 0.35 },
      { source: 'coherence', target: 'core', field: 'brightness', weight: 0.3 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class GameArcTimeline {
  constructor() {
    ...
    this._entries = [
      { target: 'activity', from: 0, to: 1, duration: 10, easing: 'ease_in_out' },
      { target: 'coherence', from: 0, to: 1, duration: 16, easing: 'ease_out' },
    ];
  }
...
class Nexus extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('glyph-nexus: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get activity() { return this._renderer?.userParams['activity'] ?? this._pendingParams['activity'] ?? 0; }
  set activity(v) { this.setParam('activity', v); }
...
customElements.define('glyph-nexus', Nexus);
})();
