// GLYPH Component: silk-void — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let weave_speed = u.p_weave_speed;
    ...
    // ── Layer 1: silk ──
    {
        ...
        { let warp_x = fbm2(p * 1.300000 + vec2<f32>(0.0, 1.3), i32(6.000000), 0.550000, (0.330000 + (u.mouse_down * 0.080000)));
        let warp_y = fbm2(p * 1.300000 + vec2<f32>(1.7, 0.0), i32(6.000000), 0.550000, (0.330000 + (u.mouse_down * 0.080000)));
        p = p + vec2<f32>(warp_x, warp_y) * (0.330000 + (u.mouse_down * 0.080000)); }
        p = p + vec2<f32>(sin(p.y * 2.000000 + time * weave_speed), cos(p.x * 2.000000 + time * weave_speed)) * 0.100000;
        var sdf_result = voronoi2(p * 7.000000 + vec2<f32>(time * 0.05, time * 0.03));
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.350000, 0.300000, 0.450000), vec3<f32>(0.250000, 0.200000, 0.300000), vec3<f32>(1.000000, 0.800000, 0.500000), vec3<f32>(0.600000, 0.650000, 0.750000));
    ...
    // ── Layer 2: threads ──
    {
        ...
        p = p + vec2<f32>(sin(p.y * 3.000000 + time * (weave_speed * 1.300000)), cos(p.x * 3.000000 + time * (weave_speed * 1.300000))) * 0.080000;
        var sdf_result = voronoi2(p * 14.000000 + vec2<f32>(time * 0.05, time * 0.03));
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.300000, 0.280000, 0.420000), vec3<f32>(0.220000, 0.180000, 0.280000), vec3<f32>(1.500000, 1.000000, 0.600000), vec3<f32>(0.550000, 0.600000, 0.720000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let warp_x = fbm2(p * (3.000000 + (u.mouse.y * 0.300000)) + vec2<f32>(0.0, 1.3), i32(5.000000), 0.500000, 0.220000);
        let warp_y = fbm2(p * (3.000000 + (u.mouse.y * 0.300000)) + vec2<f32>(1.7, 0.0), i32(5.000000), 0.500000, 0.220000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.220000; }
        p = p + vec2<f32>(sin(p.y * 4.000000 + time * (weave_speed * 1.500000)), cos(p.x * 4.000000 + time * (weave_speed * 1.500000))) * 0.060000;
        var sdf_result = fbm2((p * 5.000000 + vec2<f32>(time * 0.1, time * 0.07)), i32(4.000000), 0.450000, 2.000000);
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.280000, 0.250000, 0.400000), vec3<f32>(0.200000, 0.160000, 0.260000), vec3<f32>(0.600000, 0.500000, 0.800000), vec3<f32>(0.500000, 0.550000, 0.650000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p + vec2<f32>(sin(p.y * 5.000000 + time * (weave_speed * 0.600000)), cos(p.x * 5.000000 + time * (weave_speed * 0.600000))) * 0.180000;
        ...
        var sdf_result = sdf_circle(p, (0.008000 + (shimmer * 0.004000)));
        let glow_pulse = (2.500000 + (shimmer * 1.000000)) * (0.9 + 0.1 * sin(time * 2.0));
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p + vec2<f32>(sin(p.y * 3.000000 + time * (weave_speed * 0.400000)), cos(p.x * 3.000000 + time * (weave_speed * 0.400000))) * 0.100000;
        ...
        let glow_pulse = (1.000000 + (shimmer * 0.400000)) * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.400000, 0.350000, 0.600000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p + vec2<f32>(sin(p.y * 7.000000 + time * (weave_speed * 1.000000)), cos(p.x * 7.000000 + time * (weave_speed * 1.000000))) * 0.250000;
        { let warp_x = fbm2(p * 3.000000 + vec2<f32>(0.0, 1.3), i32(3.000000), 0.320000, 2.000000);
        let warp_y = fbm2(p * 3.000000 + vec2<f32>(1.7, 0.0), i32(3.000000), 0.320000, 2.000000);
        ...
        let glow_pulse = (3.500000 + (shimmer * 1.000000)) * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
    ...
    // ── Layer 7: ambient ──
    {
        ...
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.300000, 0.280000, 0.400000), vec3<f32>(0.180000, 0.150000, 0.250000), vec3<f32>(0.500000, 0.400000, 0.600000), vec3<f32>(0.000000, 0.050000, 0.100000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let la = color_result.a;
        let lc = color_result.rgb;
        final_color = vec4<f32>(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
...
`;
const UNIFORMS = [{name:'shimmer',default:0},{name:'weave_speed',default:0.25}];
const COMPLEXITY = {layers:7,fbmOctaves:32,passes:2,memory:true,compute:false,is3d:false,tier:'extreme'};
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'shimmer', target: 'stardust', field: 'brightness', weight: 0.3 },
      { source: 'shimmer', target: 'flickers', field: 'brightness', weight: 0.25 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class GameArcTimeline {
  constructor() {
    ...
    this._entries = [
      { target: 'shimmer', from: 0, to: 0.8, duration: 12, easing: 'ease_in_out' },
    ];
  }
...
class SilkVoid extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('glyph-silk-void: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get weave_speed() { return this._renderer?.userParams['weave_speed'] ?? this._pendingParams['weave_speed'] ?? 0.25; }
  set weave_speed(v) { this.setParam('weave_speed', v); }
...
customElements.define('glyph-silk-void', SilkVoid);
})();
