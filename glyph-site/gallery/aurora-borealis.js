// GLYPH Component: aurora-borealis — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 1: green ──
    {
        ...
        p = p - vec2<f32>(0.000000, 0.100000);
        { let warp_x = fbm2(p * 1.500000 + vec2<f32>(0.0, 1.3), i32(5.000000), 0.600000, (0.300000 + (activity * 0.120000)));
        let warp_y = fbm2(p * 1.500000 + vec2<f32>(1.7, 0.0), i32(5.000000), 0.600000, (0.300000 + (activity * 0.120000)));
        p = p + vec2<f32>(warp_x, warp_y) * (0.300000 + (activity * 0.120000)); }
        var sdf_result = fbm2((p * 2.000000 + vec2<f32>(time * 0.1, time * 0.07)), i32(5.000000), 0.500000, 2.000000);
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.001000, 0.005000, 0.002000), vec3<f32>(0.005000, 0.150000, 0.020000), vec3<f32>(0.500000, 1.000000, 0.300000), vec3<f32>(0.000000, 0.330000, 0.100000));
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>(0.060000, (-0.080000));
        { let warp_x = fbm2(p * 2.000000 + vec2<f32>(0.0, 1.3), i32(4.000000), 0.550000, (0.250000 + (curtain * 0.100000)));
        let warp_y = fbm2(p * 2.000000 + vec2<f32>(1.7, 0.0), i32(4.000000), 0.550000, (0.250000 + (curtain * 0.100000)));
        p = p + vec2<f32>(warp_x, warp_y) * (0.250000 + (curtain * 0.100000)); }
        var sdf_result = fbm2((p * 2.500000 + vec2<f32>(time * 0.1, time * 0.07)), i32(5.000000), 0.500000, 2.000000);
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.002000, 0.001000, 0.004000), vec3<f32>(0.015000, 0.030000, 0.120000), vec3<f32>(0.300000, 0.500000, 1.000000), vec3<f32>(0.150000, 0.250000, 0.500000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    // ── Layer 3: peak ──
    {
        ...
        { let warp_x = fbm2(p * 2.500000 + vec2<f32>(0.0, 1.3), i32(4.000000), (0.200000 + (activity * 0.080000)), 2.000000);
        let warp_y = fbm2(p * 2.500000 + vec2<f32>(1.7, 0.0), i32(4.000000), (0.200000 + (activity * 0.080000)), 2.000000);
        p = p + vec2<f32>(warp_x, warp_y) * (0.200000 + (activity * 0.080000)); }
        var sdf_result = noise2(p * 3.000000 + vec2<f32>(time * 0.1, time * 0.07));
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.002000, 0.008000, 0.002000), vec3<f32>(0.015000, 0.180000, 0.020000), vec3<f32>(0.800000, 1.000000, 0.600000), vec3<f32>(0.000000, 0.330000, 0.000000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    // ── Layer 4: horizon ──
    {
        ...
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.003000, 0.004000, 0.006000), vec3<f32>(0.004000, 0.006000, 0.010000), vec3<f32>(0.500000, 0.500000, 0.500000), vec3<f32>(0.000000, 0.000000, 0.000000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
...
void main(){
    ...
    // ── Layer 1: green ──
    {
        ...
        p = p - vec2(0.000000, 0.100000);
        { float warp_x = fbm2(p * 1.500000 + vec2(0.0, 1.3), int(5.000000), 0.600000, (0.300000 + (activity * 0.120000)));
        float warp_y = fbm2(p * 1.500000 + vec2(1.7, 0.0), int(5.000000), 0.600000, (0.300000 + (activity * 0.120000)));
        p = p + vec2(warp_x, warp_y) * (0.300000 + (activity * 0.120000)); }
        float sdf_result = fbm2((p * 2.000000 + vec2(time * 0.1, time * 0.07)), int(5.000000), 0.500000, 2.000000);
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.001000, 0.005000, 0.002000), vec3(0.005000, 0.150000, 0.020000), vec3(0.500000, 1.000000, 0.300000), vec3(0.000000, 0.330000, 0.100000));
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2(0.060000, (-0.080000));
        { float warp_x = fbm2(p * 2.000000 + vec2(0.0, 1.3), int(4.000000), 0.550000, (0.250000 + (curtain * 0.100000)));
        float warp_y = fbm2(p * 2.000000 + vec2(1.7, 0.0), int(4.000000), 0.550000, (0.250000 + (curtain * 0.100000)));
        p = p + vec2(warp_x, warp_y) * (0.250000 + (curtain * 0.100000)); }
        float sdf_result = fbm2((p * 2.500000 + vec2(time * 0.1, time * 0.07)), int(5.000000), 0.500000, 2.000000);
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.002000, 0.001000, 0.004000), vec3(0.015000, 0.030000, 0.120000), vec3(0.300000, 0.500000, 1.000000), vec3(0.150000, 0.250000, 0.500000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.880000);
        float la = color_result.a;
    ...
    // ── Layer 3: peak ──
    {
        ...
        { float warp_x = fbm2(p * 2.500000 + vec2(0.0, 1.3), int(4.000000), (0.200000 + (activity * 0.080000)), 2.000000);
        float warp_y = fbm2(p * 2.500000 + vec2(1.7, 0.0), int(4.000000), (0.200000 + (activity * 0.080000)), 2.000000);
        p = p + vec2(warp_x, warp_y) * (0.200000 + (activity * 0.080000)); }
        float sdf_result = noise2(p * 3.000000 + vec2(time * 0.1, time * 0.07));
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.002000, 0.008000, 0.002000), vec3(0.015000, 0.180000, 0.020000), vec3(0.800000, 1.000000, 0.600000), vec3(0.000000, 0.330000, 0.000000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.840000);
        float la = color_result.a;
    ...
    // ── Layer 4: horizon ──
    {
        ...
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.003000, 0.004000, 0.006000), vec3(0.004000, 0.006000, 0.010000), vec3(0.500000, 0.500000, 0.500000), vec3(0.000000, 0.000000, 0.000000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.800000);
        float la = color_result.a;
...
`;
const UNIFORMS = [{name:'activity',default:0.5},{name:'curtain',default:0}];
const COMPLEXITY = {layers:4,fbmOctaves:26,passes:1,memory:true,compute:false,is3d:false,tier:'extreme'};
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'activity', target: 'green', field: 'brightness', weight: 0.3 },
      { source: 'curtain', target: 'violet', field: 'intensity', weight: 0.2 },
      { source: 'curtain', target: 'peak', field: 'brightness', weight: 0.4 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class GameArcTimeline {
  constructor() {
    ...
    this._entries = [
      { target: 'curtain', from: 0, to: 1, duration: 20, easing: 'ease_in_out' },
      { target: 'activity', from: 0.5, to: 0.9, duration: 15, easing: 'ease_in_out' },
    ];
  }
...
class AuroraBorealis extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('glyph-aurora-borealis: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get activity() { return this._renderer?.userParams['activity'] ?? this._pendingParams['activity'] ?? 0.5; }
  set activity(v) { this.setParam('activity', v); }
  get curtain() { return this._renderer?.userParams['curtain'] ?? this._pendingParams['curtain'] ?? 0; }
  set curtain(v) { this.setParam('curtain', v); }
...
customElements.define('glyph-aurora-borealis', AuroraBorealis);
})();
