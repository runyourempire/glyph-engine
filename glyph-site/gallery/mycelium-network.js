// GLYPH Component: mycelium-network — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let growth = u.p_growth;
    let nutrient = u.p_nutrient;
    ...
    // ── Layer 1: soil ──
    {
        ...
        { let warp_x = fbm2(p * 2.000000 + vec2<f32>(0.0, 1.3), i32(5.000000), 0.500000, 0.120000);
        let warp_y = fbm2(p * 2.000000 + vec2<f32>(1.7, 0.0), i32(5.000000), 0.500000, 0.120000);
        ...
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.002000, 0.001500, 0.001000), vec3<f32>(0.004000, 0.003000, 0.002000), vec3<f32>(0.600000, 0.400000, 0.250000), vec3<f32>(0.000000, 0.100000, 0.150000));
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let warp_x = fbm2(p * 3.000000 + vec2<f32>(0.0, 1.3), i32(6.000000), 0.650000, (0.250000 + (growth * 0.100000)));
        let warp_y = fbm2(p * 3.000000 + vec2<f32>(1.7, 0.0), i32(6.000000), 0.650000, (0.250000 + (growth * 0.100000)));
        p = p + vec2<f32>(warp_x, warp_y) * (0.250000 + (growth * 0.100000)); }
        var sdf_result = fbm2((p * 6.000000 + vec2<f32>(time * 0.1, time * 0.07)), i32(6.000000), 0.500000, 2.000000);
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.001000, 0.003000, 0.000000), vec3<f32>(0.004000, 0.012000, 0.002000), vec3<f32>(0.300000, 0.800000, 0.200000), vec3<f32>(0.000000, 0.330000, 0.050000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let ra = time * 0.300000; let rc = cos(ra); let rs = sin(ra);
        p = vec2<f32>(p.x * rc - p.y * rs, p.x * rs + p.y * rc); }
        { let warp_x = fbm2(p * 4.000000 + vec2<f32>(0.0, 1.3), i32(5.000000), 0.600000, (0.200000 + (growth * 0.060000)));
        let warp_y = fbm2(p * 4.000000 + vec2<f32>(1.7, 0.0), i32(5.000000), 0.600000, (0.200000 + (growth * 0.060000)));
        p = p + vec2<f32>(warp_x, warp_y) * (0.200000 + (growth * 0.060000)); }
        var sdf_result = fbm2((p * 8.000000 + vec2<f32>(time * 0.1, time * 0.07)), i32(5.000000), 0.500000, 2.000000);
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.000000, 0.002000, 0.001000), vec3<f32>(0.002000, 0.008000, 0.003000), vec3<f32>(0.200000, 0.600000, 0.150000), vec3<f32>(0.000000, 0.250000, 0.050000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let warp_x = fbm2(p * 3.000000 + vec2<f32>(0.0, 1.3), i32(3.000000), 0.080000, 2.000000);
        let warp_y = fbm2(p * 3.000000 + vec2<f32>(1.7, 0.0), i32(3.000000), 0.080000, 2.000000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.080000; }
        var sdf_result = voronoi2(p * 8.000000 + vec2<f32>(time * 0.05, time * 0.03));
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.001000, 0.003000, 0.000000), vec3<f32>(0.005000, 0.015000, 0.003000), vec3<f32>(0.200000, 0.500000, 0.120000), vec3<f32>(0.000000, 0.250000, 0.050000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p + vec2<f32>(sin(p.y * 1.500000 + time * 0.300000), cos(p.x * 1.500000 + time * 0.300000)) * (0.015000 + (growth * 0.010000));
        var sdf_result = sdf_circle(p, (0.050000 + (growth * 0.020000)));
        let shade_fw = fwidth(sdf_result);
        let shade_alpha = 1.0 - smoothstep(-shade_fw, shade_fw, sdf_result);
        var color_result = vec4<f32>(vec3<f32>(0.300000, 0.800000, 0.200000) * shade_alpha, shade_alpha);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p + vec2<f32>(sin(p.y * 2.500000 + time * 0.400000), cos(p.x * 2.500000 + time * 0.400000)) * 0.030000;
        var sdf_result = abs(length(p) - (0.120000 + (growth * 0.030000))) - 0.005000;
        let shade_fw = fwidth(sdf_result);
        let shade_alpha = 1.0 - smoothstep(-shade_fw, shade_fw, sdf_result);
        var color_result = vec4<f32>(vec3<f32>(0.150000, 0.500000, 0.100000) * shade_alpha, shade_alpha);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        var sdf_result = abs(length(p) - (0.200000 + (growth * 0.040000))) - 0.003000;
        let shade_fw = fwidth(sdf_result);
        let shade_alpha = 1.0 - smoothstep(-shade_fw, shade_fw, sdf_result);
        var color_result = vec4<f32>(vec3<f32>(0.100000, 0.350000, 0.080000) * shade_alpha, shade_alpha);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
...
uniform float u_p_growth;
uniform float u_p_nutrient;
...
void main(){
    ...
    float growth = u_p_growth;
    float nutrient = u_p_nutrient;
    ...
    // ── Layer 1: soil ──
    {
        ...
        { float warp_x = fbm2(p * 2.000000 + vec2(0.0, 1.3), int(5.000000), 0.500000, 0.120000);
        float warp_y = fbm2(p * 2.000000 + vec2(1.7, 0.0), int(5.000000), 0.500000, 0.120000);
        p = p + vec2(warp_x, warp_y) * 0.120000; }
        float sdf_result = fbm2((p * 3.000000 + vec2(time * 0.1, time * 0.07)), int(5.000000), 0.500000, 2.000000);
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.002000, 0.001500, 0.001000), vec3(0.004000, 0.003000, 0.002000), vec3(0.600000, 0.400000, 0.250000), vec3(0.000000, 0.100000, 0.150000));
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        { float warp_x = fbm2(p * 3.000000 + vec2(0.0, 1.3), int(6.000000), 0.650000, (0.250000 + (growth * 0.100000)));
        float warp_y = fbm2(p * 3.000000 + vec2(1.7, 0.0), int(6.000000), 0.650000, (0.250000 + (growth * 0.100000)));
        p = p + vec2(warp_x, warp_y) * (0.250000 + (growth * 0.100000)); }
        float sdf_result = fbm2((p * 6.000000 + vec2(time * 0.1, time * 0.07)), int(6.000000), 0.500000, 2.000000);
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.001000, 0.003000, 0.000000), vec3(0.004000, 0.012000, 0.002000), vec3(0.300000, 0.800000, 0.200000), vec3(0.000000, 0.330000, 0.050000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.920000);
        float la = color_result.a;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        { float ra = time * 0.300000; float rc = cos(ra); float rs = sin(ra);
        p = vec2(p.x * rc - p.y * rs, p.x * rs + p.y * rc); }
        { float warp_x = fbm2(p * 4.000000 + vec2(0.0, 1.3), int(5.000000), 0.600000, (0.200000 + (growth * 0.060000)));
        float warp_y = fbm2(p * 4.000000 + vec2(1.7, 0.0), int(5.000000), 0.600000, (0.200000 + (growth * 0.060000)));
        p = p + vec2(warp_x, warp_y) * (0.200000 + (growth * 0.060000)); }
        float sdf_result = fbm2((p * 8.000000 + vec2(time * 0.1, time * 0.07)), int(5.000000), 0.500000, 2.000000);
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.000000, 0.002000, 0.001000), vec3(0.002000, 0.008000, 0.003000), vec3(0.200000, 0.600000, 0.150000), vec3(0.000000, 0.250000, 0.050000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.890000);
        float la = color_result.a;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        { float warp_x = fbm2(p * 3.000000 + vec2(0.0, 1.3), int(3.000000), 0.080000, 2.000000);
        float warp_y = fbm2(p * 3.000000 + vec2(1.7, 0.0), int(3.000000), 0.080000, 2.000000);
        p = p + vec2(warp_x, warp_y) * 0.080000; }
        float sdf_result = voronoi2(p * 8.000000 + vec2(time * 0.05, time * 0.03));
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.001000, 0.003000, 0.000000), vec3(0.005000, 0.015000, 0.003000), vec3(0.200000, 0.500000, 0.120000), vec3(0.000000, 0.250000, 0.050000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.860000);
        float la = color_result.a;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p + vec2(sin(p.y * 1.500000 + time * 0.300000), cos(p.x * 1.500000 + time * 0.300000)) * (0.015000 + (growth * 0.010000));
        float sdf_result = sdf_circle(p, (0.050000 + (growth * 0.020000)));
        float shade_fw = fwidth(sdf_result);
        float shade_alpha = 1.0 - smoothstep(-shade_fw, shade_fw, sdf_result);
        vec4 color_result = vec4(vec3(0.300000, 0.800000, 0.200000) * shade_alpha, shade_alpha);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.910000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p + vec2(sin(p.y * 2.500000 + time * 0.400000), cos(p.x * 2.500000 + time * 0.400000)) * 0.030000;
        float sdf_result = abs(length(p) - (0.120000 + (growth * 0.030000))) - 0.005000;
        float shade_fw = fwidth(sdf_result);
        float shade_alpha = 1.0 - smoothstep(-shade_fw, shade_fw, sdf_result);
        vec4 color_result = vec4(vec3(0.150000, 0.500000, 0.100000) * shade_alpha, shade_alpha);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.840000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        float sdf_result = abs(length(p) - (0.200000 + (growth * 0.040000))) - 0.003000;
        float shade_fw = fwidth(sdf_result);
        float shade_alpha = 1.0 - smoothstep(-shade_fw, shade_fw, sdf_result);
        vec4 color_result = vec4(vec3(0.100000, 0.350000, 0.080000) * shade_alpha, shade_alpha);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.800000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
...
`;
const UNIFORMS = [{name:'growth',default:0},{name:'nutrient',default:0.3}];
const COMPLEXITY = {layers:7,fbmOctaves:35,passes:1,memory:true,compute:false,is3d:false,tier:'extreme'};
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'growth', target: 'hyphae', field: 'intensity', weight: 0.3 },
      { source: 'growth', target: 'fruit', field: 'brightness', weight: 0.4 },
      { source: 'nutrient', target: 'nodes', field: 'brightness', weight: 0.3 },
      { source: 'nutrient', target: 'spores', field: 'brightness', weight: 0.2 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class GameArcTimeline {
  constructor() {
    ...
    this._entries = [
      { target: 'growth', from: 0, to: 1, duration: 12, easing: 'ease_in_out' },
      { target: 'nutrient', from: 0.3, to: 0.9, duration: 15, easing: 'ease_in_out' },
    ];
  }
...
class MyceliumNetwork extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('glyph-mycelium-network: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get growth() { return this._renderer?.userParams['growth'] ?? this._pendingParams['growth'] ?? 0; }
  set growth(v) { this.setParam('growth', v); }
  get nutrient() { return this._renderer?.userParams['nutrient'] ?? this._pendingParams['nutrient'] ?? 0.3; }
  set nutrient(v) { this.setParam('nutrient', v); }
...
customElements.define('glyph-mycelium-network', MyceliumNetwork);
})();
