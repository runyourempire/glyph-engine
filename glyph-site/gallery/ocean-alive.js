// GLYPH Component: ocean-alive — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 1: sea ──
    {
        ...
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.300000, 0.450000, 0.500000), vec3<f32>(0.250000, 0.300000, 0.300000), vec3<f32>(0.800000, 0.600000, 0.300000), vec3<f32>(0.100000, 0.150000, 0.200000));
    ...
    // ── Layer 3: currents ──
    {
        ...
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.200000, 0.500000, 0.700000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    // ── Layer 4: foam ──
    {
        ...
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.500000, 0.800000, 1.000000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    // ── Layer 5: depth ──
    {
        ...
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.250000, 0.350000, 0.450000), vec3<f32>(0.200000, 0.250000, 0.300000), vec3<f32>(0.800000, 0.500000, 0.200000), vec3<f32>(0.000000, 0.100000, 0.150000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let la = color_result.a;
        let lc = color_result.rgb;
        final_color = vec4<f32>(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    ...
    // ── Layer 6: biolum ──
    {
        ...
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.300000, 0.900000, 1.000000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
...
void main(){
    ...
    // ── Layer 1: sea ──
    {
        ...
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.300000, 0.450000, 0.500000), vec3(0.250000, 0.300000, 0.300000), vec3(0.800000, 0.600000, 0.300000), vec3(0.100000, 0.150000, 0.200000));
    ...
    // ── Layer 2: plankton ──
    {
        ...
        color_result = vec4(color_result.rgb * vec3(0.300000, 0.800000, 1.000000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.870000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    // ── Layer 3: currents ──
    {
        ...
        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.200000, 0.500000, 0.700000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.960000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    // ── Layer 4: foam ──
    {
        ...
        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.500000, 0.800000, 1.000000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.800000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    // ── Layer 5: depth ──
    {
        ...
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.250000, 0.350000, 0.450000), vec3(0.200000, 0.250000, 0.300000), vec3(0.800000, 0.500000, 0.200000), vec3(0.000000, 0.100000, 0.150000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        float la = color_result.a;
        vec3 lc = color_result.rgb;
        final_color = vec4(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    ...
    // ── Layer 6: biolum ──
    {
        ...
        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.300000, 0.900000, 1.000000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.720000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'life', target: 'plankton', field: 'brightness', weight: 0.6 },
      { source: 'life', target: 'currents', field: 'intensity', weight: 0.4 },
      { source: 'life', target: 'biolum', field: 'brightness', weight: 0.5 },
      { source: 'warmth', target: 'sea', field: 'brightness', weight: 0.3 },
      { source: 'warmth', target: 'depth', field: 'brightness', weight: 0.5 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class OceanAlive extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('glyph-ocean-alive: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
...
customElements.define('glyph-ocean-alive', OceanAlive);
})();
