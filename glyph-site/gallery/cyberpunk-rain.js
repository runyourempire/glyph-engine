// GLYPH Component: cyberpunk-rain — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 1: city ──
    {
        ...
        { let warp_x = fbm2(p * 1.500000 + vec2<f32>(0.0, 1.3), i32(3.000000), 0.060000, 2.000000);
        let warp_y = fbm2(p * 1.500000 + vec2<f32>(1.7, 0.0), i32(3.000000), 0.060000, 2.000000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.060000; }
        let gx = abs(glyph_mod(p.x + 0.080000 * 0.5, 0.080000) - 0.080000 * 0.5) - 0.002000;
        let gy = abs(glyph_mod(p.y + 0.080000 * 0.5, 0.080000) - 0.080000 * 0.5) - 0.002000;
        var sdf_result = min(gx, gy);
        let shade_fw = fwidth(sdf_result);
        let shade_alpha = 1.0 - smoothstep(-shade_fw, shade_fw, sdf_result);
        var color_result = vec4<f32>(vec3<f32>(0.060000, 0.080000, 0.120000) * shade_alpha, shade_alpha);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let warp_x = fbm2(p * 1.000000 + vec2<f32>(0.0, 1.3), i32(4.000000), 0.700000, (0.350000 + (downpour * 0.150000)));
        let warp_y = fbm2(p * 1.000000 + vec2<f32>(1.7, 0.0), i32(4.000000), 0.700000, (0.350000 + (downpour * 0.150000)));
        p = p + vec2<f32>(warp_x, warp_y) * (0.350000 + (downpour * 0.150000)); }
        var sdf_result = noise2(p * 15.000000 + vec2<f32>(time * 0.1, time * 0.07));
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.001000, 0.002000, 0.004000), vec3<f32>(0.005000, 0.010000, 0.020000), vec3<f32>(0.300000, 0.500000, 0.800000), vec3<f32>(0.000000, 0.200000, 0.400000));
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>((-0.120000), 0.080000);
        p = p + vec2<f32>(sin(p.y * 2.000000 + time * 0.800000), cos(p.x * 2.000000 + time * 0.800000)) * (0.015000 + (downpour * 0.008000));
        var sdf_result = sdf_circle(p, (0.065000 + (neon * 0.020000)));
        let shade_fw = fwidth(sdf_result);
        let shade_alpha = 1.0 - smoothstep(-shade_fw, shade_fw, sdf_result);
        var color_result = vec4<f32>(vec3<f32>(0.900000, 0.150000, 0.400000) * shade_alpha, shade_alpha);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>(0.140000, (-0.060000));
        p = p + vec2<f32>(sin(p.y * 2.000000 + time * 0.600000), cos(p.x * 2.000000 + time * 0.600000)) * 0.012000;
        var sdf_result = sdf_circle(p, (0.055000 + (neon * 0.015000)));
        let shade_fw = fwidth(sdf_result);
        let shade_alpha = 1.0 - smoothstep(-shade_fw, shade_fw, sdf_result);
        var color_result = vec4<f32>(vec3<f32>(0.080000, 0.700000, 0.900000) * shade_alpha, shade_alpha);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>(0.000000, (-0.200000));
        { let warp_x = fbm2(p * 4.000000 + vec2<f32>(0.0, 1.3), i32(4.000000), (0.120000 + (downpour * 0.080000)), 2.000000);
        let warp_y = fbm2(p * 4.000000 + vec2<f32>(1.7, 0.0), i32(4.000000), (0.120000 + (downpour * 0.080000)), 2.000000);
        p = p + vec2<f32>(warp_x, warp_y) * (0.120000 + (downpour * 0.080000)); }
        var sdf_result = fbm2((p * 5.000000 + vec2<f32>(time * 0.1, time * 0.07)), i32(5.000000), 0.500000, 2.000000);
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.002000, 0.001000, 0.003000), vec3<f32>(0.008000, 0.005000, 0.012000), vec3<f32>(0.800000, 0.300000, 0.600000), vec3<f32>(0.150000, 0.300000, 0.500000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let warp_x = fbm2(p * 2.000000 + vec2<f32>(0.0, 1.3), i32(4.000000), 0.080000, 2.000000);
        let warp_y = fbm2(p * 2.000000 + vec2<f32>(1.7, 0.0), i32(4.000000), 0.080000, 2.000000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.080000; }
        var sdf_result = fbm2((p * 2.500000 + vec2<f32>(time * 0.1, time * 0.07)), i32(4.000000), 0.500000, 2.000000);
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.001000, 0.001000, 0.002000), vec3<f32>(0.003000, 0.003000, 0.005000), vec3<f32>(0.600000, 0.400000, 0.800000), vec3<f32>(0.200000, 0.100000, 0.300000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
...
uniform float u_p_downpour;
uniform float u_p_neon;
...
void main(){
    ...
    // ── Layer 1: city ──
    {
        ...
        { float warp_x = fbm2(p * 1.500000 + vec2(0.0, 1.3), int(3.000000), 0.060000, 2.000000);
        float warp_y = fbm2(p * 1.500000 + vec2(1.7, 0.0), int(3.000000), 0.060000, 2.000000);
        p = p + vec2(warp_x, warp_y) * 0.060000; }
        float gx = abs(mod(p.x + 0.080000 * 0.5, 0.080000) - 0.080000 * 0.5) - 0.002000;
        float gy = abs(mod(p.y + 0.080000 * 0.5, 0.080000) - 0.080000 * 0.5) - 0.002000;
        float sdf_result = min(gx, gy);
        float shade_fw = fwidth(sdf_result);
        float shade_alpha = 1.0 - smoothstep(-shade_fw, shade_fw, sdf_result);
        vec4 color_result = vec4(vec3(0.060000, 0.080000, 0.120000) * shade_alpha, shade_alpha);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        { float warp_x = fbm2(p * 1.000000 + vec2(0.0, 1.3), int(4.000000), 0.700000, (0.350000 + (downpour * 0.150000)));
        float warp_y = fbm2(p * 1.000000 + vec2(1.7, 0.0), int(4.000000), 0.700000, (0.350000 + (downpour * 0.150000)));
        p = p + vec2(warp_x, warp_y) * (0.350000 + (downpour * 0.150000)); }
        float sdf_result = noise2(p * 15.000000 + vec2(time * 0.1, time * 0.07));
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.001000, 0.002000, 0.004000), vec3(0.005000, 0.010000, 0.020000), vec3(0.300000, 0.500000, 0.800000), vec3(0.000000, 0.200000, 0.400000));
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2((-0.120000), 0.080000);
        p = p + vec2(sin(p.y * 2.000000 + time * 0.800000), cos(p.x * 2.000000 + time * 0.800000)) * (0.015000 + (downpour * 0.008000));
        float sdf_result = sdf_circle(p, (0.065000 + (neon * 0.020000)));
        float shade_fw = fwidth(sdf_result);
        float shade_alpha = 1.0 - smoothstep(-shade_fw, shade_fw, sdf_result);
        vec4 color_result = vec4(vec3(0.900000, 0.150000, 0.400000) * shade_alpha, shade_alpha);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.900000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2(0.140000, (-0.060000));
        p = p + vec2(sin(p.y * 2.000000 + time * 0.600000), cos(p.x * 2.000000 + time * 0.600000)) * 0.012000;
        float sdf_result = sdf_circle(p, (0.055000 + (neon * 0.015000)));
        float shade_fw = fwidth(sdf_result);
        float shade_alpha = 1.0 - smoothstep(-shade_fw, shade_fw, sdf_result);
        vec4 color_result = vec4(vec3(0.080000, 0.700000, 0.900000) * shade_alpha, shade_alpha);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.890000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2(0.000000, (-0.200000));
        { float warp_x = fbm2(p * 4.000000 + vec2(0.0, 1.3), int(4.000000), (0.120000 + (downpour * 0.080000)), 2.000000);
        float warp_y = fbm2(p * 4.000000 + vec2(1.7, 0.0), int(4.000000), (0.120000 + (downpour * 0.080000)), 2.000000);
        p = p + vec2(warp_x, warp_y) * (0.120000 + (downpour * 0.080000)); }
        float sdf_result = fbm2((p * 5.000000 + vec2(time * 0.1, time * 0.07)), int(5.000000), 0.500000, 2.000000);
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.002000, 0.001000, 0.003000), vec3(0.008000, 0.005000, 0.012000), vec3(0.800000, 0.300000, 0.600000), vec3(0.150000, 0.300000, 0.500000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.850000);
        float la = color_result.a;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        { float warp_x = fbm2(p * 2.000000 + vec2(0.0, 1.3), int(4.000000), 0.080000, 2.000000);
        float warp_y = fbm2(p * 2.000000 + vec2(1.7, 0.0), int(4.000000), 0.080000, 2.000000);
        p = p + vec2(warp_x, warp_y) * 0.080000; }
        float sdf_result = fbm2((p * 2.500000 + vec2(time * 0.1, time * 0.07)), int(4.000000), 0.500000, 2.000000);
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.001000, 0.001000, 0.002000), vec3(0.003000, 0.003000, 0.005000), vec3(0.600000, 0.400000, 0.800000), vec3(0.200000, 0.100000, 0.300000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.820000);
        float la = color_result.a;
...
`;
const UNIFORMS = [{name:'downpour',default:0},{name:'neon',default:0.5}];
const COMPLEXITY = {layers:6,fbmOctaves:24,passes:2,memory:true,compute:false,is3d:false,tier:'extreme'};
const PASS_WGSL_0 = `// Post-processing pass: split
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let ca_dir = normalize(uv - 0.5) * 0.002000;
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'downpour', target: 'rain', field: 'intensity', weight: 0.4 },
      { source: 'downpour', target: 'reflections', field: 'brightness', weight: 0.3 },
      { source: 'neon', target: 'pink', field: 'brightness', weight: 0.3 },
      { source: 'neon', target: 'cyan', field: 'brightness', weight: 0.25 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class GameArcTimeline {
  constructor() {
    ...
    this._entries = [
      { target: 'downpour', from: 0, to: 1, duration: 6, easing: 'ease_out' },
      { target: 'neon', from: 0.5, to: 1, duration: 10, easing: 'ease_in_out' },
    ];
  }
...
class CyberpunkRain extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('glyph-cyberpunk-rain: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get downpour() { return this._renderer?.userParams['downpour'] ?? this._pendingParams['downpour'] ?? 0; }
  set downpour(v) { this.setParam('downpour', v); }
  get neon() { return this._renderer?.userParams['neon'] ?? this._pendingParams['neon'] ?? 0.5; }
  set neon(v) { this.setParam('neon', v); }
...
customElements.define('glyph-cyberpunk-rain', CyberpunkRain);
})();
