// GLYPH Component: supernova — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let detonation = u.p_detonation;
    let shockwave = u.p_shockwave;
    ...
    // ── Layer 1: stars ──
    {
        ...
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.000000, 0.000000, 0.001000), vec3<f32>(0.040000, 0.040000, 0.060000), vec3<f32>(0.900000, 0.900000, 1.000000), vec3<f32>(0.000000, 0.000000, 0.000000));
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let warp_x = fbm2(p * 2.000000 + vec2<f32>(0.0, 1.3), i32(5.000000), 0.550000, (0.200000 + (shockwave * 0.120000)));
        let warp_y = fbm2(p * 2.000000 + vec2<f32>(1.7, 0.0), i32(5.000000), 0.550000, (0.200000 + (shockwave * 0.120000)));
        p = p + vec2<f32>(warp_x, warp_y) * (0.200000 + (shockwave * 0.120000)); }
        var sdf_result = fbm2((p * 3.000000 + vec2<f32>(time * 0.1, time * 0.07)), i32(5.000000), 0.500000, 2.000000);
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.003000, 0.001000, 0.000000), vec3<f32>(0.010000, 0.004000, 0.002000), vec3<f32>(0.900000, 0.400000, 0.200000), vec3<f32>(0.000000, 0.150000, 0.300000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
    ...
    // ── Layer 3: remnant ──
    {
        ...
        { let warp_x = fbm2(p * 3.000000 + vec2<f32>(0.0, 1.3), i32(5.000000), (0.250000 + (detonation * 0.120000)), 2.000000);
        let warp_y = fbm2(p * 3.000000 + vec2<f32>(1.7, 0.0), i32(5.000000), (0.250000 + (detonation * 0.120000)), 2.000000);
        p = p + vec2<f32>(warp_x, warp_y) * (0.250000 + (detonation * 0.120000)); }
        var sdf_result = fbm2((p * 4.000000 + vec2<f32>(time * 0.1, time * 0.07)), i32(5.000000), 0.500000, 2.000000);
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.002000, 0.001000, 0.003000), vec3<f32>(0.008000, 0.003000, 0.010000), vec3<f32>(0.800000, 0.300000, 0.600000), vec3<f32>(0.000000, 0.150000, 0.250000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p + vec2<f32>(sin(p.y * 2.000000 + time * 1.000000), cos(p.x * 2.000000 + time * 1.000000)) * (0.040000 + (detonation * 0.020000));
        var sdf_result = abs(length(p) - (0.150000 + (shockwave * 0.200000))) - 0.012000;
        let shade_fw = fwidth(sdf_result);
        let shade_alpha = 1.0 - smoothstep(-shade_fw, shade_fw, sdf_result);
        var color_result = vec4<f32>(vec3<f32>(0.900000, 0.600000, 0.200000) * shade_alpha, shade_alpha);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p + vec2<f32>(sin(p.y * 2.500000 + time * 0.800000), cos(p.x * 2.500000 + time * 0.800000)) * 0.030000;
        var sdf_result = abs(length(p) - (0.080000 + (shockwave * 0.100000))) - 0.008000;
        let shade_fw = fwidth(sdf_result);
        let shade_alpha = 1.0 - smoothstep(-shade_fw, shade_fw, sdf_result);
        var color_result = vec4<f32>(vec3<f32>(1.000000, 0.850000, 0.500000) * shade_alpha, shade_alpha);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
        color_result = mix(color_result, prev_color, 0.860000);
        let la = color_result.a;
        let lc = color_result.rgb;
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p + vec2<f32>(sin(p.y * 3.000000 + time * 1.500000), cos(p.x * 3.000000 + time * 1.500000)) * 0.015000;
        var sdf_result = sdf_circle(p, (0.025000 + (detonation * 0.008000)));
        let shade_fw = fwidth(sdf_result);
        let shade_alpha = 1.0 - smoothstep(-shade_fw, shade_fw, sdf_result);
        var color_result = vec4<f32>(vec3<f32>(0.700000, 0.750000, 1.000000) * shade_alpha, shade_alpha);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let warp_x = fbm2(p * 4.000000 + vec2<f32>(0.0, 1.3), i32(4.000000), (0.150000 + (detonation * 0.080000)), 2.000000);
        let warp_y = fbm2(p * 4.000000 + vec2<f32>(1.7, 0.0), i32(4.000000), (0.150000 + (detonation * 0.080000)), 2.000000);
        p = p + vec2<f32>(warp_x, warp_y) * (0.150000 + (detonation * 0.080000)); }
        var sdf_result = noise2(p * 7.000000 + vec2<f32>(time * 0.1, time * 0.07));
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.001000, 0.000000, 0.002000), vec3<f32>(0.004000, 0.002000, 0.008000), vec3<f32>(0.400000, 0.250000, 0.500000), vec3<f32>(0.000000, 0.100000, 0.200000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
...
uniform float u_p_detonation;
uniform float u_p_shockwave;
...
void main(){
    ...
    float detonation = u_p_detonation;
    float shockwave = u_p_shockwave;
    ...
    // ── Layer 1: stars ──
    {
        ...
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.000000, 0.000000, 0.001000), vec3(0.040000, 0.040000, 0.060000), vec3(0.900000, 0.900000, 1.000000), vec3(0.000000, 0.000000, 0.000000));
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        { float warp_x = fbm2(p * 2.000000 + vec2(0.0, 1.3), int(5.000000), 0.550000, (0.200000 + (shockwave * 0.120000)));
        float warp_y = fbm2(p * 2.000000 + vec2(1.7, 0.0), int(5.000000), 0.550000, (0.200000 + (shockwave * 0.120000)));
        p = p + vec2(warp_x, warp_y) * (0.200000 + (shockwave * 0.120000)); }
        float sdf_result = fbm2((p * 3.000000 + vec2(time * 0.1, time * 0.07)), int(5.000000), 0.500000, 2.000000);
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.003000, 0.001000, 0.000000), vec3(0.010000, 0.004000, 0.002000), vec3(0.900000, 0.400000, 0.200000), vec3(0.000000, 0.150000, 0.300000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
    ...
    // ── Layer 3: remnant ──
    {
        ...
        { float warp_x = fbm2(p * 3.000000 + vec2(0.0, 1.3), int(5.000000), (0.250000 + (detonation * 0.120000)), 2.000000);
        float warp_y = fbm2(p * 3.000000 + vec2(1.7, 0.0), int(5.000000), (0.250000 + (detonation * 0.120000)), 2.000000);
        p = p + vec2(warp_x, warp_y) * (0.250000 + (detonation * 0.120000)); }
        float sdf_result = fbm2((p * 4.000000 + vec2(time * 0.1, time * 0.07)), int(5.000000), 0.500000, 2.000000);
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.002000, 0.001000, 0.003000), vec3(0.008000, 0.003000, 0.010000), vec3(0.800000, 0.300000, 0.600000), vec3(0.000000, 0.150000, 0.250000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.890000);
        float la = color_result.a;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p + vec2(sin(p.y * 2.000000 + time * 1.000000), cos(p.x * 2.000000 + time * 1.000000)) * (0.040000 + (detonation * 0.020000));
        float sdf_result = abs(length(p) - (0.150000 + (shockwave * 0.200000))) - 0.012000;
        float shade_fw = fwidth(sdf_result);
        float shade_alpha = 1.0 - smoothstep(-shade_fw, shade_fw, sdf_result);
        vec4 color_result = vec4(vec3(0.900000, 0.600000, 0.200000) * shade_alpha, shade_alpha);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.840000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p + vec2(sin(p.y * 2.500000 + time * 0.800000), cos(p.x * 2.500000 + time * 0.800000)) * 0.030000;
        float sdf_result = abs(length(p) - (0.080000 + (shockwave * 0.100000))) - 0.008000;
        float shade_fw = fwidth(sdf_result);
        float shade_alpha = 1.0 - smoothstep(-shade_fw, shade_fw, sdf_result);
        vec4 color_result = vec4(vec3(1.000000, 0.850000, 0.500000) * shade_alpha, shade_alpha);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.860000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p + vec2(sin(p.y * 3.000000 + time * 1.500000), cos(p.x * 3.000000 + time * 1.500000)) * 0.015000;
        float sdf_result = sdf_circle(p, (0.025000 + (detonation * 0.008000)));
        float shade_fw = fwidth(sdf_result);
        float shade_alpha = 1.0 - smoothstep(-shade_fw, shade_fw, sdf_result);
        vec4 color_result = vec4(vec3(0.700000, 0.750000, 1.000000) * shade_alpha, shade_alpha);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.940000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        { float warp_x = fbm2(p * 4.000000 + vec2(0.0, 1.3), int(4.000000), (0.150000 + (detonation * 0.080000)), 2.000000);
        float warp_y = fbm2(p * 4.000000 + vec2(1.7, 0.0), int(4.000000), (0.150000 + (detonation * 0.080000)), 2.000000);
        p = p + vec2(warp_x, warp_y) * (0.150000 + (detonation * 0.080000)); }
        float sdf_result = noise2(p * 7.000000 + vec2(time * 0.1, time * 0.07));
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.001000, 0.000000, 0.002000), vec3(0.004000, 0.002000, 0.008000), vec3(0.400000, 0.250000, 0.500000), vec3(0.000000, 0.100000, 0.200000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.820000);
        float la = color_result.a;
...
`;
const UNIFORMS = [{name:'detonation',default:0},{name:'shockwave',default:0}];
const COMPLEXITY = {layers:7,fbmOctaves:24,passes:2,memory:true,compute:false,is3d:false,tier:'extreme'};
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'detonation', target: 'core', field: 'brightness', weight: 0.5 },
      { source: 'detonation', target: 'remnant', field: 'intensity', weight: 0.4 },
      { source: 'shockwave', target: 'blast', field: 'brightness', weight: 0.4 },
      { source: 'shockwave', target: 'ejecta', field: 'intensity', weight: 0.3 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class GameArcTimeline {
  constructor() {
    ...
    this._entries = [
      { target: 'detonation', from: 0, to: 1, duration: 3, easing: 'ease_out' },
      { target: 'shockwave', from: 0, to: 1, duration: 8, easing: 'ease_out' },
    ];
  }
...
class Supernova extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('glyph-supernova: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get detonation() { return this._renderer?.userParams['detonation'] ?? this._pendingParams['detonation'] ?? 0; }
  set detonation(v) { this.setParam('detonation', v); }
  get shockwave() { return this._renderer?.userParams['shockwave'] ?? this._pendingParams['shockwave'] ?? 0; }
  set shockwave(v) { this.setParam('shockwave', v); }
...
customElements.define('glyph-supernova', Supernova);
})();
