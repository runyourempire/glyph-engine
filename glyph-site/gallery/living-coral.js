// GLYPH Component: living-coral — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let bloom_cycle = u.p_bloom_cycle;
    ...
    // ── Layer 1: water ──
    {
        ...
        { let warp_x = fbm2(p * 1.500000 + vec2<f32>(0.0, 1.3), i32(5.000000), 0.500000, (0.150000 + (current * 0.050000)));
        let warp_y = fbm2(p * 1.500000 + vec2<f32>(1.7, 0.0), i32(5.000000), 0.500000, (0.150000 + (current * 0.050000)));
        p = p + vec2<f32>(warp_x, warp_y) * (0.150000 + (current * 0.050000)); }
        var sdf_result = fbm2((p * 2.000000 + vec2<f32>(time * 0.1, time * 0.07)), i32(5.000000), 0.500000, 2.000000);
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.0, 0.1, 0.3), vec3<f32>(0.0, 0.2, 0.3), vec3<f32>(0.5, 0.8, 1.0), vec3<f32>(0.0, 0.1, 0.2));
    ...
    // ── Layer 2: coral_a ──
    {
        ...
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.6, 0.35, 0.3), vec3<f32>(0.3, 0.25, 0.2), vec3<f32>(0.8, 0.7, 0.8), vec3<f32>(0.0, 0.1, 0.25));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>(0.150000, (-0.100000));
        ...
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.6, 0.3, 0.4), vec3<f32>(0.3, 0.2, 0.3), vec3<f32>(1.0, 0.8, 1.0), vec3<f32>(0.0, 0.1, 0.3));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p + vec2<f32>(sin(p.y * 2.000000 + time * 0.300000), cos(p.x * 2.000000 + time * 0.300000)) * (0.030000 + (bloom_cycle * 0.020000));
        var sdf_result = sdf_circle(p, (0.100000 + (bloom_cycle * 0.040000)));
        let glow_pulse = (3.500000 + (bloom_cycle * 1.500000)) * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(1.000000, 0.400000, 0.500000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    // ── Layer 5: fronds ──
    {
        ...
        p = p + vec2<f32>(sin(p.y * 3.000000 + time * 0.400000), cos(p.x * 3.000000 + time * 0.400000)) * 0.040000;
        var sdf_result = sdf_line(p, vec2<f32>(-(0.180000 + (current * 0.030000)) * 0.5, 0.0), vec2<f32>((0.180000 + (current * 0.030000)) * 0.5, 0.0)) - 0.008000;
        let glow_pulse = 1.800000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.900000, 0.500000, 0.600000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let warp_x = fbm2(p * 6.000000 + vec2<f32>(0.0, 1.3), i32(3.000000), (0.180000 + (current * 0.080000)), 2.000000);
        let warp_y = fbm2(p * 6.000000 + vec2<f32>(1.7, 0.0), i32(3.000000), (0.180000 + (current * 0.080000)), 2.000000);
        p = p + vec2<f32>(warp_x, warp_y) * (0.180000 + (current * 0.080000)); }
        var sdf_result = voronoi2(p * 22.000000 + vec2<f32>(time * 0.05, time * 0.03));
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.000000, 0.030000, 0.020000), vec3<f32>(0.100000, 0.200000, 0.100000), vec3<f32>(0.400000, 1.000000, 0.600000), vec3<f32>(0.200000, 0.400000, 0.100000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>(0.000000, 0.250000);
        { let warp_x = fbm2(p * 1.000000 + vec2<f32>(0.0, 1.3), i32(3.000000), 0.100000, 2.000000);
        let warp_y = fbm2(p * 1.000000 + vec2<f32>(1.7, 0.0), i32(3.000000), 0.100000, 2.000000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.100000; }
        let sdf_result = smoothstep(0.000000, 0.400000, length(p));
        let glow_pulse = 1.200000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.300000, 0.500000, 0.600000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
...
`;
const UNIFORMS = [{name:'current',default:0},{name:'bloom_cycle',default:0.4}];
const COMPLEXITY = {layers:7,fbmOctaves:27,passes:2,memory:true,compute:false,is3d:false,tier:'extreme'};
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'current', target: 'coral_a', field: 'intensity', weight: 0.3 },
      { source: 'current', target: 'plankton', field: 'intensity', weight: 0.4 },
      { source: 'bloom_cycle', target: 'anemone', field: 'brightness', weight: 0.5 },
      { source: 'bloom_cycle', target: 'fronds', field: 'brightness', weight: 0.3 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class GameArcTimeline {
  constructor() {
    ...
    this._entries = [
      { target: 'current', from: 0, to: 0.8, duration: 10, easing: 'ease_in_out' },
      { target: 'bloom_cycle', from: 0.4, to: 1, duration: 8, easing: 'ease_in_out' },
    ];
  }
...
class LivingCoral extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('glyph-living-coral: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get current() { return this._renderer?.userParams['current'] ?? this._pendingParams['current'] ?? 0; }
  set current(v) { this.setParam('current', v); }
  get bloom_cycle() { return this._renderer?.userParams['bloom_cycle'] ?? this._pendingParams['bloom_cycle'] ?? 0.4; }
  set bloom_cycle(v) { this.setParam('bloom_cycle', v); }
...
customElements.define('glyph-living-coral', LivingCoral);
})();
