// GLYPH Component: solar-flare — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let eruption = u.p_eruption;
    let corona = u.p_corona;
    ...
    // ── Layer 1: surface ──
    {
        ...
        { let warp_x = fbm2(p * 4.000000 + vec2<f32>(0.0, 1.3), i32(6.000000), 0.550000, 0.200000);
        let warp_y = fbm2(p * 4.000000 + vec2<f32>(1.7, 0.0), i32(6.000000), 0.550000, 0.200000);
        ...
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.005000, 0.001000, 0.000000), vec3<f32>(0.025000, 0.008000, 0.001000), vec3<f32>(1.000000, 0.800000, 0.500000), vec3<f32>(0.000000, 0.000000, 0.000000));
    ...
    // ── Layer 2: field ──
    {
        ...
        { let warp_x = fbm2(p * 2.000000 + vec2<f32>(0.0, 1.3), i32(5.000000), 0.600000, (0.300000 + (corona * 0.120000)));
        let warp_y = fbm2(p * 2.000000 + vec2<f32>(1.7, 0.0), i32(5.000000), 0.600000, (0.300000 + (corona * 0.120000)));
        p = p + vec2<f32>(warp_x, warp_y) * (0.300000 + (corona * 0.120000)); }
        var sdf_result = fbm2((p * 3.000000 + vec2<f32>(time * 0.1, time * 0.07)), i32(5.000000), 0.500000, 2.000000);
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.004000, 0.001000, 0.000000), vec3<f32>(0.015000, 0.005000, 0.000000), vec3<f32>(0.800000, 0.600000, 0.300000), vec3<f32>(0.000000, 0.000000, 0.000000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>(0.000000, (0.150000 + (eruption * 0.080000)));
        { let warp_x = fbm2(p * 3.000000 + vec2<f32>(0.0, 1.3), i32(4.000000), (0.250000 + (eruption * 0.150000)), 2.000000);
        let warp_y = fbm2(p * 3.000000 + vec2<f32>(1.7, 0.0), i32(4.000000), (0.250000 + (eruption * 0.150000)), 2.000000);
        p = p + vec2<f32>(warp_x, warp_y) * (0.250000 + (eruption * 0.150000)); }
        var sdf_result = noise2(p * 6.000000 + vec2<f32>(time * 0.1, time * 0.07));
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.006000, 0.002000, 0.000000), vec3<f32>(0.030000, 0.012000, 0.001000), vec3<f32>(1.000000, 0.700000, 0.300000), vec3<f32>(0.000000, 0.050000, 0.000000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p + vec2<f32>(sin(p.y * 3.000000 + time * 0.600000), cos(p.x * 3.000000 + time * 0.600000)) * 0.030000;
        var sdf_result = abs(length(p) - 0.140000) - 0.012000;
        let shade_fw = fwidth(sdf_result);
        let shade_alpha = 1.0 - smoothstep(-shade_fw, shade_fw, sdf_result);
        var color_result = vec4<f32>(vec3<f32>(0.900000, 0.400000, 0.080000) * shade_alpha, shade_alpha);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    // ── Layer 5: core ──
    {
        ...
        var sdf_result = sdf_circle(p, (0.100000 + (eruption * 0.015000)));
        let shade_fw = fwidth(sdf_result);
        let shade_alpha = 1.0 - smoothstep(-shade_fw, shade_fw, sdf_result);
        var color_result = vec4<f32>(vec3<f32>(1.000000, 0.900000, 0.650000) * shade_alpha, shade_alpha);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        var sdf_result = abs(length(p) - 0.065000) - 0.002000;
        let shade_fw = fwidth(sdf_result);
        let shade_alpha = 1.0 - smoothstep(-shade_fw, shade_fw, sdf_result);
        var color_result = vec4<f32>(vec3<f32>(1.000000, 0.950000, 0.800000) * shade_alpha, shade_alpha);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
...
uniform float u_p_eruption;
uniform float u_p_corona;
...
void main(){
    ...
    float eruption = u_p_eruption;
    float corona = u_p_corona;
    ...
    // ── Layer 1: surface ──
    {
        ...
        { float warp_x = fbm2(p * 4.000000 + vec2(0.0, 1.3), int(6.000000), 0.550000, 0.200000);
        float warp_y = fbm2(p * 4.000000 + vec2(1.7, 0.0), int(6.000000), 0.550000, 0.200000);
        p = p + vec2(warp_x, warp_y) * 0.200000; }
        float sdf_result = fbm2((p * 5.000000 + vec2(time * 0.1, time * 0.07)), int(5.000000), 0.500000, 2.000000);
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.005000, 0.001000, 0.000000), vec3(0.025000, 0.008000, 0.001000), vec3(1.000000, 0.800000, 0.500000), vec3(0.000000, 0.000000, 0.000000));
    ...
    // ── Layer 2: field ──
    {
        ...
        { float warp_x = fbm2(p * 2.000000 + vec2(0.0, 1.3), int(5.000000), 0.600000, (0.300000 + (corona * 0.120000)));
        float warp_y = fbm2(p * 2.000000 + vec2(1.7, 0.0), int(5.000000), 0.600000, (0.300000 + (corona * 0.120000)));
        p = p + vec2(warp_x, warp_y) * (0.300000 + (corona * 0.120000)); }
        float sdf_result = fbm2((p * 3.000000 + vec2(time * 0.1, time * 0.07)), int(5.000000), 0.500000, 2.000000);
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.004000, 0.001000, 0.000000), vec3(0.015000, 0.005000, 0.000000), vec3(0.800000, 0.600000, 0.300000), vec3(0.000000, 0.000000, 0.000000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.900000);
        float la = color_result.a;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2(0.000000, (0.150000 + (eruption * 0.080000)));
        { float warp_x = fbm2(p * 3.000000 + vec2(0.0, 1.3), int(4.000000), (0.250000 + (eruption * 0.150000)), 2.000000);
        float warp_y = fbm2(p * 3.000000 + vec2(1.7, 0.0), int(4.000000), (0.250000 + (eruption * 0.150000)), 2.000000);
        p = p + vec2(warp_x, warp_y) * (0.250000 + (eruption * 0.150000)); }
        float sdf_result = noise2(p * 6.000000 + vec2(time * 0.1, time * 0.07));
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.006000, 0.002000, 0.000000), vec3(0.030000, 0.012000, 0.001000), vec3(1.000000, 0.700000, 0.300000), vec3(0.000000, 0.050000, 0.000000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.850000);
        float la = color_result.a;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p + vec2(sin(p.y * 3.000000 + time * 0.600000), cos(p.x * 3.000000 + time * 0.600000)) * 0.030000;
        float sdf_result = abs(length(p) - 0.140000) - 0.012000;
        float shade_fw = fwidth(sdf_result);
        float shade_alpha = 1.0 - smoothstep(-shade_fw, shade_fw, sdf_result);
        vec4 color_result = vec4(vec3(0.900000, 0.400000, 0.080000) * shade_alpha, shade_alpha);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.890000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    // ── Layer 5: core ──
    {
        ...
        float sdf_result = sdf_circle(p, (0.100000 + (eruption * 0.015000)));
        float shade_fw = fwidth(sdf_result);
        float shade_alpha = 1.0 - smoothstep(-shade_fw, shade_fw, sdf_result);
        vec4 color_result = vec4(vec3(1.000000, 0.900000, 0.650000) * shade_alpha, shade_alpha);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.930000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        float sdf_result = abs(length(p) - 0.065000) - 0.002000;
        float shade_fw = fwidth(sdf_result);
        float shade_alpha = 1.0 - smoothstep(-shade_fw, shade_fw, sdf_result);
        vec4 color_result = vec4(vec3(1.000000, 0.950000, 0.800000) * shade_alpha, shade_alpha);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.800000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
...
`;
const UNIFORMS = [{name:'eruption',default:0},{name:'corona',default:0.4}];
const COMPLEXITY = {layers:6,fbmOctaves:25,passes:1,memory:true,compute:false,is3d:false,tier:'extreme'};
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'eruption', target: 'flare', field: 'brightness', weight: 0.4 },
      { source: 'eruption', target: 'chromo', field: 'brightness', weight: 0.3 },
      { source: 'corona', target: 'surface', field: 'brightness', weight: 0.3 },
      { source: 'corona', target: 'core', field: 'brightness', weight: 0.2 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class GameArcTimeline {
  constructor() {
    ...
    this._entries = [
      { target: 'eruption', from: 0, to: 1, duration: 8, easing: 'ease_out' },
      { target: 'corona', from: 0.4, to: 1, duration: 12, easing: 'ease_in_out' },
    ];
  }
...
class SolarFlare extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('glyph-solar-flare: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get eruption() { return this._renderer?.userParams['eruption'] ?? this._pendingParams['eruption'] ?? 0; }
  set eruption(v) { this.setParam('eruption', v); }
  get corona() { return this._renderer?.userParams['corona'] ?? this._pendingParams['corona'] ?? 0.4; }
  set corona(v) { this.setParam('corona', v); }
...
customElements.define('glyph-solar-flare', SolarFlare);
})();
