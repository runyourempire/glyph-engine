// GLYPH Component: volcanic-core — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 1: crust ──
    {
        ...
        { let warp_x = fbm2(p * 3.000000 + vec2<f32>(0.0, 1.3), i32(5.000000), 0.550000, 0.200000);
        let warp_y = fbm2(p * 3.000000 + vec2<f32>(1.7, 0.0), i32(5.000000), 0.550000, 0.200000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.200000; }
        var sdf_result = voronoi2(p * 7.000000 + vec2<f32>(time * 0.05, time * 0.03));
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.030000, 0.010000, 0.010000), vec3<f32>(0.080000, 0.030000, 0.020000), vec3<f32>(0.150000, 0.080000, 0.050000), vec3<f32>(0.000000, 0.000000, 0.000000));
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let warp_x = fbm2(p * 2.500000 + vec2<f32>(0.0, 1.3), i32(6.000000), 0.600000, 2.100000);
        let warp_y = fbm2(p * 2.500000 + vec2<f32>(1.7, 0.0), i32(6.000000), 0.600000, 2.100000);
        p = p + vec2<f32>(warp_x, warp_y) * (0.300000 + (pressure * 0.150000)); }
        var sdf_result = fbm2((p * 3.000000 + vec2<f32>(time * 0.1, time * 0.07)), i32(6.000000), 0.550000, 2.000000);
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.5, 0.2, 0.0), vec3<f32>(0.5, 0.3, 0.1), vec3<f32>(0.8, 0.5, 0.5), vec3<f32>(0.0, 0.2, 0.3));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    // ── Layer 3: flow_b ──
    {
        ...
        { let warp_x = fbm2(p * 3.000000 + vec2<f32>(0.0, 1.3), i32(5.000000), 0.550000, (0.250000 + (heat * 0.100000)));
        let warp_y = fbm2(p * 3.000000 + vec2<f32>(1.7, 0.0), i32(5.000000), 0.550000, (0.250000 + (heat * 0.100000)));
        p = p + vec2<f32>(warp_x, warp_y) * (0.250000 + (heat * 0.100000)); }
        var sdf_result = fbm2((p * 4.000000 + vec2<f32>(time * 0.1, time * 0.07)), i32(5.000000), 0.500000, 2.000000);
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.55, 0.2, 0.08), vec3<f32>(0.45, 0.25, 0.1), vec3<f32>(1.0, 0.7, 0.4), vec3<f32>(0.0, 0.15, 0.25));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p + vec2<f32>(sin(p.y * 2.500000 + time * 0.800000), cos(p.x * 2.500000 + time * 0.800000)) * (0.040000 + (pressure * 0.030000));
        var sdf_result = sdf_circle(p, (0.080000 + (pressure * 0.040000)));
        let glow_pulse = (5.000000 + (heat * 3.000000)) * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(1.000000, 0.900000, 0.500000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p + vec2<f32>(sin(p.y * 2.000000 + time * 0.600000), cos(p.x * 2.000000 + time * 0.600000)) * 0.030000;
        var sdf_result = sdf_circle(p, (0.180000 + (pressure * 0.040000)));
        for (var onion_i: i32 = 0; onion_i < i32(3.000000); onion_i = onion_i + 1) { sdf_result = abs(sdf_result) - 0.020000; }
        let glow_pulse = (2.500000 + (heat * 1.000000)) * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(1.000000, 0.500000, 0.100000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let warp_x = fbm2(p * 7.000000 + vec2<f32>(0.0, 1.3), i32(3.000000), (0.200000 + (pressure * 0.100000)), 2.000000);
        let warp_y = fbm2(p * 7.000000 + vec2<f32>(1.7, 0.0), i32(3.000000), (0.200000 + (pressure * 0.100000)), 2.000000);
        p = p + vec2<f32>(warp_x, warp_y) * (0.200000 + (pressure * 0.100000)); }
        var sdf_result = voronoi2(p * 18.000000 + vec2<f32>(time * 0.05, time * 0.03));
    ...
    // ── Layer 7: smoke ──
    {
        ...
        { let warp_x = fbm2(p * 2.000000 + vec2<f32>(0.0, 1.3), i32(5.000000), 0.500000, (0.300000 + (pressure * 0.150000)));
        let warp_y = fbm2(p * 2.000000 + vec2<f32>(1.7, 0.0), i32(5.000000), 0.500000, (0.300000 + (pressure * 0.150000)));
        p = p + vec2<f32>(warp_x, warp_y) * (0.300000 + (pressure * 0.150000)); }
        var sdf_result = fbm2((p * 2.500000 + vec2<f32>(time * 0.1, time * 0.07)), i32(5.000000), 0.500000, 2.000000);
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.020000, 0.020000, 0.020000), vec3<f32>(0.100000, 0.080000, 0.060000), vec3<f32>(0.200000, 0.150000, 0.100000), vec3<f32>(0.000000, 0.000000, 0.000000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
...
void main(){
    ...
    // ── Layer 1: crust ──
    {
        ...
        { float warp_x = fbm2(p * 3.000000 + vec2(0.0, 1.3), int(5.000000), 0.550000, 0.200000);
        float warp_y = fbm2(p * 3.000000 + vec2(1.7, 0.0), int(5.000000), 0.550000, 0.200000);
        p = p + vec2(warp_x, warp_y) * 0.200000; }
        float sdf_result = voronoi2(p * 7.000000 + vec2(time * 0.05, time * 0.03));
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.030000, 0.010000, 0.010000), vec3(0.080000, 0.030000, 0.020000), vec3(0.150000, 0.080000, 0.050000), vec3(0.000000, 0.000000, 0.000000));
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        { float warp_x = fbm2(p * 2.500000 + vec2(0.0, 1.3), int(6.000000), 0.600000, 2.100000);
        float warp_y = fbm2(p * 2.500000 + vec2(1.7, 0.0), int(6.000000), 0.600000, 2.100000);
        p = p + vec2(warp_x, warp_y) * (0.300000 + (pressure * 0.150000)); }
        float sdf_result = fbm2((p * 3.000000 + vec2(time * 0.1, time * 0.07)), int(6.000000), 0.550000, 2.000000);
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.5, 0.2, 0.0), vec3(0.5, 0.3, 0.1), vec3(0.8, 0.5, 0.5), vec3(0.0, 0.2, 0.3));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.920000);
        float la = color_result.a;
    ...
    // ── Layer 3: flow_b ──
    {
        ...
        { float warp_x = fbm2(p * 3.000000 + vec2(0.0, 1.3), int(5.000000), 0.550000, (0.250000 + (heat * 0.100000)));
        float warp_y = fbm2(p * 3.000000 + vec2(1.7, 0.0), int(5.000000), 0.550000, (0.250000 + (heat * 0.100000)));
        p = p + vec2(warp_x, warp_y) * (0.250000 + (heat * 0.100000)); }
        float sdf_result = fbm2((p * 4.000000 + vec2(time * 0.1, time * 0.07)), int(5.000000), 0.500000, 2.000000);
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.55, 0.2, 0.08), vec3(0.45, 0.25, 0.1), vec3(1.0, 0.7, 0.4), vec3(0.0, 0.15, 0.25));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.900000);
        float la = color_result.a;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p + vec2(sin(p.y * 2.500000 + time * 0.800000), cos(p.x * 2.500000 + time * 0.800000)) * (0.040000 + (pressure * 0.030000));
        float sdf_result = sdf_circle(p, (0.080000 + (pressure * 0.040000)));
        float glow_pulse = (5.000000 + (heat * 3.000000)) * (0.9 + 0.1 * sin(time * 2.0));
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p + vec2(sin(p.y * 2.000000 + time * 0.600000), cos(p.x * 2.000000 + time * 0.600000)) * 0.030000;
        float sdf_result = sdf_circle(p, (0.180000 + (pressure * 0.040000)));
        for (int onion_i = 0; onion_i < int(3.000000); onion_i++) { sdf_result = abs(sdf_result) - 0.020000; }
        float glow_pulse = (2.500000 + (heat * 1.000000)) * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(1.000000, 0.500000, 0.100000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.890000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        { float warp_x = fbm2(p * 7.000000 + vec2(0.0, 1.3), int(3.000000), (0.200000 + (pressure * 0.100000)), 2.000000);
        float warp_y = fbm2(p * 7.000000 + vec2(1.7, 0.0), int(3.000000), (0.200000 + (pressure * 0.100000)), 2.000000);
        p = p + vec2(warp_x, warp_y) * (0.200000 + (pressure * 0.100000)); }
    ...
    // ── Layer 7: smoke ──
    {
        ...
        { float warp_x = fbm2(p * 2.000000 + vec2(0.0, 1.3), int(5.000000), 0.500000, (0.300000 + (pressure * 0.150000)));
        float warp_y = fbm2(p * 2.000000 + vec2(1.7, 0.0), int(5.000000), 0.500000, (0.300000 + (pressure * 0.150000)));
        p = p + vec2(warp_x, warp_y) * (0.300000 + (pressure * 0.150000)); }
        float sdf_result = fbm2((p * 2.500000 + vec2(time * 0.1, time * 0.07)), int(5.000000), 0.500000, 2.000000);
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.020000, 0.020000, 0.020000), vec3(0.100000, 0.080000, 0.060000), vec3(0.200000, 0.150000, 0.100000), vec3(0.000000, 0.000000, 0.000000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.860000);
        float la = color_result.a;
...
`;
const UNIFORMS = [{name:'pressure',default:0},{name:'heat',default:0.6}];
const COMPLEXITY = {layers:7,fbmOctaves:40,passes:2,memory:true,compute:false,is3d:false,tier:'extreme'};
const PASS_WGSL_0 = `// Post-processing pass: heat
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'pressure', target: 'vent', field: 'brightness', weight: 0.6 },
      { source: 'pressure', target: 'embers', field: 'intensity', weight: 0.5 },
      { source: 'heat', target: 'magma', field: 'brightness', weight: 0.4 },
      { source: 'heat', target: 'heat_zones', field: 'brightness', weight: 0.3 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class GameArcTimeline {
  constructor() {
    ...
    this._entries = [
      { target: 'pressure', from: 0, to: 1, duration: 6, easing: 'ease_out' },
      { target: 'heat', from: 0.6, to: 1, duration: 10, easing: 'ease_in_out' },
    ];
  }
...
class VolcanicCore extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('glyph-volcanic-core: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get pressure() { return this._renderer?.userParams['pressure'] ?? this._pendingParams['pressure'] ?? 0; }
  set pressure(v) { this.setParam('pressure', v); }
  get heat() { return this._renderer?.userParams['heat'] ?? this._pendingParams['heat'] ?? 0.6; }
...
customElements.define('glyph-volcanic-core', VolcanicCore);
})();
