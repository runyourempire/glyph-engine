// GLYPH Component: plasma-storm — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let voltage = u.p_voltage;
    let discharge = u.p_discharge;
    ...
    // ── Layer 1: clouds ──
    {
        ...
        { let warp_x = fbm2(p * 2.000000 + vec2<f32>(0.0, 1.3), i32(6.000000), 0.550000, 2.200000);
        let warp_y = fbm2(p * 2.000000 + vec2<f32>(1.7, 0.0), i32(6.000000), 0.550000, 2.200000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.300000; }
        var sdf_result = fbm2((p * 3.000000 + vec2<f32>(time * 0.1, time * 0.07)), i32(6.000000), 0.600000, 2.000000);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let warp_x = fbm2(p * 8.000000 + vec2<f32>(0.0, 1.3), i32(3.000000), (0.400000 + (voltage * 0.200000)), 2.000000);
        let warp_y = fbm2(p * 8.000000 + vec2<f32>(1.7, 0.0), i32(3.000000), (0.400000 + (voltage * 0.200000)), 2.000000);
        p = p + vec2<f32>(warp_x, warp_y) * (0.400000 + (voltage * 0.200000)); }
        var sdf_result = noise2(p * 15.000000 + vec2<f32>(time * 0.1, time * 0.07));
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.100000, 0.100000, 0.300000), vec3<f32>(0.300000, 0.300000, 0.800000), vec3<f32>(0.800000, 0.900000, 1.000000), vec3<f32>(0.000000, 0.150000, 0.500000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let ra = time * 0.700000; let rc = cos(ra); let rs = sin(ra);
        p = vec2<f32>(p.x * rc - p.y * rs, p.x * rs + p.y * rc); }
        { let warp_x = fbm2(p * 10.000000 + vec2<f32>(0.0, 1.3), i32(3.000000), (0.350000 + (discharge * 0.150000)), 2.000000);
        let warp_y = fbm2(p * 10.000000 + vec2<f32>(1.7, 0.0), i32(3.000000), (0.350000 + (discharge * 0.150000)), 2.000000);
        p = p + vec2<f32>(warp_x, warp_y) * (0.350000 + (discharge * 0.150000)); }
    ...
    // ── Layer 4: eye ──
    {
        ...
        { let warp_x = fbm2(p * 4.000000 + vec2<f32>(0.0, 1.3), i32(4.000000), 0.250000, 2.000000);
        let warp_y = fbm2(p * 4.000000 + vec2<f32>(1.7, 0.0), i32(4.000000), 0.250000, 2.000000);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p + vec2<f32>(sin(p.y * 3.000000 + time * 1.200000), cos(p.x * 3.000000 + time * 1.200000)) * (0.050000 + (voltage * 0.030000));
        var sdf_result = sdf_circle(p, (0.080000 + (discharge * 0.040000)));
        let glow_pulse = (4.500000 + (voltage * 2.000000)) * (0.9 + 0.1 * sin(time * 2.0));
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p + vec2<f32>(sin(p.y * 5.000000 + time * 1.500000), cos(p.x * 5.000000 + time * 1.500000)) * 0.060000;
        var sdf_result = abs(length(p) - 0.200000) - 0.008000;
        let glow_pulse = (2.500000 + (discharge * 1.500000)) * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.500000, 0.600000, 1.000000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        var sdf_result = abs(length(p) - 0.450000) - 0.003000;
        let glow_pulse = 0.700000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.200000, 0.250000, 0.400000), color_result.a);
        let la = color_result.a;
        let lc = color_result.rgb;
        final_color = vec4<f32>(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
...
uniform float u_p_voltage;
uniform float u_p_discharge;
...
void main(){
    ...
    float voltage = u_p_voltage;
    float discharge = u_p_discharge;
    ...
    // ── Layer 1: clouds ──
    {
        ...
        { float warp_x = fbm2(p * 2.000000 + vec2(0.0, 1.3), int(6.000000), 0.550000, 2.200000);
        float warp_y = fbm2(p * 2.000000 + vec2(1.7, 0.0), int(6.000000), 0.550000, 2.200000);
        p = p + vec2(warp_x, warp_y) * 0.300000; }
        float sdf_result = fbm2((p * 3.000000 + vec2(time * 0.1, time * 0.07)), int(6.000000), 0.600000, 2.000000);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        { float warp_x = fbm2(p * 8.000000 + vec2(0.0, 1.3), int(3.000000), (0.400000 + (voltage * 0.200000)), 2.000000);
        float warp_y = fbm2(p * 8.000000 + vec2(1.7, 0.0), int(3.000000), (0.400000 + (voltage * 0.200000)), 2.000000);
        p = p + vec2(warp_x, warp_y) * (0.400000 + (voltage * 0.200000)); }
        float sdf_result = noise2(p * 15.000000 + vec2(time * 0.1, time * 0.07));
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.100000, 0.100000, 0.300000), vec3(0.300000, 0.300000, 0.800000), vec3(0.800000, 0.900000, 1.000000), vec3(0.000000, 0.150000, 0.500000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.850000);
        float la = color_result.a;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        { float ra = time * 0.700000; float rc = cos(ra); float rs = sin(ra);
        p = vec2(p.x * rc - p.y * rs, p.x * rs + p.y * rc); }
        { float warp_x = fbm2(p * 10.000000 + vec2(0.0, 1.3), int(3.000000), (0.350000 + (discharge * 0.150000)), 2.000000);
        float warp_y = fbm2(p * 10.000000 + vec2(1.7, 0.0), int(3.000000), (0.350000 + (discharge * 0.150000)), 2.000000);
        p = p + vec2(warp_x, warp_y) * (0.350000 + (discharge * 0.150000)); }
    ...
    // ── Layer 4: eye ──
    {
        ...
        { float warp_x = fbm2(p * 4.000000 + vec2(0.0, 1.3), int(4.000000), 0.250000, 2.000000);
        float warp_y = fbm2(p * 4.000000 + vec2(1.7, 0.0), int(4.000000), 0.250000, 2.000000);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p + vec2(sin(p.y * 3.000000 + time * 1.200000), cos(p.x * 3.000000 + time * 1.200000)) * (0.050000 + (voltage * 0.030000));
        float sdf_result = sdf_circle(p, (0.080000 + (discharge * 0.040000)));
        float glow_pulse = (4.500000 + (voltage * 2.000000)) * (0.9 + 0.1 * sin(time * 2.0));
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p + vec2(sin(p.y * 5.000000 + time * 1.500000), cos(p.x * 5.000000 + time * 1.500000)) * 0.060000;
        float sdf_result = abs(length(p) - 0.200000) - 0.008000;
        float glow_pulse = (2.500000 + (discharge * 1.500000)) * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.500000, 0.600000, 1.000000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.870000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        float sdf_result = abs(length(p) - 0.450000) - 0.003000;
        float glow_pulse = 0.700000 * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.200000, 0.250000, 0.400000), color_result.a);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
        final_color = vec4(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
...
`;
const UNIFORMS = [{name:'voltage',default:0.6},{name:'discharge',default:0}];
const COMPLEXITY = {layers:7,fbmOctaves:27,passes:3,memory:true,compute:false,is3d:false,tier:'extreme'};
const PASS_WGSL_0 = `// Post-processing pass: energy
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'voltage', target: 'arc_core', field: 'brightness', weight: 0.5 },
      { source: 'voltage', target: 'lightning_a', field: 'intensity', weight: 0.4 },
      { source: 'discharge', target: 'crackle', field: 'brightness', weight: 0.6 },
      { source: 'discharge', target: 'lightning_b', field: 'intensity', weight: 0.3 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class GameArcTimeline {
  constructor() {
    ...
    this._entries = [
      { target: 'discharge', from: 0, to: 1, duration: 4, easing: 'ease_out' },
    ];
  }
...
class PlasmaStorm extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('glyph-plasma-storm: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get voltage() { return this._renderer?.userParams['voltage'] ?? this._pendingParams['voltage'] ?? 0.6; }
  set voltage(v) { this.setParam('voltage', v); }
  get discharge() { return this._renderer?.userParams['discharge'] ?? this._pendingParams['discharge'] ?? 0; }
  set discharge(v) { this.setParam('discharge', v); }
...
customElements.define('glyph-plasma-storm', PlasmaStorm);
})();
