/**
 * GAME Compiler Wrapper
 *
 * Spawns the GAME compiler CLI to compile .game source code into
...
// Path to the GAME compiler Cargo project root
...
/**
 * Compile raw GAME source code into a Web Component.
 *
...
export async function compileGameSource(
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'game-mcp-'));
  const srcFile = path.join(tmpDir, 'input.game');
  ...
  try {
    ...
    if (jsFile) {
      ...
      // Extract component name from filename (e.g., "game-input.js" -> "game-input")
...
/**
 * Invoke the GAME compiler as a child process.
 */
...
export async function validateGameSource(source: string): Promise<string | null> {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'game-validate-'));
  const srcFile = path.join(tmpDir, 'input.game');
  fs.writeFileSync(srcFile, source, 'utf-8');
...
/**
 * Generate a standalone HTML page wrapping a compiled GAME component.
 */
function generateHtmlWrapper(componentName: string, jsContent: string): string {
  const tagName = componentName.startsWith('game-') ? componentName : `game-${componentName}`;
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
