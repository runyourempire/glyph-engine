 ...
 * GAME Code Generator
 *
 * Uses the Anthropic SDK to generate GAME shader code from natural
 * language descriptions. The system prompt encodes the complete
 * GAME language reference so the LLM produces valid, compilable code.
 */

import Anthropic from '@anthropic-ai/sdk';
import { compileGameSource, type CompileResult } from './compiler.js';

const SYSTEM_PROMPT = `You are a GAME shader programmer. GAME is a DSL that compiles to WebGPU/WebGL2 Web Components. Every .game file produces a zero-dependency custom HTML element that renders at 60fps on the GPU.
...
OUTPUT: Only the raw .game source code. No explanation. No markdown fences. No commentary.`;
...
/**
 * Generate a GAME visual from a natural language description.
 *
 * Pipeline: description -> LLM generates .game code -> compiler produces Web Component
 */
export async function generateGameVisual(description: string): Promise<GenerateResult> {
  const apiKey = process.env.ANTHROPIC_API_KEY;
  if (!apiKey) {
    throw new Error(
      'ANTHROPIC_API_KEY environment variable is required for game_render. ' +
  ...
  // Generate GAME code from natural language
  const message = await client.messages.create({
    ...
    messages: [
      {
        role: 'user',
        content: `Create a GAME visual for: ${description}\n\nOutput only the .game source code.`,
      },
    ],
  });

  // Extract text from response
  const gameSource = message.content
  ...
  if (!gameSource) {
    throw new Error('LLM returned empty response — no GAME code generated');
  }

  // Strip markdown fences if the LLM wrapped them despite instructions
  const cleanSource = gameSource
    .replace(/^```(?:game)?\s*\n?/m, '')
    .replace(/\n?```\s*$/m, '')
    .trim();
  ...
  try {
    ...
    throw new Error(
      `Generated GAME code failed to compile.\n\n` +
      `Source:\n${cleanSource}\n\n` +
