...
const LIVING_WALLPAPER_PATTERNS = [
  '-living.glyph',
  'living-landscape.glyph',
  'living-photo.glyph',
  'ocean-alive.glyph',
  'yosemite-alive.glyph',
  'ocean-video-test.glyph',
  'video-test.glyph',
];
...
/**
 * Extract a one-line description from the first comment in a .glyph file.
 */
...
function generatePreviewHtml(componentName: string, source: string): string {
  const tagName = `glyph-${componentName}`;
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
<body>
  <${tagName}></${tagName}>
  <!-- Compile this .glyph source with: game build input.glyph -o dist --format html -->
 ...
 * Filters out living wallpaper files and non-.glyph files.
 */
