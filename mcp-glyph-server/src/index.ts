 ...
 * GLYPH MCP Server
 *
 * Exposes the GLYPH compiler to AI agents via the Model Context Protocol.
 * Four tools:
 *   glyph_compile  — compile raw .glyph source to Web Component JS + HTML
 *   glyph_render   — generate GLYPH code from natural language, compile, return result
 *   glyph_gallery  — browse pre-built showcase components with source
 *   glyph_builtins — list all available builtin functions with signatures
 */
...
const TOOLS = [
  {
    name: 'glyph_compile',
    description:
      'Compile raw GLYPH source code into a GPU-rendered Web Component. ' +
      'Returns the compiled JavaScript, a standalone HTML page, component name, and bundle size. ' +
      'GLYPH is a DSL that compiles to WebGPU/WebGL2 shaders packaged as zero-dependency custom HTML elements.',
    inputSchema: {
      ...
      properties: {
        source: {
          type: 'string',
          description:
            'Raw GLYPH source code (the contents of a .glyph file). ' +
  ...
  },
  {
    ...
    description:
      ...
      'Uses Claude Sonnet to write GLYPH shader code, then compiles it into a working Web Component. ' +
      'Returns the generated .glyph source, compiled HTML page, and component name. ' +
  ...
  },
  {
    name: 'glyph_gallery',
    description:
      'Browse pre-built showcase GLYPH components with their source code and descriptions. ' +
      'Returns procedural/generative visuals (aurora, nebula, void engine, etc). ' +
      'Each component includes compilable .glyph source code you can modify or learn from.',
    inputSchema: {
      type: 'object' as const,
      properties: {
        category: {
  ...
  },
  {
    name: 'glyph_builtins',
    description:
      'List all available GLYPH builtin functions with their signatures, input/output states, ' +
      'and descriptions. Essential reference for writing valid GLYPH code — every stage must ' +
...
async function handleCompile(args: Record<string, unknown> | undefined) {
  ...
  if (typeof source !== 'string' || !source.trim()) {
    return {
      content: [{ type: 'text', text: 'Error: "source" parameter is required and must be non-empty GLYPH source code.' }],
      isError: true,
    };
  }
        ...
        text: JSON.stringify(
          {
            ...
            game_source: result.glyph_source,
            html: result.html,
          },
          null,
          2
...
async function main() {
  ...
  process.stderr.write('GLYPH MCP Server running on stdio\n');
}
