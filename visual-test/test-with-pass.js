// GAME Component: test-with-pass — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    color_result = vec4<f32>(min(pixel.rgb + color_result.rgb, vec3<f32>(1.0)), 1.0);
    return color_result;
}
`;
...
class TestWithPass extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-test-with-pass: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
...
customElements.define('game-test-with-pass', TestWithPass);
})();
