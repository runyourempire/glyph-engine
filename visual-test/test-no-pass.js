// GAME Component: test-no-pass — auto-generated, do not edit.
(function(){
...
void main(){
    ...
    // ── Layer 1: ring ──
    {
        ...
        vec4 color_result = vec4(vec3(glow_result), 1.0);
        color_result = vec4(color_result.rgb * vec3(0.500000, 0.400000, 0.200000), 1.0);
        vec3 lc = color_result.rgb;
        final_color = vec4(final_color.rgb + lc, 1.0);
    }
...
class TestNoPass extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-test-no-pass: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
...
customElements.define('game-test-no-pass', TestNoPass);
})();
