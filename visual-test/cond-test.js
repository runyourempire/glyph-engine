// GAME Component: cond-test — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 1: bg ──
    {
        ...
        var color_result: vec4<f32>;
        {
            var p_then = p;
            { var p = p_then;
            var sdf_result = sdf_circle(p, 0.300000);
            let glow_pulse = 3.000000 * (0.9 + 0.1 * sin(time * 2.0));
            let glow_result = apply_glow(sdf_result, glow_pulse);
            var color_result = vec4<f32>(vec3<f32>(glow_result), 1.0);
            color_result = vec4<f32>(color_result.rgb * vec3<f32>(1.000000, 0.000000, 0.000000), 1.0);
            ...
            let glow_result = apply_glow(sdf_result, glow_pulse);
            var color_result = vec4<f32>(vec3<f32>(glow_result), 1.0);
            color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.000000, 1.000000, 0.000000), 1.0);
            var else_color = color_result; }
            color_result = select(else_color, then_color, (u.audio_energy > 0.500000));
        }
...
void main(){
    ...
    // ── Layer 1: bg ──
    {
        ...
        vec4 color_result;
        {
            ...
            color_result = vec4(color_result.rgb * vec3(1.000000, 0.000000, 0.000000), 1.0);
            ...
            vec4 color_result = vec4(vec3(glow_result), 1.0);
            color_result = vec4(color_result.rgb * vec3(0.000000, 1.000000, 0.000000), 1.0);
            vec4 else_color = color_result; }
            color_result = (u_audio_energy > 0.500000) ? then_color : else_color;
        }
...
`;
const UNIFORMS = [{name:'energy',default:0.5}];
...
class CondTest extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-cond-test: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get energy() { return this._renderer?.userParams['energy'] ?? 0; }
  set energy(v) { this.setParam('energy', v); }
...
customElements.define('game-cond-test', CondTest);
})();
