// GAME Component: boot-ring — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 1: track ──
    {
        ...
        var sdf_result = abs(length(p) - 0.300000) - 0.012000;
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        var sdf_result = abs(length(p) - 0.300000) - 0.025000;
...
void main(){
    ...
    // ── Layer 1: track ──
    {
        ...
        float sdf_result = abs(length(p) - 0.300000) - 0.012000;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        float sdf_result = abs(length(p) - 0.300000) - 0.025000;
...
`;
const UNIFORMS = [{name:'charcoal',default:0.3},{name:'fill_angle',default:0},{name:'glow_str',default:2},{name:'gold',default:0.83}];
...
class BootRing extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-boot-ring: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
...
customElements.define('game-boot-ring', BootRing);
})();
