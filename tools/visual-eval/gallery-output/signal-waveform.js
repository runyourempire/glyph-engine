// GAME Component: signal-waveform — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 1: base ──
    {
        ...
        var sdf_result = fbm2((p * 4.000000 + vec2<f32>(time * 0.1, time * 0.07)), i32(3.000000), 0.400000, 2.000000);
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.55, 0.42, 0.15), vec3<f32>(0.3, 0.25, 0.1), vec3<f32>(0.8, 0.6, 0.4), vec3<f32>(0.0, 0.1, 0.2));
...
void main(){
    ...
    // ── Layer 1: base ──
    {
        ...
        float sdf_result = fbm2((p * 4.000000 + vec2(time * 0.1, time * 0.07)), int(3.000000), 0.400000, 2.000000);
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.55, 0.42, 0.15), vec3(0.3, 0.25, 0.1), vec3(0.8, 0.6, 0.4), vec3(0.0, 0.1, 0.2));
...
`;
const UNIFORMS = [{name:'signal',default:0.6}];
const COMPLEXITY = {layers:2,fbmOctaves:8,passes:1,memory:true,compute:false,is3d:false,tier:'medium'};
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'signal', target: 'pulse', field: 'brightness', weight: 0.5 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class SignalWaveform extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-signal-waveform: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get signal() { return this._renderer?.userParams['signal'] ?? this._pendingParams['signal'] ?? 0.6; }
...
customElements.define('game-signal-waveform', SignalWaveform);
})();
