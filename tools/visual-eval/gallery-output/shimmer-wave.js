// GAME Component: shimmer-wave — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 0: wave ──
    {
        ...
        { let warp_x = fbm2(p * 1.500000 + vec2<f32>(0.0, 1.3), i32(3.000000), 0.100000, 2.000000);
        let warp_y = fbm2(p * 1.500000 + vec2<f32>(1.7, 0.0), i32(3.000000), 0.100000, 2.000000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.100000; }
        p = p + vec2<f32>(sin(p.y * 3.000000 + time * 0.400000), cos(p.x * 3.000000 + time * 0.400000)) * 0.150000;
...
void main(){
    ...
    // ── Layer 0: wave ──
    {
        ...
        { float warp_x = fbm2(p * 1.500000 + vec2(0.0, 1.3), int(3.000000), 0.100000, 2.000000);
        float warp_y = fbm2(p * 1.500000 + vec2(1.7, 0.0), int(3.000000), 0.100000, 2.000000);
        p = p + vec2(warp_x, warp_y) * 0.100000; }
        p = p + vec2(sin(p.y * 3.000000 + time * 0.400000), cos(p.x * 3.000000 + time * 0.400000)) * 0.150000;
...
`;
const UNIFORMS = [];
const COMPLEXITY = {layers:2,fbmOctaves:8,passes:0,memory:true,compute:false,is3d:false,tier:'medium'};
...
class ShimmerWave extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-shimmer-wave: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
...
customElements.define('game-shimmer-wave', ShimmerWave);
})();
