// GAME Component: crystal-lattice — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 0: lattice ──
    {
        ...
        { let warp_x = fbm2(p * 3.000000 + vec2<f32>(0.0, 1.3), i32(3.000000), 0.120000, 2.000000);
        let warp_y = fbm2(p * 3.000000 + vec2<f32>(1.7, 0.0), i32(3.000000), 0.120000, 2.000000);
...
void main(){
    ...
    // ── Layer 0: lattice ──
    {
        ...
        { float warp_x = fbm2(p * 3.000000 + vec2(0.0, 1.3), int(3.000000), 0.120000, 2.000000);
        float warp_y = fbm2(p * 3.000000 + vec2(1.7, 0.0), int(3.000000), 0.120000, 2.000000);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p + vec2(sin(p.y * 2.000000 + time * 0.200000), cos(p.x * 2.000000 + time * 0.200000)) * 0.080000;
        p = vec2(length(p), atan(p.y, p.x));
...
`;
const UNIFORMS = [];
const COMPLEXITY = {layers:2,fbmOctaves:3,passes:1,memory:true,compute:false,is3d:false,tier:'medium'};
...
class CrystalLattice extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-crystal-lattice: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
...
customElements.define('game-crystal-lattice', CrystalLattice);
})();
