// GAME Component: ember-field — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 0: current ──
    {
        ...
        { let warp_x = fbm2(p * 3.000000 + vec2<f32>(0.0, 1.3), i32(4.000000), 0.500000, 0.300000);
        let warp_y = fbm2(p * 3.000000 + vec2<f32>(1.7, 0.0), i32(4.000000), 0.500000, 0.300000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.300000; }
        p = p + vec2<f32>(sin(p.y * 2.000000 + time * 0.350000), cos(p.x * 2.000000 + time * 0.350000)) * 0.200000;
        var sdf_result = fbm2((p * 5.000000 + vec2<f32>(time * 0.1, time * 0.07)), i32(4.000000), 0.400000, 2.000000);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let warp_x = fbm2(p * 5.000000 + vec2<f32>(0.0, 1.3), i32(3.000000), 0.200000, 2.000000);
        let warp_y = fbm2(p * 5.000000 + vec2<f32>(1.7, 0.0), i32(3.000000), 0.200000, 2.000000);
...
void main(){
    ...
    // ── Layer 0: current ──
    {
        ...
        { float warp_x = fbm2(p * 3.000000 + vec2(0.0, 1.3), int(4.000000), 0.500000, 0.300000);
        float warp_y = fbm2(p * 3.000000 + vec2(1.7, 0.0), int(4.000000), 0.500000, 0.300000);
        p = p + vec2(warp_x, warp_y) * 0.300000; }
        p = p + vec2(sin(p.y * 2.000000 + time * 0.350000), cos(p.x * 2.000000 + time * 0.350000)) * 0.200000;
        float sdf_result = fbm2((p * 5.000000 + vec2(time * 0.1, time * 0.07)), int(4.000000), 0.400000, 2.000000);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        { float warp_x = fbm2(p * 5.000000 + vec2(0.0, 1.3), int(3.000000), 0.200000, 2.000000);
        float warp_y = fbm2(p * 5.000000 + vec2(1.7, 0.0), int(3.000000), 0.200000, 2.000000);
...
`;
const UNIFORMS = [];
const COMPLEXITY = {layers:2,fbmOctaves:11,passes:1,memory:true,compute:false,is3d:false,tier:'heavy'};
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-ember-field: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
...
customElements.define('game-ember-field', EmberField);
})();
