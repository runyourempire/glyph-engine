// GAME Component: achievement-arc — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 1: track ──
    {
        ...
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.300000, 0.300000, 0.300000), color_result.a);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let warp_x = fbm2(p * 4.000000 + vec2<f32>(0.0, 1.3), i32(2.000000), 0.060000, 2.000000);
        let warp_y = fbm2(p * 4.000000 + vec2<f32>(1.7, 0.0), i32(2.000000), 0.060000, 2.000000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.060000; }
        var sdf_result = abs(length(p) - 0.340000) - 0.030000;
...
void main(){
    ...
    // ── Layer 1: track ──
    {
        ...
        float sdf_result = abs(length(p) - 0.340000) - 0.008000;
        float glow_pulse = 0.500000 * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.300000, 0.300000, 0.300000), color_result.a);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        { float warp_x = fbm2(p * 4.000000 + vec2(0.0, 1.3), int(2.000000), 0.060000, 2.000000);
        float warp_y = fbm2(p * 4.000000 + vec2(1.7, 0.0), int(2.000000), 0.060000, 2.000000);
        p = p + vec2(warp_x, warp_y) * 0.060000; }
        float sdf_result = abs(length(p) - 0.340000) - 0.030000;
...
`;
const UNIFORMS = [{name:'fill_angle',default:5.3},{name:'glow_val',default:2}];
const COMPLEXITY = {layers:3,fbmOctaves:2,passes:1,memory:true,compute:false,is3d:false,tier:'medium'};
...
class AchievementArc extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-achievement-arc: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get fill_angle() { return this._renderer?.userParams['fill_angle'] ?? this._pendingParams['fill_angle'] ?? 5.3; }
  set fill_angle(v) { this.setParam('fill_angle', v); }
  get glow_val() { return this._renderer?.userParams['glow_val'] ?? this._pendingParams['glow_val'] ?? 2; }
...
customElements.define('game-achievement-arc', AchievementArc);
})();
