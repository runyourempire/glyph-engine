// GAME Component: hover-glow — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 1: glow_core ──
    {
        ...
        { let warp_x = fbm2(p * 2.000000 + vec2<f32>(0.0, 1.3), i32(2.000000), 0.050000, 2.000000);
        let warp_y = fbm2(p * 2.000000 + vec2<f32>(1.7, 0.0), i32(2.000000), 0.050000, 2.000000);
...
void main(){
    ...
    // ── Layer 1: glow_core ──
    {
        ...
        { float warp_x = fbm2(p * 2.000000 + vec2(0.0, 1.3), int(2.000000), 0.050000, 2.000000);
        float warp_y = fbm2(p * 2.000000 + vec2(1.7, 0.0), int(2.000000), 0.050000, 2.000000);
...
const COMPLEXITY = {layers:2,fbmOctaves:2,passes:0,memory:true,compute:false,is3d:false,tier:'light'};
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'intensity', target: 'glow_core', field: 'brightness', weight: 0.6 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class HoverGlow extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-hover-glow: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
...
customElements.define('game-hover-glow', HoverGlow);
})();
