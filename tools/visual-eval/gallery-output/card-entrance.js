// GAME Component: card-entrance — auto-generated, do not edit.
(function(){
...
`;
const UNIFORMS = [{name:'reveal',default:0}];
const COMPLEXITY = {layers:2,fbmOctaves:0,passes:1,memory:false,compute:false,is3d:false,tier:'light'};
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'reveal', target: 'glow_edge', field: 'brightness', weight: 0.8 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class GameArcTimeline {
  constructor() {
    ...
    this._entries = [
      { target: 'reveal', from: 0, to: 1, duration: 2, easing: 'ease_out' },
    ];
  }
...
class CardEntrance extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-card-entrance: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get reveal() { return this._renderer?.userParams['reveal'] ?? this._pendingParams['reveal'] ?? 0; }
  set reveal(v) { this.setParam('reveal', v); }
...
customElements.define('game-card-entrance', CardEntrance);
})();
