// GAME Component: quantum-mesh — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 0: mesh ──
    {
        ...
        { let warp_x = fbm2(p * 2.000000 + vec2<f32>(0.0, 1.3), i32(5.000000), 0.600000, 0.200000);
        let warp_y = fbm2(p * 2.000000 + vec2<f32>(1.7, 0.0), i32(5.000000), 0.600000, 0.200000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.200000; }
        p = p + vec2<f32>(sin(p.y * 1.500000 + time * 0.150000), cos(p.x * 1.500000 + time * 0.150000)) * 0.100000;
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let warp_x = fbm2(p * 4.000000 + vec2<f32>(0.0, 1.3), i32(2.000000), 0.100000, 2.000000);
        let warp_y = fbm2(p * 4.000000 + vec2<f32>(1.7, 0.0), i32(2.000000), 0.100000, 2.000000);
...
void main(){
    ...
    // ── Layer 0: mesh ──
    {
        ...
        { float warp_x = fbm2(p * 2.000000 + vec2(0.0, 1.3), int(5.000000), 0.600000, 0.200000);
        float warp_y = fbm2(p * 2.000000 + vec2(1.7, 0.0), int(5.000000), 0.600000, 0.200000);
        p = p + vec2(warp_x, warp_y) * 0.200000; }
        p = p + vec2(sin(p.y * 1.500000 + time * 0.150000), cos(p.x * 1.500000 + time * 0.150000)) * 0.100000;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        { float warp_x = fbm2(p * 4.000000 + vec2(0.0, 1.3), int(2.000000), 0.100000, 2.000000);
        float warp_y = fbm2(p * 4.000000 + vec2(1.7, 0.0), int(2.000000), 0.100000, 2.000000);
...
`;
const UNIFORMS = [];
const COMPLEXITY = {layers:2,fbmOctaves:7,passes:1,memory:true,compute:false,is3d:false,tier:'medium'};
...
class QuantumMesh extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-quantum-mesh: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
...
customElements.define('game-quantum-mesh', QuantumMesh);
})();
