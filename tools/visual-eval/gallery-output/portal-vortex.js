// GAME Component: portal-vortex — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 0: outer ──
    {
        ...
        { let ra = time * 0.150000; let rc = cos(ra); let rs = sin(ra);
    ...
    // ── Layer 1: inner ──
    {
        ...
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.800000, 0.400000, 1.000000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    // ── Layer 2: center ──
    {
        ...
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(1.000000, 0.950000, 0.700000), color_result.a);
        let la = color_result.a;
        let lc = color_result.rgb;
        final_color = vec4<f32>(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
...
void main(){
    ...
    // ── Layer 0: outer ──
    {
        ...
        { float ra = time * 0.150000; float rc = cos(ra); float rs = sin(ra);
    ...
    // ── Layer 1: inner ──
    {
        ...
        color_result = vec4(color_result.rgb * vec3(0.800000, 0.400000, 1.000000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.900000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
...
`;
const UNIFORMS = [];
const COMPLEXITY = {layers:3,fbmOctaves:13,passes:1,memory:true,compute:false,is3d:false,tier:'heavy'};
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let vign = 1.0 - 0.700000 * length(uv - 0.5);
...
class PortalVortex extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-portal-vortex: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
...
customElements.define('game-portal-vortex', PortalVortex);
})();
