// GAME Component: aurora — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 0: curtain ──
    {
        ...
        { let warp_x = fbm2(p * 2.000000 + vec2<f32>(0.0, 1.3), i32(4.000000), 0.500000, 0.250000);
        let warp_y = fbm2(p * 2.000000 + vec2<f32>(1.7, 0.0), i32(4.000000), 0.500000, 0.250000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.250000; }
        p = p + vec2<f32>(sin(p.y * 1.500000 + time * 0.300000), cos(p.x * 1.500000 + time * 0.300000)) * 0.150000;
        var sdf_result = fbm2((p * 3.000000 + vec2<f32>(time * 0.1, time * 0.07)), i32(5.000000), 0.600000, 2.000000);
    ...
    // ── Layer 1: violet ──
    {
        ...
        { let warp_x = fbm2(p * 1.800000 + vec2<f32>(0.0, 1.3), i32(3.000000), 0.200000, 2.000000);
        let warp_y = fbm2(p * 1.800000 + vec2<f32>(1.7, 0.0), i32(3.000000), 0.200000, 2.000000);
        ...
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.5, 0.3, 0.6), vec3<f32>(0.5, 0.3, 0.4), vec3<f32>(1.0, 1.0, 0.8), vec3<f32>(0.3, 0.2, 0.0));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
...
void main(){
    ...
    // ── Layer 0: curtain ──
    {
        ...
        { float warp_x = fbm2(p * 2.000000 + vec2(0.0, 1.3), int(4.000000), 0.500000, 0.250000);
        float warp_y = fbm2(p * 2.000000 + vec2(1.7, 0.0), int(4.000000), 0.500000, 0.250000);
        p = p + vec2(warp_x, warp_y) * 0.250000; }
        p = p + vec2(sin(p.y * 1.500000 + time * 0.300000), cos(p.x * 1.500000 + time * 0.300000)) * 0.150000;
        float sdf_result = fbm2((p * 3.000000 + vec2(time * 0.1, time * 0.07)), int(5.000000), 0.600000, 2.000000);
    ...
    // ── Layer 1: violet ──
    {
        ...
        { float warp_x = fbm2(p * 1.800000 + vec2(0.0, 1.3), int(3.000000), 0.200000, 2.000000);
        float warp_y = fbm2(p * 1.800000 + vec2(1.7, 0.0), int(3.000000), 0.200000, 2.000000);
        p = p + vec2(warp_x, warp_y) * 0.200000; }
        float sdf_result = fbm2((p * 2.500000 + vec2(time * 0.1, time * 0.07)), int(4.000000), 0.500000, 2.000000);
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.5, 0.3, 0.6), vec3(0.5, 0.3, 0.4), vec3(1.0, 1.0, 0.8), vec3(0.3, 0.2, 0.0));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.920000);
        float la = color_result.a;
...
class Aurora extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-aurora: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
...
customElements.define('game-aurora', Aurora);
})();
