// GAME Component: signal-meter — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 1: bars ──
    {
        ...
        p = p + vec2<f32>(sin(p.y * 2.000000 + time * 0.500000), cos(p.x * 2.000000 + time * 0.500000)) * 0.080000;
        let gx = abs(game_mod(p.x + 0.150000 * 0.5, 0.150000) - 0.150000 * 0.5) - 0.020000;
        let gy = abs(game_mod(p.y + 0.150000 * 0.5, 0.150000) - 0.150000 * 0.5) - 0.020000;
        ...
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.300000, 0.800000, 0.500000), color_result.a);
    ...
    // ── Layer 2: glow_field ──
    {
        ...
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.200000, 0.600000, 0.400000), color_result.a);
...
void main(){
    ...
    // ── Layer 1: bars ──
    {
        ...
        p = p + vec2(sin(p.y * 2.000000 + time * 0.500000), cos(p.x * 2.000000 + time * 0.500000)) * 0.080000;
        float gx = abs(mod(p.x + 0.150000 * 0.5, 0.150000) - 0.150000 * 0.5) - 0.020000;
        float gy = abs(mod(p.y + 0.150000 * 0.5, 0.150000) - 0.150000 * 0.5) - 0.020000;
        ...
        color_result = vec4(color_result.rgb * vec3(0.300000, 0.800000, 0.500000), color_result.a);
    ...
    // ── Layer 2: glow_field ──
    {
        ...
        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.200000, 0.600000, 0.400000), color_result.a);
...
`;
const UNIFORMS = [{name:'strength',default:0.7}];
const COMPLEXITY = {layers:2,fbmOctaves:0,passes:0,memory:true,compute:false,is3d:false,tier:'light'};
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'strength', target: 'bars', field: 'brightness', weight: 0.6 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class SignalMeter extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-signal-meter: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get strength() { return this._renderer?.userParams['strength'] ?? this._pendingParams['strength'] ?? 0.7; }
  set strength(v) { this.setParam('strength', v); }
...
customElements.define('game-signal-meter', SignalMeter);
})();
