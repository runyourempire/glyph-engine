// GAME Component: wave-spectrum — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 0: spectrum ──
    {
        ...
        p = p + vec2<f32>(sin(p.y * 4.000000 + time * 1.000000), cos(p.x * 4.000000 + time * 1.000000)) * 0.150000;
        ...
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.500000, 0.300000, 1.000000), color_result.a);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        var sdf_result = fbm2((p * 3.000000 + vec2<f32>(time * 0.1, time * 0.07)), i32(2.000000), 0.400000, 2.000000);
        let glow_pulse = 0.500000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.300000, 0.150000, 0.600000), color_result.a);
        let la = color_result.a;
        let lc = color_result.rgb;
        final_color = vec4<f32>(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
...
void main(){
    ...
    // ── Layer 0: spectrum ──
    {
        ...
        p = p + vec2(sin(p.y * 4.000000 + time * 1.000000), cos(p.x * 4.000000 + time * 1.000000)) * 0.150000;
        ...
        color_result = vec4(color_result.rgb * vec3(0.500000, 0.300000, 1.000000), color_result.a);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        float sdf_result = fbm2((p * 3.000000 + vec2(time * 0.1, time * 0.07)), int(2.000000), 0.400000, 2.000000);
        float glow_pulse = 0.500000 * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.300000, 0.150000, 0.600000), color_result.a);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
        final_color = vec4(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
...
`;
const UNIFORMS = [];
const COMPLEXITY = {layers:2,fbmOctaves:5,passes:0,memory:false,compute:false,is3d:false,tier:'light'};
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-wave-spectrum: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
...
customElements.define('game-wave-spectrum', WaveSpectrum);
})();
