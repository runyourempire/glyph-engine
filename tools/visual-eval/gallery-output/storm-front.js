// GAME Component: storm-front — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 0: clouds ──
    {
        ...
        { let warp_x = fbm2(p * 1.500000 + vec2<f32>(0.0, 1.3), i32(4.000000), 0.500000, 0.150000);
        let warp_y = fbm2(p * 1.500000 + vec2<f32>(1.7, 0.0), i32(4.000000), 0.500000, 0.150000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.150000; }
        var sdf_result = fbm2((p * 2.000000 + vec2<f32>(time * 0.1, time * 0.07)), i32(5.000000), 0.600000, 2.000000);
        let glow_pulse = 0.120000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.040000, 0.030000, 0.080000), color_result.a);
    ...
    // ── Layer 1: lightning ──
    {
        ...
        p = p + vec2<f32>(sin(p.y * 3.000000 + time * 0.800000), cos(p.x * 3.000000 + time * 0.800000)) * 0.250000;
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let warp_x = fbm2(p * 4.000000 + vec2<f32>(0.0, 1.3), i32(2.000000), 0.350000, 2.000000);
        let warp_y = fbm2(p * 4.000000 + vec2<f32>(1.7, 0.0), i32(2.000000), 0.350000, 2.000000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.350000; }
        p = p + vec2<f32>(sin(p.y * 4.000000 + time * 1.200000), cos(p.x * 4.000000 + time * 1.200000)) * 0.300000;
        ...
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.500000, 0.200000, 0.900000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
...
`;
const UNIFORMS = [];
const COMPLEXITY = {layers:3,fbmOctaves:14,passes:1,memory:true,compute:false,is3d:false,tier:'heavy'};
...
class StormFront extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-storm-front: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
...
customElements.define('game-storm-front', StormFront);
})();
