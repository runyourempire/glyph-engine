// GAME Component: loading-orbit — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 0: orbit ──
    {
        ...
        { let ra = time * 2.000000; let rc = cos(ra); let rs = sin(ra);
        p = vec2<f32>(p.x * rc - p.y * rs, p.x * rs + p.y * rc); }
        p = p - vec2<f32>(0.220000, 0.000000);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        var sdf_result = abs(length(p) - 0.220000) - 0.005000;
    ...
    // ── Layer 2: center ──
    {
        ...
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.600000, 0.600000, 0.650000), color_result.a);
        let la = color_result.a;
        let lc = color_result.rgb;
        final_color = vec4<f32>(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
...
`;
const UNIFORMS = [];
const COMPLEXITY = {layers:3,fbmOctaves:0,passes:0,memory:true,compute:false,is3d:false,tier:'light'};
...
class LoadingOrbit extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-loading-orbit: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
...
customElements.define('game-loading-orbit', LoadingOrbit);
})();
