// GAME Component: flow-diagram — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 0: stream ──
    {
        ...
        { let warp_x = fbm2(p * 2.000000 + vec2<f32>(0.0, 1.3), i32(4.000000), 0.200000, 2.000000);
        let warp_y = fbm2(p * 2.000000 + vec2<f32>(1.7, 0.0), i32(4.000000), 0.200000, 2.000000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.200000; }
        p = p + vec2<f32>(sin(p.y * 1.500000 + time * 0.400000), cos(p.x * 1.500000 + time * 0.400000)) * 0.120000;
    ...
    // ── Layer 1: nodes ──
    {
        ...
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.200000, 0.800000, 0.600000), color_result.a);
        let la = color_result.a;
        let lc = color_result.rgb;
        final_color = vec4<f32>(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
...
void main(){
    ...
    // ── Layer 0: stream ──
    {
        ...
        { float warp_x = fbm2(p * 2.000000 + vec2(0.0, 1.3), int(4.000000), 0.200000, 2.000000);
        float warp_y = fbm2(p * 2.000000 + vec2(1.7, 0.0), int(4.000000), 0.200000, 2.000000);
        p = p + vec2(warp_x, warp_y) * 0.200000; }
        p = p + vec2(sin(p.y * 1.500000 + time * 0.400000), cos(p.x * 1.500000 + time * 0.400000)) * 0.120000;
    ...
    // ── Layer 1: nodes ──
    {
        ...
        color_result = vec4(color_result.rgb * vec3(0.200000, 0.800000, 0.600000), color_result.a);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
        final_color = vec4(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
...
class FlowDiagram extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-flow-diagram: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
...
customElements.define('game-flow-diagram', FlowDiagram);
})();
