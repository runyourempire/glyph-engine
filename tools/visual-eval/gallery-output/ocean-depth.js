// GAME Component: ocean-depth — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 0: deep ──
    {
        ...
        { let warp_x = fbm2(p * 1.200000 + vec2<f32>(0.0, 1.3), i32(3.000000), 0.400000, 0.120000);
        let warp_y = fbm2(p * 1.200000 + vec2<f32>(1.7, 0.0), i32(3.000000), 0.400000, 0.120000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.120000; }
        p = p + vec2<f32>(sin(p.y * 0.800000 + time * 0.100000), cos(p.x * 0.800000 + time * 0.100000)) * 0.080000;
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let warp_x = fbm2(p * 2.500000 + vec2<f32>(0.0, 1.3), i32(4.000000), 0.180000, 2.000000);
        let warp_y = fbm2(p * 2.500000 + vec2<f32>(1.7, 0.0), i32(4.000000), 0.180000, 2.000000);
    ...
    // ── Layer 2: biolum ──
    {
        ...
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.100000, 0.600000, 0.900000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
...
void main(){
    ...
    // ── Layer 0: deep ──
    {
        ...
        { float warp_x = fbm2(p * 1.200000 + vec2(0.0, 1.3), int(3.000000), 0.400000, 0.120000);
        float warp_y = fbm2(p * 1.200000 + vec2(1.7, 0.0), int(3.000000), 0.400000, 0.120000);
        p = p + vec2(warp_x, warp_y) * 0.120000; }
        p = p + vec2(sin(p.y * 0.800000 + time * 0.100000), cos(p.x * 0.800000 + time * 0.100000)) * 0.080000;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        { float warp_x = fbm2(p * 2.500000 + vec2(0.0, 1.3), int(4.000000), 0.180000, 2.000000);
        float warp_y = fbm2(p * 2.500000 + vec2(1.7, 0.0), int(4.000000), 0.180000, 2.000000);
    ...
    // ── Layer 2: biolum ──
    {
        ...
        color_result = vec4(color_result.rgb * vec3(0.100000, 0.600000, 0.900000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.880000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
...
`;
const UNIFORMS = [];
const COMPLEXITY = {layers:3,fbmOctaves:11,passes:1,memory:true,compute:false,is3d:false,tier:'heavy'};
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let vign = 1.0 - 0.600000 * length(uv - 0.5);
...
class OceanDepth extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-ocean-depth: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
...
customElements.define('game-ocean-depth', OceanDepth);
})();
