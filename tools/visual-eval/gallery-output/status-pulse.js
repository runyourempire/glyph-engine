...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 1: core ──
    {
        ...
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.130000, 0.770000, 0.370000), color_result.a);
    ...
    // ── Layer 2: outer ──
    {
        ...
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.100000, 0.400000, 0.200000), color_result.a);
        let la = color_result.a;
        let lc = color_result.rgb;
        final_color = vec4<f32>(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
...
void main(){
    ...
    // ── Layer 1: core ──
    {
        ...
        color_result = vec4(color_result.rgb * vec3(0.130000, 0.770000, 0.370000), color_result.a);
    ...
    // ── Layer 2: outer ──
    {
        ...
        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.100000, 0.400000, 0.200000), color_result.a);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
        final_color = vec4(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
...
`;
const UNIFORMS = [{name:'active',default:1}];
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'active', target: 'core', field: 'brightness', weight: 0.5 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class StatusPulse extends HTMLElement {
  ...
  get active() { return this._renderer?.userParams['active'] ?? this._pendingParams['active'] ?? 1; }
  set active(v) { this.setParam('active', v); }
