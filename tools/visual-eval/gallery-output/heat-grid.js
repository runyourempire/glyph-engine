// GAME Component: heat-grid — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 0: grid_cells ──
    {
        ...
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.5, 0.3, 0.15), vec3<f32>(0.5, 0.35, 0.2), vec3<f32>(1.0, 1.0, 0.7), vec3<f32>(0.0, 0.15, 0.3));
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        let gx = abs(game_mod(p.x + 0.120000 * 0.5, 0.120000) - 0.120000 * 0.5) - 0.003000;
        let gy = abs(game_mod(p.y + 0.120000 * 0.5, 0.120000) - 0.120000 * 0.5) - 0.003000;
...
void main(){
    ...
    // ── Layer 0: grid_cells ──
    {
        ...
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.5, 0.3, 0.15), vec3(0.5, 0.35, 0.2), vec3(1.0, 1.0, 0.7), vec3(0.0, 0.15, 0.3));
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        float gx = abs(mod(p.x + 0.120000 * 0.5, 0.120000) - 0.120000 * 0.5) - 0.003000;
        float gy = abs(mod(p.y + 0.120000 * 0.5, 0.120000) - 0.120000 * 0.5) - 0.003000;
...
`;
const UNIFORMS = [];
const COMPLEXITY = {layers:2,fbmOctaves:3,passes:0,memory:false,compute:false,is3d:false,tier:'light'};
...
class HeatGrid extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-heat-grid: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
...
customElements.define('game-heat-grid', HeatGrid);
})();
