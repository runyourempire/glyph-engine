// GAME Component: void-drift — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 0: drift ──
    {
        ...
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.080000, 0.060000, 0.120000), color_result.a);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p + vec2<f32>(sin(p.y * 1.000000 + time * 0.100000), cos(p.x * 1.000000 + time * 0.100000)) * 0.050000;
        var sdf_result = voronoi2(p * 20.000000 + vec2<f32>(time * 0.05, time * 0.03));
        let glow_pulse = 0.400000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.150000, 0.120000, 0.200000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
...
void main(){
    ...
    // ── Layer 0: drift ──
    {
        ...
        color_result = vec4(color_result.rgb * vec3(0.080000, 0.060000, 0.120000), color_result.a);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p + vec2(sin(p.y * 1.000000 + time * 0.100000), cos(p.x * 1.000000 + time * 0.100000)) * 0.050000;
        float sdf_result = voronoi2(p * 20.000000 + vec2(time * 0.05, time * 0.03));
        float glow_pulse = 0.400000 * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.150000, 0.120000, 0.200000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.900000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
...
class VoidDrift extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-void-drift: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
...
customElements.define('game-void-drift', VoidDrift);
})();
