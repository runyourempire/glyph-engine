// GAME Component: dismiss-fade — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 1: content ──
    {
        ...
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.500000, 0.500000, 0.500000), color_result.a);
...
void main(){
    ...
    // ── Layer 1: content ──
    {
        ...
        color_result = vec4(color_result.rgb * vec3(0.500000, 0.500000, 0.500000), color_result.a);
...
`;
const UNIFORMS = [{name:'fade',default:1}];
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'fade', target: 'content', field: 'brightness', weight: 0.8 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class DismissFade extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-dismiss-fade: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get fade() { return this._renderer?.userParams['fade'] ?? this._pendingParams['fade'] ?? 1; }
  set fade(v) { this.setParam('fade', v); }
...
customElements.define('game-dismiss-fade', DismissFade);
})();
