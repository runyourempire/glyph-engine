...
/**
 * `<game-status-pulse>` Web Component
 *
 * A self-contained WebGPU/WebGL2 shader component.
 *
 * @example
 * ```html
 * <game-status-pulse active="1"></game-status-pulse>
 * ```
 *
 * @example
 * ```typescript
 * const el = document.querySelector('game-status-pulse')!;
   * el.active = 1;
 * ```
 */
interface GameStatusPulseElement extends HTMLElement {
...
declare global {
  interface HTMLElementTagNameMap {
    'game-status-pulse': GameStatusPulseElement;
  }
}
