// GAME Component: lightning-arc — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 0: bolt ──
    {
        ...
        { let warp_x = fbm2(p * 4.000000 + vec2<f32>(0.0, 1.3), i32(3.000000), 0.400000, 2.000000);
        let warp_y = fbm2(p * 4.000000 + vec2<f32>(1.7, 0.0), i32(3.000000), 0.400000, 2.000000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.400000; }
        p = p + vec2<f32>(sin(p.y * 5.000000 + time * 2.000000), cos(p.x * 5.000000 + time * 2.000000)) * 0.300000;
        var sdf_result = voronoi2(p * 5.000000 + vec2<f32>(time * 0.05, time * 0.03));
        sdf_result = abs(sdf_result) - 0.004000;
        let glow_pulse = 5.000000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.600000, 0.700000, 1.000000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
        color_result = mix(color_result, prev_color, 0.600000);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        var sdf_result = fbm2((p * 6.000000 + vec2<f32>(time * 0.1, time * 0.07)), i32(3.000000), 0.500000, 2.000000);
...
void main(){
    ...
    // ── Layer 0: bolt ──
    {
        ...
        { float warp_x = fbm2(p * 4.000000 + vec2(0.0, 1.3), int(3.000000), 0.400000, 2.000000);
        float warp_y = fbm2(p * 4.000000 + vec2(1.7, 0.0), int(3.000000), 0.400000, 2.000000);
        p = p + vec2(warp_x, warp_y) * 0.400000; }
        p = p + vec2(sin(p.y * 5.000000 + time * 2.000000), cos(p.x * 5.000000 + time * 2.000000)) * 0.300000;
        ...
        color_result = vec4(color_result.rgb * vec3(0.600000, 0.700000, 1.000000), color_result.a);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        float sdf_result = fbm2((p * 6.000000 + vec2(time * 0.1, time * 0.07)), int(3.000000), 0.500000, 2.000000);
...
`;
const UNIFORMS = [];
const COMPLEXITY = {layers:2,fbmOctaves:6,passes:0,memory:true,compute:false,is3d:false,tier:'medium'};
...
class LightningArc extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-lightning-arc: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
...
customElements.define('game-lightning-arc', LightningArc);
})();
