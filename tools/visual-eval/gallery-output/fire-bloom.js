// GAME Component: fire-bloom — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 0: flame ──
    {
        ...
        { let warp_x = fbm2(p * 2.500000 + vec2<f32>(0.0, 1.3), i32(5.000000), 0.600000, 0.350000);
        let warp_y = fbm2(p * 2.500000 + vec2<f32>(1.7, 0.0), i32(5.000000), 0.600000, 0.350000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.350000; }
        p = p + vec2<f32>(sin(p.y * 2.000000 + time * 1.000000), cos(p.x * 2.000000 + time * 1.000000)) * 0.200000;
        var sdf_result = fbm2((p * 3.000000 + vec2<f32>(time * 0.1, time * 0.07)), i32(4.000000), 0.500000, 2.000000);
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.5, 0.3, 0.1), vec3<f32>(0.5, 0.2, 0.1), vec3<f32>(1.0, 1.0, 1.0), vec3<f32>(0.0, 0.25, 0.25));
...
`;
const UNIFORMS = [];
const COMPLEXITY = {layers:2,fbmOctaves:9,passes:2,memory:true,compute:false,is3d:false,tier:'heavy'};
...
class FireBloom extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-fire-bloom: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
...
customElements.define('game-fire-bloom', FireBloom);
})();
