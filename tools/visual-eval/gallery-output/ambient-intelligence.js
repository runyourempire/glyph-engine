...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 0: field ──
    {
        ...
        { let warp_x = fbm2(p * 1.500000 + vec2<f32>(0.0, 1.3), i32(4.000000), 0.500000, 0.100000);
        let warp_y = fbm2(p * 1.500000 + vec2<f32>(1.7, 0.0), i32(4.000000), 0.500000, 0.100000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.100000; }
        p = p + vec2<f32>(sin(p.y * 1.000000 + time * 0.150000), cos(p.x * 1.000000 + time * 0.150000)) * 0.060000;
        var sdf_result = fbm2((p * 2.000000 + vec2<f32>(time * 0.1, time * 0.07)), i32(5.000000), 0.500000, 2.000000);
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.050000, 0.060000, 0.100000), vec3<f32>(0.100000, 0.120000, 0.200000), vec3<f32>(1.000000, 1.000000, 1.000000), vec3<f32>(0.250000, 0.000000, 0.100000));
    ...
    // ── Layer 1: pulse ──
    {
        ...
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.080000, 0.120000, 0.200000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
...
void main(){
    ...
    // ── Layer 0: field ──
    {
        ...
        { float warp_x = fbm2(p * 1.500000 + vec2(0.0, 1.3), int(4.000000), 0.500000, 0.100000);
        float warp_y = fbm2(p * 1.500000 + vec2(1.7, 0.0), int(4.000000), 0.500000, 0.100000);
        p = p + vec2(warp_x, warp_y) * 0.100000; }
        p = p + vec2(sin(p.y * 1.000000 + time * 0.150000), cos(p.x * 1.000000 + time * 0.150000)) * 0.060000;
        float sdf_result = fbm2((p * 2.000000 + vec2(time * 0.1, time * 0.07)), int(5.000000), 0.500000, 2.000000);
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.050000, 0.060000, 0.100000), vec3(0.100000, 0.120000, 0.200000), vec3(1.000000, 1.000000, 1.000000), vec3(0.250000, 0.000000, 0.100000));
    ...
    // ── Layer 1: pulse ──
    {
        ...
        color_result = vec4(color_result.rgb * vec3(0.080000, 0.120000, 0.200000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.920000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
...
`;
const UNIFORMS = [];
const COMPLEXITY = {layers:2,fbmOctaves:9,passes:1,memory:true,compute:false,is3d:false,tier:'medium'};
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let vign = 1.0 - 0.800000 * length(uv - 0.5);
