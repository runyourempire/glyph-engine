// GAME Component: chaos-wind — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.000000, 0.010000, 0.020000), color_result.a);
...
void main(){
    ...
    color_result = vec4(color_result.rgb * vec3(0.000000, 0.010000, 0.020000), color_result.a);
...
fn cs_flow(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    // Perlin-based flow (not divergence-free)
    let angle = fbm(p) * 6.28318;
    let vel = vec2<f32>(cos(angle), sin(angle)) * params.strength;
...
class GameFlowField {
  ...
  dispatch(dt) {
    ...
    f[0] = 5; f[1] = 0.8; f[2] = 2; f[3] = this._time;
...
class ChaosWind extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-chaos-wind: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
...
customElements.define('game-chaos-wind', ChaosWind);
})();
