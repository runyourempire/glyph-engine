// GAME Component: living-campfire — auto-generated, do not edit.
(function(){
...
uniform sampler2D u_tex_mask_fire;
uniform sampler2D u_tex_mask_smoke;
...
void main(){
    ...
    // ── Layer 0: world ──
    {
        ...
        vec2 _px_orbit = vec2(sin(time * 0.040000), cos(time * 0.040000 * 0.7)) * 0.012000;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2((sin((time * 0.800000)) * 0.006000), (time * (-0.045000)));
        p = p + vec2(sin(p.y * 4.000000 + time * 5.000000), cos(p.x * 4.000000 + time * 5.000000)) * 0.140000;
        { float warp_x = fbm2(p * 1.500000 + vec2(0.0, 1.3), int(4.000000), 0.650000, 0.400000);
        float warp_y = fbm2(p * 1.500000 + vec2(1.7, 0.0), int(4.000000), 0.650000, 0.400000);
        ...
        color_result = vec4(color_result.rgb * vec3(1.000000, 0.750000, 0.150000), color_result.a);
        vec2 _mask_uv = vec2(v_uv.x, 1.0 - v_uv.y);
        float _mask_raw = texture(u_tex_mask_fire, _mask_uv).r;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2((sin((time * 0.500000)) * 0.008000), (time * (-0.030000)));
        p = p + vec2(sin(p.y * 2.500000 + time * 3.500000), cos(p.x * 2.500000 + time * 3.500000)) * 0.100000;
        { float warp_x = fbm2(p * 1.000000 + vec2(0.0, 1.3), int(5.000000), 0.600000, 0.320000);
        float warp_y = fbm2(p * 1.000000 + vec2(1.7, 0.0), int(5.000000), 0.600000, 0.320000);
        p = p + vec2(warp_x, warp_y) * 0.320000; }
        float sdf_result = fbm2((p * 0.800000 + vec2(time * 0.1, time * 0.07)), int(4.000000), 0.550000, 2.000000);
        float glow_pulse = 3.000000 * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(1.000000, 0.450000, 0.080000), color_result.a);
        vec2 _mask_uv = vec2(v_uv.x, 1.0 - v_uv.y);
        float _mask_raw = texture(u_tex_mask_fire, _mask_uv).r;
        float _mask_val = mix(_mask_raw, 1.0 - _mask_raw, 0.0);
        color_result = vec4(color_result.rgb * _mask_val, color_result.a * _mask_val);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2((sin((time * 1.200000)) * 0.004000), (time * (-0.060000)));
        p = p + vec2(sin(p.y * 6.000000 + time * 7.000000), cos(p.x * 6.000000 + time * 7.000000)) * 0.180000;
        float sdf_result = voronoi2(p * 8.000000 + vec2(time * 0.05, time * 0.03));
        float glow_pulse = 4.000000 * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(1.000000, 0.920000, 0.550000), color_result.a);
        vec2 _mask_uv = vec2(v_uv.x, 1.0 - v_uv.y);
        float _mask_raw = texture(u_tex_mask_fire, _mask_uv).r;
        float _mask_val = mix(_mask_raw, 1.0 - _mask_raw, 0.0);
        color_result = vec4(color_result.rgb * _mask_val, color_result.a * _mask_val);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2((sin((time * 0.300000)) * 0.012000), (time * (-0.015000)));
        p = p + vec2(sin(p.y * 18.000000 + time * 2.000000), cos(p.x * 18.000000 + time * 2.000000)) * 0.100000;
        float sdf_result = voronoi2(p * 30.000000 + vec2(time * 0.05, time * 0.03));
        float glow_pulse = 10.000000 * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(1.000000, 0.650000, 0.150000), color_result.a);
        vec2 _mask_uv = vec2(v_uv.x, 1.0 - v_uv.y);
        float _mask_raw = texture(u_tex_mask_smoke, _mask_uv).r;
        float _mask_val = mix(_mask_raw, 1.0 - _mask_raw, 0.0);
        color_result = vec4(color_result.rgb * _mask_val, color_result.a * _mask_val);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2((sin((time * 0.060000)) * 0.008000), (time * (-0.012000)));
        { float warp_x = fbm2(p * 0.600000 + vec2(0.0, 1.3), int(5.000000), 0.650000, 0.220000);
        float warp_y = fbm2(p * 0.600000 + vec2(1.7, 0.0), int(5.000000), 0.650000, 0.220000);
        p = p + vec2(warp_x, warp_y) * 0.220000; }
        float sdf_result = fbm2((p * 0.900000 + vec2(time * 0.1, time * 0.07)), int(5.000000), 0.550000, 2.000000);
        float glow_pulse = 0.800000 * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.550000, 0.500000, 0.480000), color_result.a);
        vec2 _mask_uv = vec2(v_uv.x, 1.0 - v_uv.y);
        float _mask_raw = texture(u_tex_mask_smoke, _mask_uv).r;
        float _mask_val = mix(_mask_raw, 1.0 - _mask_raw, 0.0);
        color_result = vec4(color_result.rgb * _mask_val, color_result.a * _mask_val);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p + vec2(sin(p.y * 1.500000 + time * 2.500000), cos(p.x * 1.500000 + time * 2.500000)) * 0.015000;
    ...
    // ── Layer 7: firelight ──
    {
        ...
        p = p + vec2(sin(p.y * 0.800000 + time * 3.500000), cos(p.x * 0.800000 + time * 3.500000)) * 0.040000;
        { float warp_x = fbm2(p * 0.400000 + vec2(0.0, 1.3), int(3.000000), 0.500000, 0.080000);
        float warp_y = fbm2(p * 0.400000 + vec2(1.7, 0.0), int(3.000000), 0.500000, 0.080000);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2((time * 0.000500), (time * 0.000200));
        p = p + vec2(sin(p.y * 20.000000 + time * 2.500000), cos(p.x * 20.000000 + time * 2.500000)) * 0.100000;
        float sdf_result = voronoi2(p * 38.000000 + vec2(time * 0.05, time * 0.03));
        float glow_pulse = 11.000000 * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.900000, 0.920000, 1.000000), color_result.a);
        vec2 _mask_uv = vec2(v_uv.x, 1.0 - v_uv.y);
...
`;
const UNIFORMS = [];
const COMPLEXITY = {layers:9,fbmOctaves:29,passes:3,memory:false,compute:false,is3d:false,tier:'extreme'};
const TEX_INDEX = {'photo': 0, 'depth': 1, 'mask_fire': 2, 'mask_smoke': 3, 'mask_sky': 4};
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let grain_val = (fract(sin(dot(grain_seed, vec2<f32>(12.9898, 78.233))) * 43758.5453) - 0.5) * 0.018000;
...
class GameRendererGL {
  ...
  init() {
    ...
    this._texLocs['mask_fire'] = gl.getUniformLocation(this.program, 'u_tex_mask_fire');
    this._texLocs['mask_smoke'] = gl.getUniformLocation(this.program, 'u_tex_mask_smoke');
  ...
  render() {
    ...
    if (this._texImages['mask_fire']) gl.bindTexture(gl.TEXTURE_2D, this._texImages['mask_fire']);
    gl.uniform1i(this._texLocs['mask_fire'], 4);
    gl.activeTexture(gl.TEXTURE0 + 5);
    if (this._texImages['mask_smoke']) gl.bindTexture(gl.TEXTURE_2D, this._texImages['mask_smoke']);
    gl.uniform1i(this._texLocs['mask_smoke'], 5);
...
class LivingCampfire extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-living-campfire: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
    ...
    this.loadTexture('photo', 'campfire.jpg').catch(e => console.warn('texture load failed:', e));
    this.loadTexture('depth', 'campfire-depth.png').catch(e => console.warn('texture load failed:', e));
    this.loadTexture('mask_fire', 'campfire-mask_fire.png').catch(e => console.warn('texture load failed:', e));
    this.loadTexture('mask_smoke', 'campfire-mask_smoke.png').catch(e => console.warn('texture load failed:', e));
    this.loadTexture('mask_sky', 'campfire-mask_sky.png').catch(e => console.warn('texture load failed:', e));
...
customElements.define('game-living-campfire', LivingCampfire);
})();
