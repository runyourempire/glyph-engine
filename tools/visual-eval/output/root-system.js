// GAME Component: root-system — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.008000, 0.012000, 0.004000), color_result.a);
...
`;
const UNIFORMS = [{name:'color_r',default:0.2},{name:'color_g',default:1.2},{name:'color_b',default:0.3}];
const COMPLEXITY = {layers:1,fbmOctaves:0,passes:1,memory:false,compute:true,is3d:false,tier:'medium'};
...
fn cs_agent(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    let sense_l = sample_trail(agent.pos.x + cos(agent.angle + 0.17453292519943295) * 20, agent.pos.y + sin(agent.angle + 0.17453292519943295) * 20);
    let sense_f = sample_trail(agent.pos.x + cos(agent.angle) * 20, agent.pos.y + sin(agent.angle) * 20);
    let sense_r = sample_trail(agent.pos.x + cos(agent.angle - 0.17453292519943295) * 20, agent.pos.y + sin(agent.angle - 0.17453292519943295) * 20);

    // Turn toward strongest pheromone
    if (sense_f >= sense_l && sense_f >= sense_r) {
        // Keep going forward
    } else if (sense_l > sense_r) {
        agent.angle += 0.17453292519943295;
    } else if (sense_r > sense_l) {
        agent.angle -= 0.17453292519943295;
    } else {
        agent.angle += (rng - 0.5) * 0.17453292519943295 * 2.0;
    }

    agent.pos.x += cos(agent.angle) * 0.8;
    agent.pos.y += sin(agent.angle) * 0.8;
    ...
    trail[dep_y * params.width + dep_x] += 3;
...
class GameSwarmSim {
  constructor(device, agentCode, trailCode) { this._count = 80000; this._w = 512; this._h = 512; this._device = device; this._agentCode = agentCode; this._trailCode = trailCode; }
  ...
  dispatch(dt) {
    ...
    f[0] = 0.17453292519943295; f[1] = 20; f[2] = 0.17453292519943295; f[3] = 0.8; f[4] = 3;
  ...
  get agentCount() { return 80000; }
}


class RootSystem extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-root-system: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get color_r() { return this._renderer?.userParams['color_r'] ?? this._pendingParams['color_r'] ?? 0.2; }
  ...
  get color_b() { return this._renderer?.userParams['color_b'] ?? this._pendingParams['color_b'] ?? 0.3; }
...
customElements.define('game-root-system', RootSystem);
})();
