// GAME Component: physarum-veins — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.000000, 0.000000, 0.000000), color_result.a);
...
void main(){
    ...
    color_result = vec4(color_result.rgb * vec3(0.000000, 0.000000, 0.000000), color_result.a);
...
fn cs_agent(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    let sense_l = sample_trail(agent.pos.x + cos(agent.angle + 0.39269908169872414) * 5, agent.pos.y + sin(agent.angle + 0.39269908169872414) * 5);
    let sense_f = sample_trail(agent.pos.x + cos(agent.angle) * 5, agent.pos.y + sin(agent.angle) * 5);
    let sense_r = sample_trail(agent.pos.x + cos(agent.angle - 0.39269908169872414) * 5, agent.pos.y + sin(agent.angle - 0.39269908169872414) * 5);

    // Turn toward strongest pheromone
    if (sense_f >= sense_l && sense_f >= sense_r) {
        // Keep going forward
    } else if (sense_l > sense_r) {
        agent.angle += 0.39269908169872414;
    } else if (sense_r > sense_l) {
        agent.angle -= 0.39269908169872414;
    } else {
        agent.angle += (rng - 0.5) * 0.39269908169872414 * 2.0;
    }

    agent.pos.x += cos(agent.angle) * 0.5;
    agent.pos.y += sin(agent.angle) * 0.5;
    ...
    trail[dep_y * params.width + dep_x] += 8;
...
fn cs_diffuse(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    trail_out[gid.y * params.width + gid.x] = (sum / 9.0) * 0.98;
}
`;
...
class GameSwarmSim {
  constructor(device, agentCode, trailCode) { this._count = 200000; this._w = 512; this._h = 512; this._device = device; this._agentCode = agentCode; this._trailCode = trailCode; }
  ...
  dispatch(dt) {
    ...
    f[0] = 0.39269908169872414; f[1] = 5; f[2] = 0.39269908169872414; f[3] = 0.5; f[4] = 8;
  ...
  get agentCount() { return 200000; }
}


class PhysarumVeins extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    const gpu = new GameRenderer(this._canvas, WGSL_V, WGSL_F, UNIFORMS, 'swarm');
    if (await gpu.init()) {
      this._renderer = gpu;
    } else {
      ...
      if (gl.init()) {
        ...
        console.warn('game-physarum-veins: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
...
customElements.define('game-physarum-veins', PhysarumVeins);
})();
