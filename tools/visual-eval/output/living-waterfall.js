// GAME Component: living-waterfall — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 0: world ──
    {
        ...
        let _px_orbit = vec2<f32>(sin(time * 0.050000), cos(time * 0.050000 * 0.7)) * 0.015000;
    ...
    // ── Layer 1: cascade ──
    {
        ...
        let _fm_flow = textureSample(flow_tex, flow_samp, _fm_uv).rg;
        let _fm_dir = (_fm_flow - vec2<f32>(0.5)) * 2.0 * 0.100000;
        let _fm_phase0 = fract(time * 0.350000);
        let _fm_phase1 = fract(time * 0.350000 + 0.5);
        ...
        let la = color_result.a * 0.940000;
        let lc = color_result.rgb * 0.940000;
        final_color = vec4<f32>(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>((time * 0.008000), (time * 0.065000));
        p = p + vec2<f32>(sin(p.y * 3.500000 + time * 3.000000), cos(p.x * 3.500000 + time * 3.000000)) * 0.080000;
        { let warp_x = fbm2(p * 2.000000 + vec2<f32>(0.0, 1.3), i32(4.000000), 0.600000, 0.350000);
        let warp_y = fbm2(p * 2.000000 + vec2<f32>(1.7, 0.0), i32(4.000000), 0.600000, 0.350000);
        ...
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.920000, 0.950000, 1.000000), color_result.a);
        let _mask_uv = vec2<f32>(input.uv.x, 1.0 - input.uv.y);
        let _mask_raw = textureSample(mask_water_tex, mask_water_samp, _mask_uv).r;
        let _mask_val = mix(_mask_raw, 1.0 - _mask_raw, 0.0);
        color_result = vec4<f32>(color_result.rgb * _mask_val, color_result.a * _mask_val);
        let la = color_result.a * 0.220000;
        let lc = color_result.rgb * 0.220000;
        final_color = vec4<f32>(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>((time * 0.012000), (time * 0.005000));
        p = p + vec2<f32>(sin(p.y * 5.000000 + time * 4.000000), cos(p.x * 5.000000 + time * 4.000000)) * 0.120000;
        var sdf_result = voronoi2(p * 10.000000 + vec2<f32>(time * 0.05, time * 0.03));
        let glow_pulse = 3.500000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.880000, 0.920000, 1.000000), color_result.a);
        let _mask_uv = vec2<f32>(input.uv.x, 1.0 - input.uv.y);
        let _mask_raw = textureSample(mask_water_tex, mask_water_samp, _mask_uv).r;
        let _mask_val = mix(_mask_raw, 1.0 - _mask_raw, 0.0);
        color_result = vec4<f32>(color_result.rgb * _mask_val, color_result.a * _mask_val);
        let la = color_result.a * 0.160000;
        let lc = color_result.rgb * 0.160000;
        final_color = vec4<f32>(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
    ...
    // ── Layer 4: pool_ripple ──
    {
        ...
        { let warp_x = fbm2(p * 4.000000 + vec2<f32>(0.0, 1.3), i32(3.000000), 0.150000, 2.000000);
        let warp_y = fbm2(p * 4.000000 + vec2<f32>(1.7, 0.0), i32(3.000000), 0.150000, 2.000000);
        ...
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.400000, 0.650000, 0.850000), color_result.a);
        let _mask_uv = vec2<f32>(input.uv.x, 1.0 - input.uv.y);
        let _mask_raw = textureSample(mask_water_tex, mask_water_samp, _mask_uv).r;
        let _mask_val = mix(_mask_raw, 1.0 - _mask_raw, 0.0);
        color_result = vec4<f32>(color_result.rgb * _mask_val, color_result.a * _mask_val);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>((sin((time * 0.080000)) * 0.005000), (time * (-0.018000)));
        { let warp_x = fbm2(p * 0.700000 + vec2<f32>(0.0, 1.3), i32(5.000000), 0.650000, 0.280000);
        let warp_y = fbm2(p * 0.700000 + vec2<f32>(1.7, 0.0), i32(5.000000), 0.650000, 0.280000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.280000; }
        ...
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.880000, 0.900000, 0.980000), color_result.a);
        let _mask_uv = vec2<f32>(input.uv.x, 1.0 - input.uv.y);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>((time * 0.004000), (sin((time * 0.050000)) * 0.003000));
        { let warp_x = fbm2(p * 0.500000 + vec2<f32>(0.0, 1.3), i32(4.000000), 0.550000, 0.160000);
        let warp_y = fbm2(p * 0.500000 + vec2<f32>(1.7, 0.0), i32(4.000000), 0.550000, 0.160000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.160000; }
        var sdf_result = fbm2((p * 0.600000 + vec2<f32>(time * 0.1, time * 0.07)), i32(4.000000), 0.500000, 2.000000);
        let glow_pulse = 1.500000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.820000, 0.860000, 0.950000), color_result.a);
        let _mask_uv = vec2<f32>(input.uv.x, 1.0 - input.uv.y);
        let _mask_raw = textureSample(depth_tex, depth_samp, _mask_uv).r;
    ...
    // ── Layer 7: rainbow ──
    {
        ...
        var sdf_result = fbm2((p * 0.350000 + vec2<f32>(time * 0.1, time * 0.07)), i32(3.000000), 0.550000, 2.000000);
        let glow_pulse = 3.000000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.850000, 0.550000, 0.950000), color_result.a);
        let _mask_uv = vec2<f32>(input.uv.x, 1.0 - input.uv.y);
        let _mask_raw = textureSample(depth_tex, depth_samp, _mask_uv).r;
        let _mask_val = mix(_mask_raw, 1.0 - _mask_raw, 0.0);
        color_result = vec4<f32>(color_result.rgb * _mask_val, color_result.a * _mask_val);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>((time * 0.003000), (time * 0.008000));
        p = p + vec2<f32>(sin(p.y * 12.000000 + time * 1.500000), cos(p.x * 12.000000 + time * 1.500000)) * 0.040000;
        var sdf_result = voronoi2(p * 22.000000 + vec2<f32>(time * 0.05, time * 0.03));
        let glow_pulse = 6.000000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.700000, 0.750000, 0.850000), color_result.a);
        let _mask_uv = vec2<f32>(input.uv.x, 1.0 - input.uv.y);
        let _mask_raw = textureSample(depth_tex, depth_samp, _mask_uv).r;
        let _mask_val = mix(_mask_raw, 1.0 - _mask_raw, 0.0);
        color_result = vec4<f32>(color_result.rgb * _mask_val, color_result.a * _mask_val);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p + vec2<f32>(sin(p.y * 0.200000 + time * 0.030000), cos(p.x * 0.200000 + time * 0.030000)) * 0.010000;
...
void main(){
    ...
    // ── Layer 0: world ──
    {
        ...
        vec2 _px_orbit = vec2(sin(time * 0.050000), cos(time * 0.050000 * 0.7)) * 0.015000;
    ...
    // ── Layer 1: cascade ──
    {
        ...
        vec2 _fm_dir = (_fm_flow - vec2(0.5)) * 2.0 * 0.100000;
        float _fm_phase0 = fract(time * 0.350000);
        float _fm_phase1 = fract(time * 0.350000 + 0.5);
        ...
        float la = color_result.a * 0.940000;
        vec3 lc = color_result.rgb * 0.940000;
        final_color = vec4(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2((time * 0.008000), (time * 0.065000));
        p = p + vec2(sin(p.y * 3.500000 + time * 3.000000), cos(p.x * 3.500000 + time * 3.000000)) * 0.080000;
        { float warp_x = fbm2(p * 2.000000 + vec2(0.0, 1.3), int(4.000000), 0.600000, 0.350000);
        float warp_y = fbm2(p * 2.000000 + vec2(1.7, 0.0), int(4.000000), 0.600000, 0.350000);
        ...
        color_result = vec4(color_result.rgb * vec3(0.920000, 0.950000, 1.000000), color_result.a);
        vec2 _mask_uv = vec2(v_uv.x, 1.0 - v_uv.y);
        float _mask_raw = texture(u_tex_mask_water, _mask_uv).r;
        float _mask_val = mix(_mask_raw, 1.0 - _mask_raw, 0.0);
        color_result = vec4(color_result.rgb * _mask_val, color_result.a * _mask_val);
        float la = color_result.a * 0.220000;
        vec3 lc = color_result.rgb * 0.220000;
        final_color = vec4(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2((time * 0.012000), (time * 0.005000));
        p = p + vec2(sin(p.y * 5.000000 + time * 4.000000), cos(p.x * 5.000000 + time * 4.000000)) * 0.120000;
        ...
        float _mask_val = mix(_mask_raw, 1.0 - _mask_raw, 0.0);
        color_result = vec4(color_result.rgb * _mask_val, color_result.a * _mask_val);
        float la = color_result.a * 0.160000;
        vec3 lc = color_result.rgb * 0.160000;
        final_color = vec4(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
    ...
    // ── Layer 4: pool_ripple ──
    {
        ...
        { float warp_x = fbm2(p * 4.000000 + vec2(0.0, 1.3), int(3.000000), 0.150000, 2.000000);
        float warp_y = fbm2(p * 4.000000 + vec2(1.7, 0.0), int(3.000000), 0.150000, 2.000000);
        ...
        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.400000, 0.650000, 0.850000), color_result.a);
        vec2 _mask_uv = vec2(v_uv.x, 1.0 - v_uv.y);
        float _mask_raw = texture(u_tex_mask_water, _mask_uv).r;
        float _mask_val = mix(_mask_raw, 1.0 - _mask_raw, 0.0);
        color_result = vec4(color_result.rgb * _mask_val, color_result.a * _mask_val);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2((sin((time * 0.080000)) * 0.005000), (time * (-0.018000)));
        { float warp_x = fbm2(p * 0.700000 + vec2(0.0, 1.3), int(5.000000), 0.650000, 0.280000);
        float warp_y = fbm2(p * 0.700000 + vec2(1.7, 0.0), int(5.000000), 0.650000, 0.280000);
        ...
        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.880000, 0.900000, 0.980000), color_result.a);
        vec2 _mask_uv = vec2(v_uv.x, 1.0 - v_uv.y);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2((time * 0.004000), (sin((time * 0.050000)) * 0.003000));
        { float warp_x = fbm2(p * 0.500000 + vec2(0.0, 1.3), int(4.000000), 0.550000, 0.160000);
        float warp_y = fbm2(p * 0.500000 + vec2(1.7, 0.0), int(4.000000), 0.550000, 0.160000);
        p = p + vec2(warp_x, warp_y) * 0.160000; }
        float sdf_result = fbm2((p * 0.600000 + vec2(time * 0.1, time * 0.07)), int(4.000000), 0.500000, 2.000000);
        float glow_pulse = 1.500000 * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.820000, 0.860000, 0.950000), color_result.a);
        vec2 _mask_uv = vec2(v_uv.x, 1.0 - v_uv.y);
        float _mask_raw = texture(u_tex_depth, _mask_uv).r;
    ...
    // ── Layer 7: rainbow ──
    {
        ...
        float sdf_result = fbm2((p * 0.350000 + vec2(time * 0.1, time * 0.07)), int(3.000000), 0.550000, 2.000000);
        float glow_pulse = 3.000000 * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.850000, 0.550000, 0.950000), color_result.a);
        vec2 _mask_uv = vec2(v_uv.x, 1.0 - v_uv.y);
        float _mask_raw = texture(u_tex_depth, _mask_uv).r;
        float _mask_val = mix(_mask_raw, 1.0 - _mask_raw, 0.0);
        color_result = vec4(color_result.rgb * _mask_val, color_result.a * _mask_val);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2((time * 0.003000), (time * 0.008000));
        p = p + vec2(sin(p.y * 12.000000 + time * 1.500000), cos(p.x * 12.000000 + time * 1.500000)) * 0.040000;
        float sdf_result = voronoi2(p * 22.000000 + vec2(time * 0.05, time * 0.03));
        float glow_pulse = 6.000000 * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.700000, 0.750000, 0.850000), color_result.a);
        vec2 _mask_uv = vec2(v_uv.x, 1.0 - v_uv.y);
        float _mask_raw = texture(u_tex_depth, _mask_uv).r;
        float _mask_val = mix(_mask_raw, 1.0 - _mask_raw, 0.0);
        color_result = vec4(color_result.rgb * _mask_val, color_result.a * _mask_val);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p + vec2(sin(p.y * 0.200000 + time * 0.030000), cos(p.x * 0.200000 + time * 0.030000)) * 0.010000;
...
`;
const UNIFORMS = [];
const COMPLEXITY = {layers:10,fbmOctaves:31,passes:3,memory:false,compute:false,is3d:false,tier:'extreme'};
const TEX_INDEX = {'photo': 0, 'depth': 1, 'flow': 2, 'mask_water': 3, 'mask_sky': 4};
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let grain_val = (fract(sin(dot(grain_seed, vec2<f32>(12.9898, 78.233))) * 43758.5453) - 0.5) * 0.012000;
...
class GameRendererGL {
  ...
  init() {
    ...
    this._texLocs['flow'] = gl.getUniformLocation(this.program, 'u_tex_flow');
  ...
  render() {
    ...
    if (this._texImages['flow']) gl.bindTexture(gl.TEXTURE_2D, this._texImages['flow']);
    gl.uniform1i(this._texLocs['flow'], 4);
...
class LivingWaterfall extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    const gpu = new GameRenderer(this._canvas, WGSL_V, WGSL_F, UNIFORMS, PASS_SHADERS, 5);
    if (await gpu.init()) {
      this._renderer = gpu;
    } else {
      ...
      if (gl.init()) {
        ...
        console.warn('game-living-waterfall: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
    ...
    this.loadTexture('photo', 'waterfall.jpg').catch(e => console.warn('texture load failed:', e));
    this.loadTexture('depth', 'waterfall-depth.png').catch(e => console.warn('texture load failed:', e));
    this.loadTexture('flow', 'waterfall-flow.png').catch(e => console.warn('texture load failed:', e));
    this.loadTexture('mask_water', 'waterfall-mask_water.png').catch(e => console.warn('texture load failed:', e));
    this.loadTexture('mask_sky', 'waterfall-mask_sky.png').catch(e => console.warn('texture load failed:', e));
...
customElements.define('game-living-waterfall', LivingWaterfall);
})();
