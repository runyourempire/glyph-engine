// GAME Component: living-test-landscape — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 1: world ──
    {
        ...
        let _px_orbit = vec2<f32>(sin(time * 0.200000), cos(time * 0.200000 * 0.7)) * 0.040000;
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p + vec2<f32>(sin(p.y * 2.000000 + time * 1.800000), cos(p.x * 2.000000 + time * 1.800000)) * 0.100000;
        ...
        let la = color_result.a * 0.950000;
        let lc = color_result.rgb * 0.950000;
        final_color = vec4<f32>(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>((time * 0.100000), (time * 0.050000));
        ...
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.500000, 0.750000, 1.000000), color_result.a);
        let _mask_uv = vec2<f32>(input.uv.x, 1.0 - input.uv.y);
        let _mask_raw = textureSample(mask_water_tex, mask_water_samp, _mask_uv).r;
        let _mask_val = mix(_mask_raw, 1.0 - _mask_raw, 0.0);
        color_result = vec4<f32>(color_result.rgb * _mask_val, color_result.a * _mask_val);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>((time * 0.120000), (time * 0.060000));
        p = p + vec2<f32>(sin(p.y * 8.000000 + time * 2.000000), cos(p.x * 8.000000 + time * 2.000000)) * 0.600000;
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p + vec2<f32>(sin(p.y * 0.300000 + time * 0.150000), cos(p.x * 0.300000 + time * 0.150000)) * 0.035000;
        let _tex_uv = clamp(vec2<f32>(p.x / aspect * 0.5 + 0.5, 1.0 - (p.y * 0.5 + 0.5)), vec2<f32>(0.0), vec2<f32>(1.0));
        var color_result = textureSample(photo_tex, photo_samp, _tex_uv);
        let _mask_uv = vec2<f32>(input.uv.x, 1.0 - input.uv.y);
        ...
        let _mask_val = mix(_mask_raw, 1.0 - _mask_raw, 0.0);
        color_result = vec4<f32>(color_result.rgb * _mask_val, color_result.a * _mask_val);
        let la = color_result.a * 0.900000;
        let lc = color_result.rgb * 0.900000;
        final_color = vec4<f32>(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>((time * drift_x), (sin((time * 0.120000)) * 0.020000));
        { let warp_x = fbm2(p * 0.700000 + vec2<f32>(0.0, 1.3), i32(5.000000), 0.650000, 0.250000);
        let warp_y = fbm2(p * 0.700000 + vec2<f32>(1.7, 0.0), i32(5.000000), 0.650000, 0.250000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.250000; }
        ...
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.920000, 0.880000, 0.820000), color_result.a);
        let _mask_uv = vec2<f32>(input.uv.x, 1.0 - input.uv.y);
        let _mask_raw = textureSample(depth_tex, depth_samp, _mask_uv).r;
        let _mask_val = mix(_mask_raw, 1.0 - _mask_raw, 1.000000);
        color_result = vec4<f32>(color_result.rgb * _mask_val, color_result.a * _mask_val);
        let la = color_result.a * 0.250000;
        let lc = color_result.rgb * 0.250000;
        final_color = vec4<f32>(1.0 - (1.0 - final_color.rgb) * (1.0 - lc), max(final_color.a, la));
    }
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>((time * 0.020000), (time * 0.005000));
        { let warp_x = fbm2(p * 0.350000 + vec2<f32>(0.0, 1.3), i32(3.000000), 0.500000, 0.160000);
        let warp_y = fbm2(p * 0.350000 + vec2<f32>(1.7, 0.0), i32(3.000000), 0.500000, 0.160000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.160000; }
        var sdf_result = fbm2((p * 0.300000 + vec2<f32>(time * 0.1, time * 0.07)), i32(4.000000), 0.500000, 2.000000);
    ...
    // ── Layer 8: godrays ──
    {
        ...
        p = p + vec2<f32>(sin(p.y * 0.200000 + time * 0.100000), cos(p.x * 0.200000 + time * 0.100000)) * 0.050000;
        let sdf_result = smoothstep(0.000000, 0.450000, length(p));
...
void main(){
    ...
    // ── Layer 1: world ──
    {
        ...
        vec2 _px_orbit = vec2(sin(time * 0.200000), cos(time * 0.200000 * 0.7)) * 0.040000;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p + vec2(sin(p.y * 2.000000 + time * 1.800000), cos(p.x * 2.000000 + time * 1.800000)) * 0.100000;
        ...
        float la = color_result.a * 0.950000;
        vec3 lc = color_result.rgb * 0.950000;
        final_color = vec4(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2((time * 0.100000), (time * 0.050000));
        { float warp_x = fbm2(p * 4.000000 + vec2(0.0, 1.3), int(3.000000), 0.250000, 2.000000);
        float warp_y = fbm2(p * 4.000000 + vec2(1.7, 0.0), int(3.000000), 0.250000, 2.000000);
        ...
        color_result = vec4(color_result.rgb * vec3(0.500000, 0.750000, 1.000000), color_result.a);
        vec2 _mask_uv = vec2(v_uv.x, 1.0 - v_uv.y);
        float _mask_raw = texture(u_tex_mask_water, _mask_uv).r;
        float _mask_val = mix(_mask_raw, 1.0 - _mask_raw, 0.0);
        color_result = vec4(color_result.rgb * _mask_val, color_result.a * _mask_val);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2((time * 0.120000), (time * 0.060000));
        p = p + vec2(sin(p.y * 8.000000 + time * 2.000000), cos(p.x * 8.000000 + time * 2.000000)) * 0.600000;
        ...
        float _mask_val = mix(_mask_raw, 1.0 - _mask_raw, 0.0);
        color_result = vec4(color_result.rgb * _mask_val, color_result.a * _mask_val);
        float la = color_result.a * 0.200000;
        vec3 lc = color_result.rgb * 0.200000;
        final_color = vec4(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p + vec2(sin(p.y * 0.300000 + time * 0.150000), cos(p.x * 0.300000 + time * 0.150000)) * 0.035000;
        vec2 _tex_uv = clamp(vec2(p.x / aspect * 0.5 + 0.5, 1.0 - (p.y * 0.5 + 0.5)), 0.0, 1.0);
        vec4 color_result = texture(u_tex_photo, _tex_uv);
        vec2 _mask_uv = vec2(v_uv.x, 1.0 - v_uv.y);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2((time * drift_x), (sin((time * 0.120000)) * 0.020000));
        { float warp_x = fbm2(p * 0.700000 + vec2(0.0, 1.3), int(5.000000), 0.650000, 0.250000);
        float warp_y = fbm2(p * 0.700000 + vec2(1.7, 0.0), int(5.000000), 0.650000, 0.250000);
        p = p + vec2(warp_x, warp_y) * 0.250000; }
        ...
        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.920000, 0.880000, 0.820000), color_result.a);
        vec2 _mask_uv = vec2(v_uv.x, 1.0 - v_uv.y);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2((time * 0.020000), (time * 0.005000));
        { float warp_x = fbm2(p * 0.350000 + vec2(0.0, 1.3), int(3.000000), 0.500000, 0.160000);
        float warp_y = fbm2(p * 0.350000 + vec2(1.7, 0.0), int(3.000000), 0.500000, 0.160000);
        p = p + vec2(warp_x, warp_y) * 0.160000; }
        float sdf_result = fbm2((p * 0.300000 + vec2(time * 0.1, time * 0.07)), int(4.000000), 0.500000, 2.000000);
        float glow_pulse = 2.000000 * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(1.000000, 0.880000, 0.650000), color_result.a);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2(0.000000, 0.350000);
        p = vec2(length(p), atan(p.y, p.x));
        p = p + vec2(sin(p.y * 0.200000 + time * 0.100000), cos(p.x * 0.200000 + time * 0.100000)) * 0.050000;
        float sdf_result = smoothstep(0.000000, 0.450000, length(p));
...
`;
const UNIFORMS = [{name:'drift_x',default:0.04}];
const COMPLEXITY = {layers:8,fbmOctaves:20,passes:3,memory:false,compute:false,is3d:false,tier:'extreme'};
...
class LivingTestLandscape extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-living-test-landscape: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
    ...
    this.loadTexture('photo', 'test-landscape.jpg').catch(e => console.warn('texture load failed:', e));
    this.loadTexture('depth', 'test-landscape-depth.png').catch(e => console.warn('texture load failed:', e));
    this.loadTexture('flow', 'test-landscape-flow.png').catch(e => console.warn('texture load failed:', e));
    this.loadTexture('mask_water', 'test-landscape-mask_water.png').catch(e => console.warn('texture load failed:', e));
    this.loadTexture('mask_sky', 'test-landscape-mask_sky.png').catch(e => console.warn('texture load failed:', e));
  ...
  get drift_x() { return this._renderer?.userParams['drift_x'] ?? this._pendingParams['drift_x'] ?? 0.04; }
  set drift_x(v) { this.setParam('drift_x', v); }
...
customElements.define('game-living-test-landscape', LivingTestLandscape);
})();
