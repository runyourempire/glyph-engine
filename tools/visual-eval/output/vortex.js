// GAME Component: vortex — auto-generated, do not edit.
(function(){
...
fn cs_flow(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    // Vortex field: rotational flow around center
    let center = vec2<f32>(0.5, 0.5);
    let diff = uv - center;
    let dist = max(length(diff), 0.01);
    let vortex = vec2<f32>(-diff.y, diff.x) / dist;
    let turbulence = fbm(p) * 0.5;
    let vel = (vortex + turbulence) * params.strength;
...
class GameFlowField {
  ...
  dispatch(dt) {
    ...
    f[0] = 2; f[1] = 0.3; f[2] = 1.5; f[3] = this._time;
...
class Vortex extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-vortex: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
...
customElements.define('game-vortex', Vortex);
})();
