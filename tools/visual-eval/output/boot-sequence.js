// GAME Component: boot-sequence — auto-generated, do not edit.
(function(){
...
void main(){
    ...
    // ── Layer 1: texture ──
    {
        ...
        { float warp_x = fbm2(p * 1.800000 + vec2(0.0, 1.3), int(3.000000), 0.100000, 2.000000);
        float warp_y = fbm2(p * 1.800000 + vec2(1.7, 0.0), int(3.000000), 0.100000, 2.000000);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        float sdf_result = abs(length(p) - 0.380000) - 0.008000;
        float glow_pulse = 0.800000 * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.150000, 0.150000, 0.150000), color_result.a);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
        final_color = vec4(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
    ...
    // ── Layer 3: fill ──
    {
        ...
        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.200000, 0.800000, 0.400000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.900000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        { float ra = time * fill_angle; float rc = cos(ra); float rs = sin(ra);
        p = vec2(p.x * rc - p.y * rs, p.x * rs + p.y * rc); }
        p = p - vec2(0.380000, 0.000000);
        float sdf_result = sdf_circle(p, 0.012000);
        float glow_pulse = 3.000000 * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.400000, 1.000000, 0.600000), color_result.a);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
        final_color = vec4(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
...
`;
const UNIFORMS = [{name:'fill_angle',default:0}];
const COMPLEXITY = {layers:4,fbmOctaves:6,passes:1,memory:true,compute:false,is3d:false,tier:'heavy'};
...
class BootSequence extends HTMLElement {
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-boot-sequence: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get fill_angle() { return this._renderer?.userParams['fill_angle'] ?? this._pendingParams['fill_angle'] ?? 0; }
...
customElements.define('game-boot-sequence', BootSequence);
})();
