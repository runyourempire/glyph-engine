// GAME Component: living-city-night — auto-generated, do not edit.
(function(){
...
@group(0) @binding(9) var mask_water_tex: texture_2d<f32>;
@group(0) @binding(10) var mask_water_samp: sampler;

@group(0) @binding(11) var mask_sky_tex: texture_2d<f32>;
@group(0) @binding(12) var mask_sky_samp: sampler;
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 0: world ──
    {
        ...
        let _px_orbit = vec2<f32>(sin(time * 0.040000), cos(time * 0.040000 * 0.7)) * 0.015000;
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p + vec2<f32>(sin(p.y * 1.200000 + time * 0.800000), cos(p.x * 1.200000 + time * 0.800000)) * 0.020000;
        ...
        let la = color_result.a * 0.880000;
        let lc = color_result.rgb * 0.880000;
        final_color = vec4<f32>(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }

    // ── Layer 2: neon_shimmer ──
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>((time * 0.008000), (time * 0.004000));
        p = p + vec2<f32>(sin(p.y * 3.000000 + time * 1.200000), cos(p.x * 3.000000 + time * 1.200000)) * 0.060000;
        { let warp_x = fbm2(p * 2.500000 + vec2<f32>(0.0, 1.3), i32(3.000000), 0.180000, 2.000000);
        let warp_y = fbm2(p * 2.500000 + vec2<f32>(1.7, 0.0), i32(3.000000), 0.180000, 2.000000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.180000; }
        ...
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.850000, 0.400000, 0.950000), color_result.a);
        let _mask_uv = vec2<f32>(input.uv.x, 1.0 - input.uv.y);
        let _mask_raw = textureSample(mask_water_tex, mask_water_samp, _mask_uv).r;
        let _mask_val = mix(_mask_raw, 1.0 - _mask_raw, 0.0);
        color_result = vec4<f32>(color_result.rgb * _mask_val, color_result.a * _mask_val);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>((time * 0.006000), (time * 0.003000));
        p = p + vec2<f32>(sin(p.y * 2.500000 + time * 0.900000), cos(p.x * 2.500000 + time * 0.900000)) * 0.050000;
        { let warp_x = fbm2(p * 2.000000 + vec2<f32>(0.0, 1.3), i32(3.000000), 0.150000, 2.000000);
        let warp_y = fbm2(p * 2.000000 + vec2<f32>(1.7, 0.0), i32(3.000000), 0.150000, 2.000000);
        ...
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(1.000000, 0.550000, 0.200000), color_result.a);
        let _mask_uv = vec2<f32>(input.uv.x, 1.0 - input.uv.y);
        let _mask_raw = textureSample(mask_water_tex, mask_water_samp, _mask_uv).r;
        let _mask_val = mix(_mask_raw, 1.0 - _mask_raw, 0.0);
        color_result = vec4<f32>(color_result.rgb * _mask_val, color_result.a * _mask_val);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>((time * 0.001000), (time * 0.001000));
        p = p + vec2<f32>(sin(p.y * 15.000000 + time * 2.000000), cos(p.x * 15.000000 + time * 2.000000)) * 0.080000;
        var sdf_result = voronoi2(p * 20.000000 + vec2<f32>(time * 0.05, time * 0.03));
        let glow_pulse = 8.000000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(1.000000, 0.900000, 0.650000), color_result.a);
        let _mask_uv = vec2<f32>(input.uv.x, 1.0 - input.uv.y);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>((sin((time * 0.100000)) * 0.006000), (time * (-0.015000)));
        { let warp_x = fbm2(p * 0.900000 + vec2<f32>(0.0, 1.3), i32(5.000000), 0.600000, 0.220000);
        let warp_y = fbm2(p * 0.900000 + vec2<f32>(1.7, 0.0), i32(5.000000), 0.600000, 0.220000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.220000; }
        var sdf_result = fbm2((p * 1.200000 + vec2<f32>(time * 0.1, time * 0.07)), i32(5.000000), 0.500000, 2.000000);
        let glow_pulse = 1.000000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.600000, 0.580000, 0.650000), color_result.a);
        let _mask_uv = vec2<f32>(input.uv.x, 1.0 - input.uv.y);
        let _mask_raw = textureSample(depth_tex, depth_samp, _mask_uv).r;
        let _mask_val = mix(_mask_raw, 1.0 - _mask_raw, 0.0);
        color_result = vec4<f32>(color_result.rgb * _mask_val, color_result.a * _mask_val);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>((time * 0.030000), (time * 0.500000));
        p = p + vec2<f32>(sin(p.y * 30.000000 + time * 4.000000), cos(p.x * 30.000000 + time * 4.000000)) * 0.060000;
        var sdf_result = voronoi2(p * 50.000000 + vec2<f32>(time * 0.05, time * 0.03));
        let glow_pulse = 9.000000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.600000, 0.650000, 0.800000), color_result.a);
        let la = color_result.a * 0.050000;
        let lc = color_result.rgb * 0.050000;
        final_color = vec4<f32>(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>((time * 0.020000), (sin((time * 0.150000)) * 0.002000));
        { let warp_x = fbm2(p * 1.500000 + vec2<f32>(0.0, 1.3), i32(2.000000), 0.080000, 2.000000);
        let warp_y = fbm2(p * 1.500000 + vec2<f32>(1.7, 0.0), i32(2.000000), 0.080000, 2.000000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.080000; }
        var sdf_result = fbm2((p * 0.500000 + vec2<f32>(time * 0.1, time * 0.07)), i32(2.000000), 0.500000, 2.000000);
        let glow_pulse = 4.000000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(1.000000, 0.800000, 0.400000), color_result.a);
        let _mask_uv = vec2<f32>(input.uv.x, 1.0 - input.uv.y);
        let _mask_raw = textureSample(depth_tex, depth_samp, _mask_uv).r;
        let _mask_val = mix(_mask_raw, 1.0 - _mask_raw, 0.0);
        color_result = vec4<f32>(color_result.rgb * _mask_val, color_result.a * _mask_val);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>((time * 0.006000), (time * 0.002000));
        { let warp_x = fbm2(p * 0.400000 + vec2<f32>(0.0, 1.3), i32(3.000000), 0.500000, 0.100000);
        let warp_y = fbm2(p * 0.400000 + vec2<f32>(1.7, 0.0), i32(3.000000), 0.500000, 0.100000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.100000; }
        var sdf_result = fbm2((p * 0.350000 + vec2<f32>(time * 0.1, time * 0.07)), i32(3.000000), 0.500000, 2.000000);
        let glow_pulse = 1.500000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.550000, 0.450000, 0.650000), color_result.a);
        let _mask_uv = vec2<f32>(input.uv.x, 1.0 - input.uv.y);
        let _mask_raw = textureSample(depth_tex, depth_samp, _mask_uv).r;
        let _mask_val = mix(_mask_raw, 1.0 - _mask_raw, 1.000000);
        color_result = vec4<f32>(color_result.rgb * _mask_val, color_result.a * _mask_val);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>((time * 0.008000), (time * 0.003000));
        { let warp_x = fbm2(p * 0.300000 + vec2<f32>(0.0, 1.3), i32(3.000000), 0.550000, 0.120000);
        let warp_y = fbm2(p * 0.300000 + vec2<f32>(1.7, 0.0), i32(3.000000), 0.550000, 0.120000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.120000; }
        var sdf_result = fbm2((p * 0.400000 + vec2<f32>(time * 0.1, time * 0.07)), i32(3.000000), 0.500000, 2.000000);
        let glow_pulse = 2.000000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.650000, 0.450000, 0.700000), color_result.a);
        let _mask_uv = vec2<f32>(input.uv.x, 1.0 - input.uv.y);
...
void main(){
    ...
    // ── Layer 0: world ──
    {
        ...
        vec2 _px_orbit = vec2(sin(time * 0.040000), cos(time * 0.040000 * 0.7)) * 0.015000;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p + vec2(sin(p.y * 1.200000 + time * 0.800000), cos(p.x * 1.200000 + time * 0.800000)) * 0.020000;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2((time * 0.008000), (time * 0.004000));
        p = p + vec2(sin(p.y * 3.000000 + time * 1.200000), cos(p.x * 3.000000 + time * 1.200000)) * 0.060000;
        { float warp_x = fbm2(p * 2.500000 + vec2(0.0, 1.3), int(3.000000), 0.180000, 2.000000);
        float warp_y = fbm2(p * 2.500000 + vec2(1.7, 0.0), int(3.000000), 0.180000, 2.000000);
        ...
        color_result = vec4(color_result.rgb * vec3(0.850000, 0.400000, 0.950000), color_result.a);
        vec2 _mask_uv = vec2(v_uv.x, 1.0 - v_uv.y);
        float _mask_raw = texture(u_tex_mask_water, _mask_uv).r;
        float _mask_val = mix(_mask_raw, 1.0 - _mask_raw, 0.0);
        color_result = vec4(color_result.rgb * _mask_val, color_result.a * _mask_val);
        float la = color_result.a * 0.150000;
        vec3 lc = color_result.rgb * 0.150000;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2((time * 0.006000), (time * 0.003000));
        p = p + vec2(sin(p.y * 2.500000 + time * 0.900000), cos(p.x * 2.500000 + time * 0.900000)) * 0.050000;
        ...
        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(1.000000, 0.550000, 0.200000), color_result.a);
        vec2 _mask_uv = vec2(v_uv.x, 1.0 - v_uv.y);
        float _mask_raw = texture(u_tex_mask_water, _mask_uv).r;
        float _mask_val = mix(_mask_raw, 1.0 - _mask_raw, 0.0);
        color_result = vec4(color_result.rgb * _mask_val, color_result.a * _mask_val);
    ...
    // ── Layer 4: windows ──
    {
        ...
        p = p + vec2(sin(p.y * 15.000000 + time * 2.000000), cos(p.x * 15.000000 + time * 2.000000)) * 0.080000;
        float sdf_result = voronoi2(p * 20.000000 + vec2(time * 0.05, time * 0.03));
        float glow_pulse = 8.000000 * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(1.000000, 0.900000, 0.650000), color_result.a);
        vec2 _mask_uv = vec2(v_uv.x, 1.0 - v_uv.y);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2((sin((time * 0.100000)) * 0.006000), (time * (-0.015000)));
        { float warp_x = fbm2(p * 0.900000 + vec2(0.0, 1.3), int(5.000000), 0.600000, 0.220000);
        float warp_y = fbm2(p * 0.900000 + vec2(1.7, 0.0), int(5.000000), 0.600000, 0.220000);
        p = p + vec2(warp_x, warp_y) * 0.220000; }
        float sdf_result = fbm2((p * 1.200000 + vec2(time * 0.1, time * 0.07)), int(5.000000), 0.500000, 2.000000);
        float glow_pulse = 1.000000 * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.600000, 0.580000, 0.650000), color_result.a);
        vec2 _mask_uv = vec2(v_uv.x, 1.0 - v_uv.y);
        float _mask_raw = texture(u_tex_depth, _mask_uv).r;
        float _mask_val = mix(_mask_raw, 1.0 - _mask_raw, 0.0);
        color_result = vec4(color_result.rgb * _mask_val, color_result.a * _mask_val);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2((time * 0.030000), (time * 0.500000));
        p = p + vec2(sin(p.y * 30.000000 + time * 4.000000), cos(p.x * 30.000000 + time * 4.000000)) * 0.060000;
        float sdf_result = voronoi2(p * 50.000000 + vec2(time * 0.05, time * 0.03));
        float glow_pulse = 9.000000 * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.600000, 0.650000, 0.800000), color_result.a);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2((time * 0.020000), (sin((time * 0.150000)) * 0.002000));
        { float warp_x = fbm2(p * 1.500000 + vec2(0.0, 1.3), int(2.000000), 0.080000, 2.000000);
        float warp_y = fbm2(p * 1.500000 + vec2(1.7, 0.0), int(2.000000), 0.080000, 2.000000);
        p = p + vec2(warp_x, warp_y) * 0.080000; }
        float sdf_result = fbm2((p * 0.500000 + vec2(time * 0.1, time * 0.07)), int(2.000000), 0.500000, 2.000000);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2((time * 0.006000), (time * 0.002000));
        { float warp_x = fbm2(p * 0.400000 + vec2(0.0, 1.3), int(3.000000), 0.500000, 0.100000);
        float warp_y = fbm2(p * 0.400000 + vec2(1.7, 0.0), int(3.000000), 0.500000, 0.100000);
        p = p + vec2(warp_x, warp_y) * 0.100000; }
        float sdf_result = fbm2((p * 0.350000 + vec2(time * 0.1, time * 0.07)), int(3.000000), 0.500000, 2.000000);
        float glow_pulse = 1.500000 * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.550000, 0.450000, 0.650000), color_result.a);
        vec2 _mask_uv = vec2(v_uv.x, 1.0 - v_uv.y);
        float _mask_raw = texture(u_tex_depth, _mask_uv).r;
        float _mask_val = mix(_mask_raw, 1.0 - _mask_raw, 1.000000);
        color_result = vec4(color_result.rgb * _mask_val, color_result.a * _mask_val);
    ...
    // ── Layer 9: sky_glow ──
    {
        ...
        { float warp_x = fbm2(p * 0.300000 + vec2(0.0, 1.3), int(3.000000), 0.550000, 0.120000);
        float warp_y = fbm2(p * 0.300000 + vec2(1.7, 0.0), int(3.000000), 0.550000, 0.120000);
        p = p + vec2(warp_x, warp_y) * 0.120000; }
        float sdf_result = fbm2((p * 0.400000 + vec2(time * 0.1, time * 0.07)), int(3.000000), 0.500000, 2.000000);
        float glow_pulse = 2.000000 * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.650000, 0.450000, 0.700000), color_result.a);
        vec2 _mask_uv = vec2(v_uv.x, 1.0 - v_uv.y);
...
`;
const UNIFORMS = [];
const COMPLEXITY = {layers:10,fbmOctaves:32,passes:3,memory:false,compute:false,is3d:false,tier:'extreme'};
const TEX_INDEX = {'photo': 0, 'depth': 1, 'mask_water': 2, 'mask_sky': 3};
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let grain_val = (fract(sin(dot(grain_seed, vec2<f32>(12.9898, 78.233))) * 43758.5453) - 0.5) * 0.020000;
...
class LivingCityNight extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    const gpu = new GameRenderer(this._canvas, WGSL_V, WGSL_F, UNIFORMS, PASS_SHADERS, 4);
    if (await gpu.init()) {
      this._renderer = gpu;
    } else {
      ...
      if (gl.init()) {
        ...
        console.warn('game-living-city-night: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
    ...
    this.loadTexture('photo', 'city-night.jpg').catch(e => console.warn('texture load failed:', e));
    this.loadTexture('depth', 'city-night-depth.png').catch(e => console.warn('texture load failed:', e));
    this.loadTexture('mask_water', 'city-night-mask_water.png').catch(e => console.warn('texture load failed:', e));
    this.loadTexture('mask_sky', 'city-night-mask_sky.png').catch(e => console.warn('texture load failed:', e));
...
customElements.define('game-living-city-night', LivingCityNight);
})();
