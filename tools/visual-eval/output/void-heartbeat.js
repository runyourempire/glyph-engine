// GAME Component: void-heartbeat — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 1: core ──
    {
        ...
        { let warp_x = fbm2(p * 3.000000 + vec2<f32>(0.0, 1.3), i32(2.000000), 0.060000, 2.000000);
        let warp_y = fbm2(p * 3.000000 + vec2<f32>(1.7, 0.0), i32(2.000000), 0.060000, 2.000000);
    ...
    // ── Layer 2: halo ──
    {
        ...
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.500000, 0.500000, 0.550000), color_result.a);
        let la = color_result.a;
        let lc = color_result.rgb;
        final_color = vec4<f32>(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
...
void main(){
    ...
    // ── Layer 1: core ──
    {
        ...
        { float warp_x = fbm2(p * 3.000000 + vec2(0.0, 1.3), int(2.000000), 0.060000, 2.000000);
        float warp_y = fbm2(p * 3.000000 + vec2(1.7, 0.0), int(2.000000), 0.060000, 2.000000);
    ...
    // ── Layer 2: halo ──
    {
        ...
        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.500000, 0.500000, 0.550000), color_result.a);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
        final_color = vec4(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
...
`;
const UNIFORMS = [{name:'alive',default:1}];
const COMPLEXITY = {layers:2,fbmOctaves:2,passes:1,memory:true,compute:false,is3d:false,tier:'medium'};
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'alive', target: 'core', field: 'brightness', weight: 0.5 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class VoidHeartbeat extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-void-heartbeat: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get alive() { return this._renderer?.userParams['alive'] ?? this._pendingParams['alive'] ?? 1; }
  set alive(v) { this.setParam('alive', v); }
...
customElements.define('game-void-heartbeat', VoidHeartbeat);
})();
