// GAME Component: living-aurora-mountains — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 0: world ──
    {
        ...
        let _px_orbit = vec2<f32>(sin(time * 0.080000), cos(time * 0.080000 * 0.7)) * 0.025000;
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p + vec2<f32>(sin(p.y * 0.350000 + time * 0.060000), cos(p.x * 0.350000 + time * 0.060000)) * 0.018000;
    ...
    // ── Layer 2: aurora_green ──
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>((time * 0.025000), (time * 0.004000));
        { let warp_x = fbm2(p * 0.500000 + vec2<f32>(0.0, 1.3), i32(5.000000), 0.700000, 0.350000);
        let warp_y = fbm2(p * 0.500000 + vec2<f32>(1.7, 0.0), i32(5.000000), 0.700000, 0.350000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.350000; }
        var sdf_result = fbm2((p * 0.700000 + vec2<f32>(time * 0.1, time * 0.07)), i32(5.000000), 0.600000, 2.000000);
        let glow_pulse = 2.500000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.300000, 1.000000, 0.500000), color_result.a);
        let _mask_uv = vec2<f32>(input.uv.x, 1.0 - input.uv.y);
        let _mask_raw = textureSample(mask_sky_tex, mask_sky_samp, _mask_uv).r;
        let _mask_val = mix(_mask_raw, 1.0 - _mask_raw, 0.0);
        color_result = vec4<f32>(color_result.rgb * _mask_val, color_result.a * _mask_val);
        let la = color_result.a * 0.280000;
        let lc = color_result.rgb * 0.280000;
        final_color = vec4<f32>(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }

    // ── Layer 3: aurora_purple ──
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>((time * 0.018000), (time * (-0.006000)));
        { let warp_x = fbm2(p * 0.400000 + vec2<f32>(0.0, 1.3), i32(4.000000), 0.650000, 0.280000);
        let warp_y = fbm2(p * 0.400000 + vec2<f32>(1.7, 0.0), i32(4.000000), 0.650000, 0.280000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.280000; }
        var sdf_result = fbm2((p * 0.550000 + vec2<f32>(time * 0.1, time * 0.07)), i32(4.000000), 0.550000, 2.000000);
        let glow_pulse = 3.500000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.650000, 0.250000, 1.000000), color_result.a);
        let _mask_uv = vec2<f32>(input.uv.x, 1.0 - input.uv.y);
        let _mask_raw = textureSample(mask_sky_tex, mask_sky_samp, _mask_uv).r;
        let _mask_val = mix(_mask_raw, 1.0 - _mask_raw, 0.0);
        color_result = vec4<f32>(color_result.rgb * _mask_val, color_result.a * _mask_val);
    ...
    // ── Layer 4: aurora_shimmer ──
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>((time * 0.040000), (time * 0.012000));
        p = p + vec2<f32>(sin(p.y * 0.800000 + time * 0.250000), cos(p.x * 0.800000 + time * 0.250000)) * 0.060000;
        { let warp_x = fbm2(p * 1.200000 + vec2<f32>(0.0, 1.3), i32(3.000000), 0.220000, 2.000000);
        let warp_y = fbm2(p * 1.200000 + vec2<f32>(1.7, 0.0), i32(3.000000), 0.220000, 2.000000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.220000; }
        var sdf_result = voronoi2(p * 5.000000 + vec2<f32>(time * 0.05, time * 0.03));
        let glow_pulse = 5.000000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.350000, 0.950000, 0.550000), color_result.a);
        let _mask_uv = vec2<f32>(input.uv.x, 1.0 - input.uv.y);
        let _mask_raw = textureSample(mask_sky_tex, mask_sky_samp, _mask_uv).r;
        let _mask_val = mix(_mask_raw, 1.0 - _mask_raw, 0.0);
        color_result = vec4<f32>(color_result.rgb * _mask_val, color_result.a * _mask_val);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>((time * 0.001000), (time * 0.000300));
        p = p + vec2<f32>(sin(p.y * 18.000000 + time * 2.500000), cos(p.x * 18.000000 + time * 2.500000)) * 0.120000;
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>((time * 0.010000), (time * 0.004000));
        { let warp_x = fbm2(p * 0.350000 + vec2<f32>(0.0, 1.3), i32(3.000000), 0.500000, 0.120000);
        let warp_y = fbm2(p * 0.350000 + vec2<f32>(1.7, 0.0), i32(3.000000), 0.500000, 0.120000);
        ...
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.250000, 0.750000, 0.450000), color_result.a);
        let _mask_uv = vec2<f32>(input.uv.x, 1.0 - input.uv.y);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>((time * 0.012000), (sin((time * 0.050000)) * 0.006000));
        { let warp_x = fbm2(p * 0.800000 + vec2<f32>(0.0, 1.3), i32(4.000000), 0.600000, 0.160000);
        let warp_y = fbm2(p * 0.800000 + vec2<f32>(1.7, 0.0), i32(4.000000), 0.600000, 0.160000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.160000; }
        var sdf_result = fbm2((p * 1.200000 + vec2<f32>(time * 0.1, time * 0.07)), i32(4.000000), 0.500000, 2.000000);
        let glow_pulse = 1.000000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.350000, 0.450000, 0.800000), color_result.a);
        let _mask_uv = vec2<f32>(input.uv.x, 1.0 - input.uv.y);
        let _mask_raw = textureSample(depth_tex, depth_samp, _mask_uv).r;
        let _mask_val = mix(_mask_raw, 1.0 - _mask_raw, 1.000000);
        color_result = vec4<f32>(color_result.rgb * _mask_val, color_result.a * _mask_val);
        let la = color_result.a * 0.090000;
        let lc = color_result.rgb * 0.090000;
        final_color = vec4<f32>(1.0 - (1.0 - final_color.rgb) * (1.0 - lc), max(final_color.a, la));
    }

    // ── Layer 8: aurora_pulse ──
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>((time * 0.006000), (time * 0.002000));
        { let warp_x = fbm2(p * 0.300000 + vec2<f32>(0.0, 1.3), i32(2.000000), 0.500000, 0.080000);
        let warp_y = fbm2(p * 0.300000 + vec2<f32>(1.7, 0.0), i32(2.000000), 0.500000, 0.080000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.080000; }
        var sdf_result = fbm2((p * 0.250000 + vec2<f32>(time * 0.1, time * 0.07)), i32(3.000000), 0.500000, 2.000000);
        let glow_pulse = 1.500000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.500000, 0.900000, 0.650000), color_result.a);
...
void main(){
    ...
    // ── Layer 0: world ──
    {
        ...
        vec2 _px_orbit = vec2(sin(time * 0.080000), cos(time * 0.080000 * 0.7)) * 0.025000;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p + vec2(sin(p.y * 0.350000 + time * 0.060000), cos(p.x * 0.350000 + time * 0.060000)) * 0.018000;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2((time * 0.025000), (time * 0.004000));
        ...
        float sdf_result = fbm2((p * 0.700000 + vec2(time * 0.1, time * 0.07)), int(5.000000), 0.600000, 2.000000);
        float glow_pulse = 2.500000 * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.300000, 1.000000, 0.500000), color_result.a);
        vec2 _mask_uv = vec2(v_uv.x, 1.0 - v_uv.y);
        float _mask_raw = texture(u_tex_mask_sky, _mask_uv).r;
        float _mask_val = mix(_mask_raw, 1.0 - _mask_raw, 0.0);
        color_result = vec4(color_result.rgb * _mask_val, color_result.a * _mask_val);
        float la = color_result.a * 0.280000;
        vec3 lc = color_result.rgb * 0.280000;
        final_color = vec4(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2((time * 0.018000), (time * (-0.006000)));
        { float warp_x = fbm2(p * 0.400000 + vec2(0.0, 1.3), int(4.000000), 0.650000, 0.280000);
        float warp_y = fbm2(p * 0.400000 + vec2(1.7, 0.0), int(4.000000), 0.650000, 0.280000);
        p = p + vec2(warp_x, warp_y) * 0.280000; }
        float sdf_result = fbm2((p * 0.550000 + vec2(time * 0.1, time * 0.07)), int(4.000000), 0.550000, 2.000000);
        float glow_pulse = 3.500000 * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.650000, 0.250000, 1.000000), color_result.a);
        vec2 _mask_uv = vec2(v_uv.x, 1.0 - v_uv.y);
        float _mask_raw = texture(u_tex_mask_sky, _mask_uv).r;
        float _mask_val = mix(_mask_raw, 1.0 - _mask_raw, 0.0);
        color_result = vec4(color_result.rgb * _mask_val, color_result.a * _mask_val);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2((time * 0.040000), (time * 0.012000));
        p = p + vec2(sin(p.y * 0.800000 + time * 0.250000), cos(p.x * 0.800000 + time * 0.250000)) * 0.060000;
        { float warp_x = fbm2(p * 1.200000 + vec2(0.0, 1.3), int(3.000000), 0.220000, 2.000000);
        float warp_y = fbm2(p * 1.200000 + vec2(1.7, 0.0), int(3.000000), 0.220000, 2.000000);
        ...
        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.350000, 0.950000, 0.550000), color_result.a);
        vec2 _mask_uv = vec2(v_uv.x, 1.0 - v_uv.y);
        float _mask_raw = texture(u_tex_mask_sky, _mask_uv).r;
        float _mask_val = mix(_mask_raw, 1.0 - _mask_raw, 0.0);
        color_result = vec4(color_result.rgb * _mask_val, color_result.a * _mask_val);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2((time * 0.001000), (time * 0.000300));
        p = p + vec2(sin(p.y * 18.000000 + time * 2.500000), cos(p.x * 18.000000 + time * 2.500000)) * 0.120000;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2((time * 0.010000), (time * 0.004000));
        { float warp_x = fbm2(p * 0.350000 + vec2(0.0, 1.3), int(3.000000), 0.500000, 0.120000);
        float warp_y = fbm2(p * 0.350000 + vec2(1.7, 0.0), int(3.000000), 0.500000, 0.120000);
        ...
        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.250000, 0.750000, 0.450000), color_result.a);
        vec2 _mask_uv = vec2(v_uv.x, 1.0 - v_uv.y);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2((time * 0.012000), (sin((time * 0.050000)) * 0.006000));
        { float warp_x = fbm2(p * 0.800000 + vec2(0.0, 1.3), int(4.000000), 0.600000, 0.160000);
        float warp_y = fbm2(p * 0.800000 + vec2(1.7, 0.0), int(4.000000), 0.600000, 0.160000);
        p = p + vec2(warp_x, warp_y) * 0.160000; }
        float sdf_result = fbm2((p * 1.200000 + vec2(time * 0.1, time * 0.07)), int(4.000000), 0.500000, 2.000000);
        float glow_pulse = 1.000000 * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.350000, 0.450000, 0.800000), color_result.a);
        vec2 _mask_uv = vec2(v_uv.x, 1.0 - v_uv.y);
        float _mask_raw = texture(u_tex_depth, _mask_uv).r;
    ...
    // ── Layer 8: aurora_pulse ──
    {
        ...
        { float warp_x = fbm2(p * 0.300000 + vec2(0.0, 1.3), int(2.000000), 0.500000, 0.080000);
        float warp_y = fbm2(p * 0.300000 + vec2(1.7, 0.0), int(2.000000), 0.500000, 0.080000);
        ...
        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.500000, 0.900000, 0.650000), color_result.a);
...
`;
const UNIFORMS = [];
const COMPLEXITY = {layers:9,fbmOctaves:40,passes:3,memory:false,compute:false,is3d:false,tier:'extreme'};
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let vign = 1.0 - 0.280000 * length(uv - 0.5);
...
class LivingAuroraMountains extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-living-aurora-mountains: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
    ...
    this.loadTexture('photo', 'aurora-mountains.jpg').catch(e => console.warn('texture load failed:', e));
    this.loadTexture('depth', 'aurora-mountains-depth.png').catch(e => console.warn('texture load failed:', e));
    this.loadTexture('mask_sky', 'aurora-mountains-mask_sky.png').catch(e => console.warn('texture load failed:', e));
...
customElements.define('game-living-aurora-mountains', LivingAuroraMountains);
})();
