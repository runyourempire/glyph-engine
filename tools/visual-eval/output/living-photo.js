// GAME Component: living-photo — auto-generated, do not edit.
(function(){
...
void main(){
    ...
    p = p + vec2(sin(p.y * 1.500000 + time * flow_speed), cos(p.x * 1.500000 + time * flow_speed)) * (flow_strength + (u_mouse_down * 0.030000));
    { float warp_x = fbm2(p * 1.200000 + vec2(0.0, 1.3), int(3.000000), 0.500000, (breath + (u_mouse_down * 0.020000)));
    float warp_y = fbm2(p * 1.200000 + vec2(1.7, 0.0), int(3.000000), 0.500000, (breath + (u_mouse_down * 0.020000)));
    p = p + vec2(warp_x, warp_y) * (breath + (u_mouse_down * 0.020000)); }
    vec2 _tex_uv = clamp(vec2(p.x / aspect * 0.5 + 0.5, 1.0 - (p.y * 0.5 + 0.5)), 0.0, 1.0);
    vec4 color_result = texture(u_tex_photo, _tex_uv);
    color_result = vec4(clamp(color_result.rgb, 0.0, 1.0), color_result.a);
    color_result += (dither_noise(v_uv * u_resolution) - 0.5) / 255.0;
    fragColor = color_result;
}
`;
const UNIFORMS = [{name:'flow_speed',default:0.2},{name:'flow_strength',default:0.04},{name:'breath',default:0.03}];
const COMPLEXITY = {layers:1,fbmOctaves:3,passes:0,memory:false,compute:false,is3d:false,tier:'light'};
const TEX_INDEX = {'photo': 0};
...
class LivingPhoto extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    const gpu = new GameRenderer(this._canvas, WGSL_V, WGSL_F, UNIFORMS, 1);
    if (await gpu.init()) {
      this._renderer = gpu;
    } else {
      ...
      if (gl.init()) {
        ...
        console.warn('game-living-photo: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
    ...
    this.loadTexture('photo', 'https://images.unsplash.com/photo-1506744038136-46273834b3fb?w=1920&q=80').catch(e => console.warn('texture load failed:', e));
  ...
  get flow_strength() { return this._renderer?.userParams['flow_strength'] ?? this._pendingParams['flow_strength'] ?? 0.04; }
  set flow_strength(v) { this.setParam('flow_strength', v); }
  get breath() { return this._renderer?.userParams['breath'] ?? this._pendingParams['breath'] ?? 0.03; }
...
customElements.define('game-living-photo', LivingPhoto);
})();
