// GAME Component: dendrite — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.010000, 0.003000, 0.015000), color_result.a);
...
void main(){
    ...
    color_result = vec4(color_result.rgb * vec3(0.010000, 0.003000, 0.015000), color_result.a);
...
`;
const UNIFORMS = [{name:'color_r',default:0.9},{name:'color_g',default:0.2},{name:'color_b',default:1.4}];
const COMPLEXITY = {layers:1,fbmOctaves:0,passes:2,memory:false,compute:true,is3d:false,tier:'medium'};
...
fn cs_agent(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    let sense_l = sample_trail(agent.pos.x + cos(agent.angle + 0.2617993877991494) * 12, agent.pos.y + sin(agent.angle + 0.2617993877991494) * 12);
    let sense_f = sample_trail(agent.pos.x + cos(agent.angle) * 12, agent.pos.y + sin(agent.angle) * 12);
    let sense_r = sample_trail(agent.pos.x + cos(agent.angle - 0.2617993877991494) * 12, agent.pos.y + sin(agent.angle - 0.2617993877991494) * 12);

    // Turn toward strongest pheromone
    if (sense_f >= sense_l && sense_f >= sense_r) {
        // Keep going forward
    } else if (sense_l > sense_r) {
        agent.angle += 0.5235987755982988;
    } else if (sense_r > sense_l) {
        agent.angle -= 0.5235987755982988;
    } else {
        agent.angle += (rng - 0.5) * 0.5235987755982988 * 2.0;
    }
    ...
    trail[dep_y * params.width + dep_x] += 4;
...
fn cs_diffuse(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    trail_out[gid.y * params.width + gid.x] = (sum / 9.0) * 0.96;
}
`;
...
class GameSwarmSim {
  constructor(device, agentCode, trailCode) { this._count = 120000; this._w = 512; this._h = 512; this._device = device; this._agentCode = agentCode; this._trailCode = trailCode; }
  ...
  dispatch(dt) {
    ...
    f[0] = 0.2617993877991494; f[1] = 12; f[2] = 0.5235987755982988; f[3] = 1; f[4] = 4;
  ...
  get agentCount() { return 120000; }
}


class Dendrite extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    const gpu = new GameRenderer(this._canvas, WGSL_V, WGSL_F, UNIFORMS, PASS_SHADERS, 'swarm');
    if (await gpu.init()) {
      this._renderer = gpu;
    } else {
      ...
      if (gl.init()) {
        ...
        console.warn('game-dendrite: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get color_r() { return this._renderer?.userParams['color_r'] ?? this._pendingParams['color_r'] ?? 0.9; }
  set color_r(v) { this.setParam('color_r', v); }
  get color_g() { return this._renderer?.userParams['color_g'] ?? this._pendingParams['color_g'] ?? 0.2; }
  set color_g(v) { this.setParam('color_g', v); }
  get color_b() { return this._renderer?.userParams['color_b'] ?? this._pendingParams['color_b'] ?? 1.4; }
...
customElements.define('game-dendrite', Dendrite);
})();
