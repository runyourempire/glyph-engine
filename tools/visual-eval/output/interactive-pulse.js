// GAME Component: interactive-pulse — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 1: field ──
    {
        ...
        { let warp_x = fbm2(p * 2.500000 + vec2<f32>(0.0, 1.3), i32(4.000000), 0.150000, 2.000000);
        let warp_y = fbm2(p * 2.500000 + vec2<f32>(1.7, 0.0), i32(4.000000), 0.150000, 2.000000);
    ...
    // ── Layer 2: core ──
    {
        ...
        var sdf_result = sdf_circle(p, (0.080000 + (u.mouse_down * 0.120000)));
        let glow_pulse = (2.000000 + (u.mouse_down * 2.500000)) * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(1.000000, (0.600000 + (u.mouse_down * 0.300000)), 0.200000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    // ── Layer 3: ring ──
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>(((u.mouse.x * 2.000000) - 1.000000), ((u.mouse.y * 2.000000) - 1.000000));
        var sdf_result = abs(length(p) - (0.200000 + (u.mouse_down * 0.150000))) - 0.015000;
        let glow_pulse = (1.500000 + (u.mouse_down * 1.000000)) * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.800000, 0.500000, 1.000000), color_result.a);
        let prev_color = textureSample(prev_frame, prev_sampler, input.uv);
    ...
    // ── Layer 4: edge ──
    {
        ...
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.300000, 0.300000, 0.350000), color_result.a);
        let la = color_result.a;
        let lc = color_result.rgb;
        final_color = vec4<f32>(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
...
void main(){
    ...
    // ── Layer 1: field ──
    {
        ...
        { float warp_x = fbm2(p * 2.500000 + vec2(0.0, 1.3), int(4.000000), 0.150000, 2.000000);
        float warp_y = fbm2(p * 2.500000 + vec2(1.7, 0.0), int(4.000000), 0.150000, 2.000000);
    ...
    // ── Layer 2: core ──
    {
        ...
        float sdf_result = sdf_circle(p, (0.080000 + (u_mouse_down * 0.120000)));
        float glow_pulse = (2.000000 + (u_mouse_down * 2.500000)) * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(1.000000, (0.600000 + (u_mouse_down * 0.300000)), 0.200000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.950000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
    ...
    // ── Layer 3: ring ──
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2(((u_mouse.x * 2.000000) - 1.000000), ((u_mouse.y * 2.000000) - 1.000000));
        float sdf_result = abs(length(p) - (0.200000 + (u_mouse_down * 0.150000))) - 0.015000;
        float glow_pulse = (1.500000 + (u_mouse_down * 1.000000)) * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.800000, 0.500000, 1.000000), color_result.a);
        vec4 prev_color = texture(u_prev_frame, v_uv);
        color_result = mix(color_result, prev_color, 0.880000);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
...
`;
const UNIFORMS = [{name:'intensity',default:0.5}];
const COMPLEXITY = {layers:4,fbmOctaves:9,passes:2,memory:true,compute:false,is3d:false,tier:'heavy'};
const PASS_WGSL_0 = `// Post-processing pass: glow_pass
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'intensity', target: 'core', field: 'brightness', weight: 0.3 },
      { source: 'intensity', target: 'ring', field: 'scale', weight: 0.2 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class GameArcTimeline {
  constructor() {
    ...
    this._entries = [
      { target: 'intensity', from: 0.5, to: 1, duration: 8, easing: 'ease_in_out' },
    ];
  }
...
class InteractivePulse extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-interactive-pulse: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get intensity() { return this._renderer?.userParams['intensity'] ?? this._pendingParams['intensity'] ?? 0.5; }
...
customElements.define('game-interactive-pulse', InteractivePulse);
})();
