// GAME Component: living-thunderstorm — auto-generated, do not edit.
(function(){
...
@group(0) @binding(13) var mask_vegetation_tex: texture_2d<f32>;
@group(0) @binding(14) var mask_vegetation_samp: sampler;
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p + vec2<f32>(sin(p.y * 0.350000 + time * 0.150000), cos(p.x * 0.350000 + time * 0.150000)) * 0.025000;
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>((time * 0.035000), (time * 0.008000));
        ...
        var sdf_result = fbm2((p * 0.600000 + vec2<f32>(time * 0.1, time * 0.07)), i32(5.000000), 0.600000, 2.000000);
        let glow_pulse = 1.000000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.300000, 0.320000, 0.450000), color_result.a);
        let _mask_uv = vec2<f32>(input.uv.x, 1.0 - input.uv.y);
        let _mask_raw = textureSample(mask_sky_tex, mask_sky_samp, _mask_uv).r;
        let _mask_val = mix(_mask_raw, 1.0 - _mask_raw, 0.0);
        color_result = vec4<f32>(color_result.rgb * _mask_val, color_result.a * _mask_val);
        let la = color_result.a * 0.180000;
        let lc = color_result.rgb * 0.180000;
    ...
    // ── Layer 3: lightning ──
    {
        ...
        p = p + vec2<f32>(sin(p.y * 0.150000 + time * 8.000000), cos(p.x * 0.150000 + time * 8.000000)) * 0.020000;
        { let warp_x = fbm2(p * 0.300000 + vec2<f32>(0.0, 1.3), i32(2.000000), 0.500000, 0.050000);
        let warp_y = fbm2(p * 0.300000 + vec2<f32>(1.7, 0.0), i32(2.000000), 0.500000, 0.050000);
        ...
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.700000, 0.750000, 1.000000), color_result.a);
        let _mask_uv = vec2<f32>(input.uv.x, 1.0 - input.uv.y);
        let _mask_raw = textureSample(mask_sky_tex, mask_sky_samp, _mask_uv).r;
        let _mask_val = mix(_mask_raw, 1.0 - _mask_raw, 0.0);
        color_result = vec4<f32>(color_result.rgb * _mask_val, color_result.a * _mask_val);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>((time * 0.150000), (time * 0.800000));
        p = p + vec2<f32>(sin(p.y * 25.000000 + time * 6.000000), cos(p.x * 25.000000 + time * 6.000000)) * 0.080000;
        var sdf_result = voronoi2(p * 45.000000 + vec2<f32>(time * 0.05, time * 0.03));
        let glow_pulse = 8.000000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.650000, 0.700000, 0.850000), color_result.a);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>((time * 0.120000), (time * 0.650000));
        p = p + vec2<f32>(sin(p.y * 35.000000 + time * 5.000000), cos(p.x * 35.000000 + time * 5.000000)) * 0.060000;
        var sdf_result = voronoi2(p * 55.000000 + vec2<f32>(time * 0.05, time * 0.03));
        let glow_pulse = 10.000000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.550000, 0.600000, 0.780000), color_result.a);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>((time * 0.004000), (time * 0.003000));
        p = p + vec2<f32>(sin(p.y * 8.000000 + time * 4.000000), cos(p.x * 8.000000 + time * 4.000000)) * 0.100000;
        var sdf_result = voronoi2(p * 12.000000 + vec2<f32>(time * 0.05, time * 0.03));
        let glow_pulse = 2.500000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.500000, 0.550000, 0.750000), color_result.a);
        let _mask_uv = vec2<f32>(input.uv.x, 1.0 - input.uv.y);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p + vec2<f32>(sin(p.y * 0.600000 + time * 0.250000), cos(p.x * 0.600000 + time * 0.250000)) * 0.018000;
        let _tex_uv = clamp(vec2<f32>(p.x / aspect * 0.5 + 0.5, 1.0 - (p.y * 0.5 + 0.5)), vec2<f32>(0.0), vec2<f32>(1.0));
        var color_result = textureSample(photo_tex, photo_samp, _tex_uv);
        let _mask_uv = vec2<f32>(input.uv.x, 1.0 - input.uv.y);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>((time * 0.025000), (sin((time * 0.080000)) * 0.005000));
        { let warp_x = fbm2(p * 0.700000 + vec2<f32>(0.0, 1.3), i32(4.000000), 0.600000, 0.180000);
        let warp_y = fbm2(p * 0.700000 + vec2<f32>(1.7, 0.0), i32(4.000000), 0.600000, 0.180000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.180000; }
        var sdf_result = fbm2((p * 1.000000 + vec2<f32>(time * 0.1, time * 0.07)), i32(4.000000), 0.500000, 2.000000);
        let glow_pulse = 0.800000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.350000, 0.380000, 0.550000), color_result.a);
        let _mask_uv = vec2<f32>(input.uv.x, 1.0 - input.uv.y);
    ...
    // ── Layer 9: ground_flash ──
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>((time * 0.001000), (time * 0.001000));
        p = p + vec2<f32>(sin(p.y * 0.200000 + time * 8.000000), cos(p.x * 0.200000 + time * 8.000000)) * 0.030000;
        var sdf_result = fbm2((p * 0.150000 + vec2<f32>(time * 0.1, time * 0.07)), i32(2.000000), 0.500000, 2.000000);
        let glow_pulse = 6.000000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.550000, 0.580000, 0.800000), color_result.a);
        let _mask_uv = vec2<f32>(input.uv.x, 1.0 - input.uv.y);
        let _mask_raw = textureSample(depth_tex, depth_samp, _mask_uv).r;
        let _mask_val = mix(_mask_raw, 1.0 - _mask_raw, 0.0);
        color_result = vec4<f32>(color_result.rgb * _mask_val, color_result.a * _mask_val);
...
`;
const UNIFORMS = [];
const COMPLEXITY = {layers:10,fbmOctaves:24,passes:3,memory:false,compute:false,is3d:false,tier:'extreme'};
const TEX_INDEX = {'photo': 0, 'depth': 1, 'mask_sky': 2, 'mask_water': 3, 'mask_vegetation': 4};
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let grain_val = (fract(sin(dot(grain_seed, vec2<f32>(12.9898, 78.233))) * 43758.5453) - 0.5) * 0.022000;
...
class GameRendererGL {
  ...
  render() {
    ...
    gl.uniform1i(this._texLocs['mask_vegetation'], 6);
...
class LivingThunderstorm extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-living-thunderstorm: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
    ...
    this.loadTexture('photo', 'thunderstorm.jpg').catch(e => console.warn('texture load failed:', e));
    this.loadTexture('depth', 'thunderstorm-depth.png').catch(e => console.warn('texture load failed:', e));
    this.loadTexture('mask_sky', 'thunderstorm-mask_sky.png').catch(e => console.warn('texture load failed:', e));
    this.loadTexture('mask_water', 'thunderstorm-mask_water.png').catch(e => console.warn('texture load failed:', e));
    this.loadTexture('mask_vegetation', 'thunderstorm-mask_vegetation.png').catch(e => console.warn('texture load failed:', e));
...
customElements.define('game-living-thunderstorm', LivingThunderstorm);
})();
