// GAME Component: event-horizon — auto-generated, do not edit.
(function(){
...
`;
const UNIFORMS = [{name:'color_r',default:0.3},{name:'color_g',default:0.8},{name:'color_b',default:1.5}];
...
fn cs_agent(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    let sense_l = sample_trail(agent.pos.x + cos(agent.angle + 0.5235987755982988) * 10, agent.pos.y + sin(agent.angle + 0.5235987755982988) * 10);
    let sense_f = sample_trail(agent.pos.x + cos(agent.angle) * 10, agent.pos.y + sin(agent.angle) * 10);
    let sense_r = sample_trail(agent.pos.x + cos(agent.angle - 0.5235987755982988) * 10, agent.pos.y + sin(agent.angle - 0.5235987755982988) * 10);

    // Turn toward strongest pheromone
    if (sense_f >= sense_l && sense_f >= sense_r) {
        // Keep going forward
    } else if (sense_l > sense_r) {
        agent.angle += 0.6108652381980153;
    } else if (sense_r > sense_l) {
        agent.angle -= 0.6108652381980153;
    } else {
        agent.angle += (rng - 0.5) * 0.6108652381980153 * 2.0;
    }
...
class GameSwarmSim {
  ...
  dispatch(dt) {
    ...
    f[0] = 0.5235987755982988; f[1] = 10; f[2] = 0.6108652381980153; f[3] = 1; f[4] = 5;
...
class EventHorizon extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-event-horizon: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
  ...
  get color_g() { return this._renderer?.userParams['color_g'] ?? this._pendingParams['color_g'] ?? 0.8; }
...
customElements.define('game-event-horizon', EventHorizon);
})();
