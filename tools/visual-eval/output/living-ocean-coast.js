// GAME Component: living-ocean-coast — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // ── Layer 0: world ──
    {
        ...
        let _px_orbit = vec2<f32>(sin(time * 0.060000), cos(time * 0.060000 * 0.7)) * 0.020000;
    ...
    // ── Layer 1: wave_flow ──
    {
        ...
        let _fm_phase0 = fract(time * 0.180000);
        let _fm_phase1 = fract(time * 0.180000 + 0.5);
        ...
        let la = color_result.a * 0.920000;
        let lc = color_result.rgb * 0.920000;
        final_color = vec4<f32>(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p + vec2<f32>(sin(p.y * 0.250000 + time * 0.040000), cos(p.x * 0.250000 + time * 0.040000)) * 0.012000;
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>((time * 0.015000), (time * 0.035000));
        { let warp_x = fbm2(p * 1.800000 + vec2<f32>(0.0, 1.3), i32(4.000000), 0.550000, 0.300000);
        let warp_y = fbm2(p * 1.800000 + vec2<f32>(1.7, 0.0), i32(4.000000), 0.550000, 0.300000);
        ...
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.950000, 0.970000, 1.000000), color_result.a);
        let _mask_uv = vec2<f32>(input.uv.x, 1.0 - input.uv.y);
        let _mask_raw = textureSample(mask_water_tex, mask_water_samp, _mask_uv).r;
        let _mask_val = mix(_mask_raw, 1.0 - _mask_raw, 0.0);
        color_result = vec4<f32>(color_result.rgb * _mask_val, color_result.a * _mask_val);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>((time * 0.008000), (time * 0.020000));
        { let warp_x = fbm2(p * 0.900000 + vec2<f32>(0.0, 1.3), i32(3.000000), 0.500000, 0.180000);
        let warp_y = fbm2(p * 0.900000 + vec2<f32>(1.7, 0.0), i32(3.000000), 0.500000, 0.180000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.180000; }
        var sdf_result = voronoi2(p * 4.000000 + vec2<f32>(time * 0.05, time * 0.03));
        let glow_pulse = 4.500000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>((time * 0.025000), (time * 0.012000));
        { let warp_x = fbm2(p * 3.500000 + vec2<f32>(0.0, 1.3), i32(3.000000), 0.200000, 2.000000);
        let warp_y = fbm2(p * 3.500000 + vec2<f32>(1.7, 0.0), i32(3.000000), 0.200000, 2.000000);
        ...
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.300000, 0.650000, 0.850000), color_result.a);
        let _mask_uv = vec2<f32>(input.uv.x, 1.0 - input.uv.y);
        let _mask_raw = textureSample(mask_water_tex, mask_water_samp, _mask_uv).r;
        let _mask_val = mix(_mask_raw, 1.0 - _mask_raw, 0.0);
        color_result = vec4<f32>(color_result.rgb * _mask_val, color_result.a * _mask_val);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>((time * 0.006000), (sin((time * 0.150000)) * 0.008000));
        { let warp_x = fbm2(p * 1.200000 + vec2<f32>(0.0, 1.3), i32(5.000000), 0.600000, 0.250000);
        let warp_y = fbm2(p * 1.200000 + vec2<f32>(1.7, 0.0), i32(5.000000), 0.600000, 0.250000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.250000; }
        var sdf_result = fbm2((p * 1.800000 + vec2<f32>(time * 0.1, time * 0.07)), i32(5.000000), 0.500000, 2.000000);
        let glow_pulse = 1.000000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.900000, 0.930000, 1.000000), color_result.a);
        let _mask_uv = vec2<f32>(input.uv.x, 1.0 - input.uv.y);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>((time * 0.010000), (sin((time * 0.060000)) * 0.004000));
        { let warp_x = fbm2(p * 0.600000 + vec2<f32>(0.0, 1.3), i32(4.000000), 0.550000, 0.140000);
        let warp_y = fbm2(p * 0.600000 + vec2<f32>(1.7, 0.0), i32(4.000000), 0.550000, 0.140000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.140000; }
        var sdf_result = fbm2((p * 0.800000 + vec2<f32>(time * 0.1, time * 0.07)), i32(4.000000), 0.500000, 2.000000);
        let glow_pulse = 1.200000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.750000, 0.820000, 0.950000), color_result.a);
        let _mask_uv = vec2<f32>(input.uv.x, 1.0 - input.uv.y);
        let _mask_raw = textureSample(depth_tex, depth_samp, _mask_uv).r;
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>((sin((time * 0.080000)) * 0.003000), (time * 0.002000));
        ...
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.600000, 0.780000, 0.950000), color_result.a);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        p = p - vec2<f32>((time * 0.003000), (time * 0.001000));
        { let warp_x = fbm2(p * 0.200000 + vec2<f32>(0.0, 1.3), i32(2.000000), 0.500000, 0.040000);
        let warp_y = fbm2(p * 0.200000 + vec2<f32>(1.7, 0.0), i32(2.000000), 0.500000, 0.040000);
        ...
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.850000, 0.800000, 0.720000), color_result.a);
        let _mask_uv = vec2<f32>(input.uv.x, 1.0 - input.uv.y);
        let _mask_raw = textureSample(mask_sky_tex, mask_sky_samp, _mask_uv).r;
        let _mask_val = mix(_mask_raw, 1.0 - _mask_raw, 0.0);
        color_result = vec4<f32>(color_result.rgb * _mask_val, color_result.a * _mask_val);
...
`;
const UNIFORMS = [];
const COMPLEXITY = {layers:10,fbmOctaves:37,passes:3,memory:false,compute:false,is3d:false,tier:'extreme'};
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let grain_val = (fract(sin(dot(grain_seed, vec2<f32>(12.9898, 78.233))) * 43758.5453) - 0.5) * 0.016000;
...
class LivingOceanCoast extends HTMLElement {
  constructor() {
    super();
  ...
  async _initRenderer() {
    ...
    if (await gpu.init()) {
      ...
      if (gl.init()) {
        ...
        console.warn('game-living-ocean-coast: no WebGPU or WebGL2 support');
        return;
      }
    }
    this._resize();
    ...
    this.loadTexture('photo', 'ocean-coast.jpg').catch(e => console.warn('texture load failed:', e));
    this.loadTexture('depth', 'ocean-coast-depth.png').catch(e => console.warn('texture load failed:', e));
    this.loadTexture('flow', 'ocean-coast-flow.png').catch(e => console.warn('texture load failed:', e));
    this.loadTexture('mask_water', 'ocean-coast-mask_water.png').catch(e => console.warn('texture load failed:', e));
    this.loadTexture('mask_sky', 'ocean-coast-mask_sky.png').catch(e => console.warn('texture load failed:', e));
...
customElements.define('game-living-ocean-coast', LivingOceanCoast);
})();
