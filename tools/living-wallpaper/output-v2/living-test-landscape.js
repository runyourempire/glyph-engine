...
void main(){
    ...
    // ── Layer 1: world ──
    {
        ...
        vec2 _px_orbit = vec2(sin(time * 0.080000), cos(time * 0.080000 * 0.7)) * 0.003000;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2((time * 0.014000), (time * 0.007000));
        { float warp_x = fbm2(p * 6.000000 + vec2(0.0, 1.3), int(2.000000), 0.120000, 2.000000);
        float warp_y = fbm2(p * 6.000000 + vec2(1.7, 0.0), int(2.000000), 0.120000, 2.000000);
        ...
        color_result = vec4(color_result.rgb * vec3(0.500000, 0.700000, 1.000000), color_result.a);
        ...
        float la = color_result.a * 0.018000;
        vec3 lc = color_result.rgb * 0.018000;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2((time * 0.020000), (time * 0.010000));
        p = p + vec2(sin(p.y * 15.000000 + time * 0.800000), cos(p.x * 15.000000 + time * 0.800000)) * 0.300000;
        ...
        float _mask_val = mix(_mask_raw, 1.0 - _mask_raw, 0.0);
        color_result = vec4(color_result.rgb * _mask_val, color_result.a * _mask_val);
        float la = color_result.a * 0.009000;
        vec3 lc = color_result.rgb * 0.009000;
        final_color = vec4(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2((time * drift_x), (sin((time * 0.040000)) * 0.003000));
        { float warp_x = fbm2(p * 1.200000 + vec2(0.0, 1.3), int(4.000000), 0.650000, 0.100000);
        float warp_y = fbm2(p * 1.200000 + vec2(1.7, 0.0), int(4.000000), 0.650000, 0.100000);
        p = p + vec2(warp_x, warp_y) * 0.100000; }
        float sdf_result = fbm2((p * 1.800000 + vec2(time * 0.1, time * 0.07)), int(4.000000), 0.550000, 2.000000);
        float glow_pulse = 0.500000 * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.900000, 0.850000, 0.800000), color_result.a);
        vec2 _mask_uv = vec2(v_uv.x, 1.0 - v_uv.y);
        float _mask_raw = texture(u_tex_depth, _mask_uv).r;
        float _mask_val = mix(_mask_raw, 1.0 - _mask_raw, 1.000000);
        color_result = vec4(color_result.rgb * _mask_val, color_result.a * _mask_val);
        float la = color_result.a * 0.015000;
        vec3 lc = color_result.rgb * 0.015000;
        final_color = vec4(vec3(1.0) - (vec3(1.0) - final_color.rgb) * (vec3(1.0) - lc), max(final_color.a, la));
    }
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p - vec2((time * 0.004000), (time * 0.001000));
        { float warp_x = fbm2(p * 0.600000 + vec2(0.0, 1.3), int(2.000000), 0.500000, 0.060000);
        float warp_y = fbm2(p * 0.600000 + vec2(1.7, 0.0), int(2.000000), 0.500000, 0.060000);
        p = p + vec2(warp_x, warp_y) * 0.060000; }
        float sdf_result = fbm2((p * 0.500000 + vec2(time * 0.1, time * 0.07)), int(3.000000), 0.500000, 2.000000);
    ...
    // ── Layer 6: godrays ──
    {
        ...
        p = p + vec2(sin(p.y * 0.400000 + time * 0.025000), cos(p.x * 0.400000 + time * 0.025000)) * 0.012000;
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        p = p + vec2(sin(p.y * 0.600000 + time * 0.030000), cos(p.x * 0.600000 + time * 0.030000)) * 0.005000;
...
`;
const UNIFORMS = [{name:'drift_x',default:0.008},{name:'drift_y',default:0}];
const COMPLEXITY = {layers:7,fbmOctaves:15,passes:3,memory:false,compute:false,is3d:false,tier:'extreme'};
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let vign = 1.0 - 0.150000 * length(uv - 0.5);
...
class LivingTestLandscape extends HTMLElement {
  ...
  get drift_x() { return this._renderer?.userParams['drift_x'] ?? this._pendingParams['drift_x'] ?? 0.008; }
  set drift_x(v) { this.setParam('drift_x', v); }
  get drift_y() { return this._renderer?.userParams['drift_y'] ?? this._pendingParams['drift_y'] ?? 0; }
  set drift_y(v) { this.setParam('drift_y', v); }
